...
var _fq8CapMeta = [
  {n:1, icon:'<i class="ph ph-flask"></i>',          label:'Reações Químicas'},
  {n:2, icon:'<i class="ph ph-wave-sine"></i>',      label:'Som'},
  {n:3, icon:'<i class="ph ph-sun"></i>',            label:'Luz'}
];

// Cores por capítulo
var _fq8CapColors = {
  1:'#4d978f', 2:'#a8854d', 3:'#4f86b0'
};

// Subtemas por capítulo (programa FQ 8.º — Universo FQ8)
var _fq8Subtemas = {
  1: ['Transformações físicas e químicas', 'Reagentes e produtos', 'Tipos de reações', 'Ácidos e bases', 'Velocidade das reações'],
  2: ['Produção e propagação do som', 'Características do som', 'Audição e ruído', 'Velocidade do som'],
  3: ['Propagação da luz', 'Reflexão e espelhos', 'Refração e lentes', 'Cor e espetro']
};
...
var _fq8Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function fq8SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('fq8p-' + tab);
  ...
  if (titles[tab]) document.title = 'Físico-Química 8.º ' + titles[tab] + ' · 3ponto14';

  if (tab === 'resumo') fq8BuildResumoNav();
  else if (tab === 'exercicios') fq8BuildPraticarNav();
  else if (tab === 'quiz') fq8QuizBuildNav();
...
function fq8OpenPraticar(modo) { fq8SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function fq8BuildResumoNav() {
  var capRow = document.getElementById('fq8-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _fq8Sel['resumo'] || 1;
  var h = '';
  _fq8CapMeta.forEach(function(m) {
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="fq8ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  fq8ResumoShowSts(activeCap);
  fq8RenderResumoInline();
}

function fq8ResumoSelectCap(cap, btn) {
  _fq8Sel['resumo'] = cap;
  _fq8Sel['resumo-st'] = 0;
  var capRow = document.getElementById('fq8-resumo-cap-row');
  if (capRow) {
  ...
  fq8ResumoShowSts(cap);
  fq8RenderResumoInline();
}

function fq8ResumoShowSts(cap) {
  var stRow = document.getElementById('fq8-resumo-st-row');
  if (!stRow) return;
...
function fq8ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('fq8-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _fq8Sel['resumo'] = cap;
  _fq8Sel['resumo-st'] = stIdx;
  fq8RenderResumoInline();
}

function fq8RenderResumoInline() {
  var cap = _fq8Sel['resumo'] || 1;
  var dest = document.getElementById('fq8-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _fq8CapColors[cap] || '#516860';
    return c;
  })();
  ...
  _fq8PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
/* ═══ BANCO DE EXERCÍCIOS FQ 7.º (conceptual) ═══
   ...
   Cap 1 (Universo) baseado no Universo FQ8. Caps 2-4 starters. */
var _fq8Banco = {
  1: [
    { t:1, tipo:'mc', enun:'Qual destas é uma transformação <strong>física</strong>?', opcoes:['Derreter gelo','Queimar madeira','O ferro enferrujar','Cozer um ovo'], resposta:'Derreter gelo', expl:'Derreter gelo não forma novas substâncias (continua a ser água) → física.' },
    { t:1, tipo:'mc', enun:'Qual destas é uma transformação <strong>química</strong>?', opcoes:['Queimar papel','Partir um copo','Evaporar água','Esticar um elástico'], resposta:'Queimar papel', expl:'Ao queimar formam-se novas substâncias → química.' },
    { t:1, tipo:'vf', enun:'Numa transformação química formam-se novas substâncias.', opcoes:['Verdadeiro','Falso'], resposta:'Verdadeiro', expl:'É essa a definição de reação química.' },
    { t:2, tipo:'mc', enun:'Numa reação química, as substâncias do início chamam-se:', opcoes:['reagentes','produtos','catalisadores','indicadores'], resposta:'reagentes', expl:'Reagentes → produtos.' },
    { t:2, tipo:'mc', enun:'As novas substâncias formadas numa reação chamam-se:', opcoes:['produtos','reagentes','soluções','iões'], resposta:'produtos', expl:'Os produtos estão à direita da seta.' },
    { t:2, tipo:'vf', enun:'A Lei de Lavoisier diz que a massa se conserva numa reação química.', opcoes:['Verdadeiro','Falso'], resposta:'Verdadeiro', expl:'Massa dos reagentes = massa dos produtos.' },
    { t:3, tipo:'mc', enun:'A combustão é uma reação que liberta:', opcoes:['energia (calor e luz)','apenas água','um gás frio','eletrões'], resposta:'energia (calor e luz)', expl:'A combustão é rápida e liberta energia.' },
    { t:3, tipo:'mc', enun:'O ferro a enferrujar é um exemplo de:', opcoes:['oxidação (reação lenta)','combustão','transformação física','neutralização'], resposta:'oxidação (reação lenta)', expl:'A ferrugem é uma oxidação lenta com o oxigénio.' },
    { t:4, tipo:'mc', enun:'Uma solução com pH = 3 é:', opcoes:['ácida','neutra','básica','sem pH'], resposta:'ácida', expl:'pH < 7 → ácida.' },
    { t:4, tipo:'mc', enun:'Uma solução com pH = 10 é:', opcoes:['básica','ácida','neutra','gasosa'], resposta:'básica', expl:'pH > 7 → básica (alcalina).' },
    { t:4, tipo:'mc', enun:'A água pura tem pH:', opcoes:['7 (neutro)','1','14','0'], resposta:'7 (neutro)', expl:'pH = 7 é neutro.' },
  ...
  2: [
    ...
    { t:1, tipo:'vf', enun:'O som propaga-se no vácuo.', opcoes:['Verdadeiro','Falso'], resposta:'Falso', expl:'O som precisa de um meio material; no vácuo não há som.' },
    { t:1, tipo:'mc', enun:'Em que meio o som se propaga mais depressa?', opcoes:['nos sólidos','nos gases','no vácuo','no ar frio'], resposta:'nos sólidos', expl:'Sólidos > líquidos > gases.' },
    { t:4, tipo:'mc', enun:'A velocidade do som no ar é aproximadamente:', opcoes:['340 m/s','300 000 km/s','3 m/s','34 m/s'], resposta:'340 m/s', expl:'~340 m/s no ar (a luz é muito mais rápida).' },
    { t:2, tipo:'mc', enun:'A frequência de um som mede-se em:', opcoes:['hertz (Hz)','decibéis (dB)','metros (m)','newton (N)'], resposta:'hertz (Hz)', expl:'A frequência mede-se em hertz.' },
    { t:2, tipo:'mc', enun:'Um som AGUDO tem:', opcoes:['frequência alta','frequência baixa','amplitude zero','timbre fixo'], resposta:'frequência alta', expl:'Agudo = alta frequência; grave = baixa frequência.' },
    { t:2, tipo:'mc', enun:'O "volume" (intensidade) do som está relacionado com:', opcoes:['a amplitude','a frequência','o timbre','a velocidade'], resposta:'a amplitude', expl:'Maior amplitude → som mais forte.' },
    { t:2, tipo:'mc', enun:'O que permite distinguir um piano de um violino a tocar a mesma nota?', opcoes:['o timbre','a frequência','o volume','a velocidade'], resposta:'o timbre', expl:'O timbre é a "qualidade" do som de cada fonte.' },
    { t:3, tipo:'mc', enun:'O ouvido humano deteta aproximadamente:', opcoes:['20 Hz a 20 000 Hz','0 a 10 Hz','só acima de 50 000 Hz','qualquer frequência'], resposta:'20 Hz a 20 000 Hz', expl:'Abaixo são infrassons; acima são ultrassons.' },
    { t:3, tipo:'mc', enun:'A intensidade sonora mede-se em:', opcoes:['decibéis (dB)','hertz (Hz)','metros (m)','watt (W)'], resposta:'decibéis (dB)', expl:'Mede-se em dB; >85 dB prolongado é prejudicial.' },
  ...
  3: [
    ...
    { t:1, tipo:'vf', enun:'A luz, ao contrário do som, propaga-se no vácuo.', opcoes:['Verdadeiro','Falso'], resposta:'Verdadeiro', expl:'A luz não precisa de meio material (chega-nos do Sol pelo espaço).' },
    { t:1, tipo:'mc', enun:'Qual destas é uma fonte de luz PRIMÁRIA?', opcoes:['O Sol','A Lua','Um espelho','Um livro'], resposta:'O Sol', expl:'Primária = luz própria. A Lua/espelho refletem (secundárias).' },
    { t:1, tipo:'mc', enun:'Um corpo TRANSPARENTE:', opcoes:['deixa passar a luz e ver através','não deixa passar luz','só deixa passar som','reflete tudo'], resposta:'deixa passar a luz e ver através', expl:'Ex.: vidro. Opaco não deixa passar; translúcido deixa passar mas não ver bem.' },
    { t:2, tipo:'mc', enun:'A reflexão da luz acontece quando a luz:', opcoes:['volta para trás ao bater numa superfície','passa de um meio para outro','desaparece','muda de cor só por si'], resposta:'volta para trás ao bater numa superfície', expl:'É o que acontece nos espelhos.' },
    { t:2, tipo:'mc', enun:'Na lei da reflexão, o ângulo de incidência é:', opcoes:['igual ao ângulo de reflexão','o dobro','metade','zero'], resposta:'igual ao ângulo de reflexão', expl:'Ângulo de incidência = ângulo de reflexão.' },
    { t:2, tipo:'mc', enun:'A imagem num espelho plano é:', opcoes:['virtual, direita e do mesmo tamanho','real e invertida','maior que o objeto','menor que o objeto'], resposta:'virtual, direita e do mesmo tamanho', expl:'Fica à mesma distância atrás do espelho e troca esquerda↔direita.' },
    { t:3, tipo:'mc', enun:'A refração da luz é:', opcoes:['o desvio ao passar de um meio para outro','a luz a voltar para trás','a luz a desaparecer','a produção de som'], resposta:'o desvio ao passar de um meio para outro', expl:'Ex.: a palhinha parece "partida" dentro de água.' },
    { t:3, tipo:'mc', enun:'Uma lente CONVERGENTE é mais espessa:', opcoes:['no centro','nas bordas','em todo o lado igual','não tem espessura'], resposta:'no centro', expl:'Convergente: grossa no centro, concentra os raios (lupa).' },
    { t:3, tipo:'mc', enun:'Uma lente DIVERGENTE usa-se para corrigir:', opcoes:['a miopia','a fome','a surdez','a febre'], resposta:'a miopia', expl:'Divergente (fina no centro) corrige a miopia.' },
...
function _fq8Gerador(cap) {
  // FQ é conceptual → usa banco de perguntas por capítulo (não geradores numéricos)
  if (typeof _fq8Banco !== 'undefined' && _fq8Banco[cap] && _fq8Banco[cap].length) {
    return function (tema, tipo, dif) { return _fq8PuxaBanco(cap, tema); };
  }
  return null;
}
var _fq8TemasCount = { 1: 5, 2: 4, 3: 4 };

/* Puxa uma pergunta aleatória do banco do capítulo (opcionalmente do tema). */
var _fq8BancoUsadas = {};
function _fq8PuxaBanco(cap, tema) {
  var pool = (_fq8Banco[cap] || []).filter(function (q) { return !tema || String(q.t) === String(tema); });
  if (!pool.length) pool = _fq8Banco[cap] || [];
...
// Estado da prática
var _fq8Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _fq8TarefaAtiva=null, _fq8TarefaResp={};

function _fq8PratStorageKey(cap) { return 'edupt_fq8_cap' + cap; }

function _fq8SelCfg() {
  return { capMeta: _fq8CapMeta, capColors: _fq8CapColors, subtemas: _fq8Subtemas,
    subtemaTemas: _fq8SubtemaTemas, temasCount: _fq8TemasCount, gerador: _fq8Gerador };
}
function _fq8PratSel() {
  if (!_fq8Prat.sel) {
    _fq8Prat.sel = _selNew();
    if (_fq8Prat.cap) { _fq8Prat.sel.caps = [_fq8Prat.cap]; if (_fq8Prat.st) _fq8Prat.sel.stsByCap[_fq8Prat.cap] = [_fq8Prat.st]; }
  }
  return _fq8Prat.sel;
}

function fq8BuildPraticarNav() {
  var capRow = document.getElementById('fq8-praticar-cap-row');
  if (!capRow) return;
  var stRow = document.getElementById('fq8-praticar-st-row');
  if (stRow) stRow.style.display = 'none';
  capRow.innerHTML = _selBarsHTML(_fq8PratSel(), _fq8SelCfg(), 'fq8PratToggleCap', 'fq8PratToggleSt');
}

function fq8PratToggleCap(cap) {
  _selToggleCap(_fq8PratSel(), _fq8SelCfg(), cap);
  fq8BuildPraticarNav();
  fq8GerarExercicios();
}
function fq8PratToggleSt(cap, st) {
  _selToggleSt(_fq8PratSel(), cap, st);
  fq8BuildPraticarNav();
  fq8GerarExercicios();
}

function fq8PraticarSetNivel(nivel, btn) {
  _fq8Prat.nivel = nivel;
  var grp = document.getElementById('fq8-praticar-nivel');
...
function fq8GerarExercicios() {
  var dest = document.getElementById('fq8-praticar-content');
  if (!dest) return;
  var pares = _selPares(_fq8PratSel(), _fq8SelCfg());
  ...
  var cap = _fq8PratSel().caps[0] || (pares[0] && pares[0].cap) || 1;
  ...
  for (var i = 0; i < QTD; i++) {
    ...
    var gen = _fq8Gerador(par.cap);
    if (!gen) continue;
    var ex = gen(par.tema, tipos[i % tipos.length], _fq8Prat.nivel);
    if (ex) geradas.push(ex);
  }
  var temasPorCap = {};
  pares.forEach(function (pr) { (temasPorCap[pr.cap] = temasPorCap[pr.cap] || []).push(pr.tema); });
  var banco = [];
  if (typeof _fq8Banco !== "undefined") {
    Object.keys(temasPorCap).forEach(function (c) {
      if (_fq8Banco[c]) banco = banco.concat(_fq8Banco[c].filter(function (q) { return temasPorCap[c].indexOf(String(q.t)) !== -1; }));
    });
  }
  var exs = (typeof _mixBancoGeradas === "function") ? _mixBancoGeradas(banco, geradas, QTD, 2, _fq8Prat.nivel)
    : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  _fq8Prat.exs = exs;
  _fq8Prat.answered = {};
  _fq8Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="fq8-prat-scorebar">'
    + '<div><div class="score-num" id="fq8-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="fq8-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="fq8-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="fq8GerarExercicios()">↺ Novas questões</button>'
    + (_fq8TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="fq8EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'fq8CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="fq8-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  _fq8Prat.cap = cap;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _fq8CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _selA = _fq8PratSel();
    var _stA = (_selA.caps.length === 1 && (_selA.stsByCap[cap] || []).length === 1) ? _selA.stsByCap[cap][0] : 0;
    var _sn = (_fq8Subtemas[cap] && _stA > 0) ? (_fq8Subtemas[cap][_stA - 1] || '') : '';
    Atribuir.montar('fq8-atribuir', { curso: 'fq8', cursoNome: 'Físico-Química 8.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_stA || ''), subNome: _sn, tipo: 'quiz', nivel: _fq8Prat.nivel });
  }
}

function fq8CheckEx(qid, tipo, val, btn) {
  if (_fq8Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _fq8Prat.answered[qid] = true;
  if (r.correct) _fq8Prat.score.correct++;
  _fq8Prat.score.total++; if(_fq8TarefaAtiva){_fq8TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('fq8-prat-score'); if (se) se.textContent = _fq8Prat.score.correct;
  var te = document.getElementById('fq8-prat-total'); if (te) te.textContent = '/ ' + _fq8Prat.score.total;
  var pe = document.getElementById('fq8-prat-prog'); if (pe && _fq8Prat.exs.length) pe.style.width = (_fq8Prat.score.total / _fq8Prat.exs.length * 100) + '%';
  fq8SaveProgress(_fq8Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function fq8SaveProgress(cap, correct) {
  try {
    var key = _fq8PratStorageKey(cap);
...
// ('fq8capN') para não misturar com o progresso do 7.º ano.
function _fq8PM(cap, tipo, opts) {
  if (typeof ProgressManager === 'undefined') return;
  try { ProgressManager.record('fq8cap' + cap, tipo, opts || {}); } catch (e) {}
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _fq8BuildCapRow(rowId, activeCap, onclickName) {
  var row = document.getElementById(rowId);
  if (!row) return;
  var h = '';
  _fq8CapMeta.forEach(function(m) {
    var hasGen = !!_fq8Gerador(m.n);
    var color = _fq8CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
...
function _fq8SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
  ...
  if (btn) { var color = _fq8CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background=color; btn.style.borderColor=color; btn.style.color='#fff'; }
}

// Gera uma questão de escolha múltipla para um capítulo (usada por quiz).
function _fq8BuildMcQuestion(cap) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _fq8Quiz = { cap: 1, sel: null, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function _fq8QuizSel() { if (!_fq8Quiz.sel) _fq8Quiz.sel = _selNew(); return _fq8Quiz.sel; }

function fq8QuizBuildNav() {
  var row = document.getElementById('fq8-quiz-cap-row');
  if (row) row.innerHTML = _selBarsHTML(_fq8QuizSel(), _fq8SelCfg(), 'fq8QuizToggleCap', 'fq8QuizToggleSt');
  var st = document.getElementById('fq8-quiz-st-row'); if (st) st.style.display = 'none';
  fq8QuizStart();
}

function fq8QuizToggleCap(cap) {
  _selToggleCap(_fq8QuizSel(), _fq8SelCfg(), cap);
  fq8QuizBuildNav();
}
function fq8QuizToggleSt(cap, st) {
  _selToggleSt(_fq8QuizSel(), cap, st);
  fq8QuizBuildNav();
}

function fq8QuizStart() {
  _fq8Quiz.lives = 3; _fq8Quiz.streak = 0; _fq8Quiz.maxStreak = 0;
  _fq8Quiz.score = 0; _fq8Quiz.total = 0; _fq8Quiz.answered = false;
  fq8QuizNext();
}

function fq8QuizNext() {
  var app = document.getElementById('fq8-quiz-app');
  if (!app) return;
  if (_fq8Quiz.lives <= 0) { fq8QuizGameOver(app); return; }
  var _qpares = _selPares(_fq8QuizSel(), _fq8SelCfg());
  _fq8Quiz.cap = (_fq8QuizSel().caps[0]) || (_qpares[0] && _qpares[0].cap) || 1;
  var ex = _selMcQuestion(_fq8QuizSel(), _fq8SelCfg());
  ...
  _fq8Quiz.current = ex; _fq8Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _fq8Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="fq8qopt-' + idx + '" onclick="fq8QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_fq8Quiz.streak > 1 ? '🔥 ' + _fq8Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _fq8Quiz.score + ' / ' + _fq8Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="fq8-quiz-fb" style="min-height:2.5rem"></div>';
}

function fq8QuizAnswer(idx) {
  if (_fq8Quiz.answered) return;
  _fq8Quiz.answered = true;
  var ex = _fq8Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('fq8qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('fq8-quiz-fb');
  if (correct) {
    _fq8Quiz.score++; _fq8Quiz.streak++;
    if (_fq8Quiz.streak > _fq8Quiz.maxStreak) _fq8Quiz.maxStreak = _fq8Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _fq8Quiz.lives--; _fq8Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  fq8SaveProgress(_fq8Quiz.cap, correct);
  setTimeout(function(){ fq8QuizNext(); }, 1400);
}

function fq8QuizGameOver(app) {
  var pct = _fq8Quiz.total > 0 ? Math.round(_fq8Quiz.score / _fq8Quiz.total * 100) : 0;
  _fq8PM(_fq8Quiz.cap, 'quiz', { pontuacao: _fq8Quiz.score, total: _fq8Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _fq8Quiz.score + ' certas em ' + _fq8Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_fq8Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="fq8QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _fq8Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function fq8FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_fq8Cards[_fq8Fc.cap] && _fq8Cards[_fq8Fc.cap].length)) {
    for (var c = 1; c <= 8; c++) { if (_fq8Cards[c] && _fq8Cards[c].length) { _fq8Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('fq8-fc-cap-row');
  if (row) {
    var h = '';
    _fq8CapMeta.forEach(function(m) {
      var has = !!(_fq8Cards[m.n] && _fq8Cards[m.n].length);
      var color = _fq8CapColors[m.n] || '#516860';
      var isActive = _fq8Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('fq8FcSelectCap(' + m.n + ',this)') : '';
  ...
  fq8FcSelectCap(_fq8Fc.cap, null);
}

function fq8FcSelectCap(cap, btn) {
  if (!(_fq8Cards[cap] && _fq8Cards[cap].length)) return;
  if (btn) _fq8SetActiveCapBtn('fq8-fc-cap-row', btn, cap);
  _fq8Fc.cap = cap;
  _fq8Fc.cards = _fq8Cards[cap].slice();
  _fq8Fc.idx = 0; _fq8Fc.flipped = false;
  _fq8PM(cap, 'flashcard');
  fq8FcRender();
}

function fq8FcRender() {
  var app = document.getElementById('fq8-fc-app');
  if (!app) return;
  var cards = _fq8Fc.cards;
  ...
  var card = cards[_fq8Fc.idx];
  var color = _fq8CapColors[_fq8Fc.cap] || '#516860';
  var pct = Math.round((_fq8Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_fq8Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="fq8FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_fq8Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_fq8Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="fq8FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="fq8FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="fq8FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function fq8FcFlip() { _fq8Fc.flipped = !_fq8Fc.flipped; fq8FcRender(); }
function fq8FcNext() { _fq8Fc.idx = (_fq8Fc.idx + 1) % (_fq8Fc.cards.length || 1); _fq8Fc.flipped = false; fq8FcRender(); }
function fq8FcPrev() { _fq8Fc.idx = (_fq8Fc.idx - 1 + (_fq8Fc.cards.length || 1)) % (_fq8Fc.cards.length || 1); _fq8Fc.flipped = false; fq8FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _fq8Teste = { cap: 1, sel: null, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function _fq8TesteSel() { if (!_fq8Teste.sel) _fq8Teste.sel = _selNew(); return _fq8Teste.sel; }

function fq8TesteBuildNav() {
  var row = document.getElementById('fq8-teste-cap-row');
  if (row) row.innerHTML = _selBarsHTML(_fq8TesteSel(), _fq8SelCfg(), 'fq8TesteToggleCap', 'fq8TesteToggleSt');
  var st = document.getElementById('fq8-teste-st-row'); if (st) st.style.display = 'none';
  fq8TesteRenderConfig();
}

function fq8TesteToggleCap(cap) {
  _selToggleCap(_fq8TesteSel(), _fq8SelCfg(), cap);
  fq8TesteBuildNav();
}
function fq8TesteToggleSt(cap, st) {
  _selToggleSt(_fq8TesteSel(), cap, st);
  fq8TesteBuildNav();
}

function fq8TesteRenderConfig() {
  var app = document.getElementById('fq8-teste-app');
  if (!app) return;
  if (_fq8Teste.timer) { clearInterval(_fq8Teste.timer); _fq8Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="fq8-teste-nivel">' +
            '<button class="gen-level-btn' + (_fq8Teste.nivel==='facil'?' active':'') + '" onclick="fq8TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_fq8Teste.nivel==='medio'?' active':'') + '" onclick="fq8TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_fq8Teste.nivel==='dificil'?' active':'') + '" onclick="fq8TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="fq8-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="fq8-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="fq8TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function fq8TesteSetNivel(nivel, btn) {
  _fq8Teste.nivel = nivel;
  var grp = document.getElementById('fq8-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function fq8TesteStart() {
  var pares = _selPares(_fq8TesteSel(), _fq8SelCfg());
  ...
  _fq8Teste.cap = (_fq8TesteSel().caps[0]) || (pares[0] && pares[0].cap) || 1;
  var qtdEl = document.getElementById('fq8-teste-qtd'), tempoEl = document.getElementById('fq8-teste-tempo');
  _fq8Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _fq8Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
  var exs = [];
  for (var i = 0; i < _fq8Teste.qtd; i++) {
    var par = pares[i % pares.length];
    var gen = _fq8Gerador(par.cap); if (!gen) continue;
    var ex = gen(par.tema, tipos[i % tipos.length], _fq8Teste.nivel);
    if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
  }
  _fq8Teste.exs = exs; _fq8Teste.answered = {}; _fq8Teste.score = { correct: 0, total: 0 };
  _fq8Teste.restante = _fq8Teste.tempo;

  var app = document.getElementById('fq8-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'fq8TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="fq8-teste-timer">' + _fq8FmtTime(_fq8Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="fq8-teste-answered">0 / ' + _fq8Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="fq8TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="fq8-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _fq8Teste.timer = setInterval(function() {
    _fq8Teste.restante--;
    var t = document.getElementById('fq8-teste-timer');
    if (t) t.textContent = _fq8FmtTime(_fq8Teste.restante);
    if (_fq8Teste.restante <= 0) fq8TesteFinish();
  }, 1000);
}

function _fq8FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function fq8TesteCheck(qid, tipo, val, btn) {
  if (_fq8Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _fq8Teste.answered[qid] = true;
  if (r.correct) _fq8Teste.score.correct++;
  _fq8Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('fq8-teste-answered');
  if (a) a.textContent = _fq8Teste.score.total + ' / ' + _fq8Teste.qtd;
  fq8SaveProgress(_fq8Teste.cap, r.correct);
  if (_fq8Teste.score.total >= _fq8Teste.qtd) setTimeout(fq8TesteFinish, 600);
}

function fq8TesteFinish() {
  if (_fq8Teste.timer) { clearInterval(_fq8Teste.timer); _fq8Teste.timer = null; }
  var app = document.getElementById('fq8-teste-app');
  if (!app) return;
  var total = _fq8Teste.qtd;
  var correct = _fq8Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _fq8PM(_fq8Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="fq8TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _fq8JogosInited = false;
function fq8JogosInit() {
  (function(){ var pj=document.getElementById('fq8p-jogos'); if(pj && !document.getElementById('fq8-jogos-atr')){ var d=document.createElement('div'); d.id='fq8-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('fq8-jogos-atr',{curso:'fq8',cursoNome:'Físico-Química 8.º',tipo:'jogo',nivel:'',caps:_fq8CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  ...
  if (typeof _j24AutoInit === 'function') {
    ...
    _fq8JogosInited = true;
  } else {
    var app = document.getElementById('fq8-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_fq8_capN (gravado por fq8SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _fq8ProgGetCaps() {
  var out = [];
  for (var cap = 1; cap <= 8; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_fq8_cap' + cap) || '{}');
...
function fq8RenderProgresso() {
  var el = document.getElementById('fq8-progresso-content');
  if (!el) return;
  var caps = _fq8ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="fq8ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m8c1-base);border:1.5px solid var(--m8c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="fq8ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  if (typeof ProgressManager !== 'undefined') {
    try {
      ...
      if (sum.caps) Object.keys(sum.caps).forEach(function(k) { if (k.indexOf('fq8cap') === 0) pmXp += (sum.caps[k].xp || 0); });
    } catch (e) {}
  }
  ...
  caps.forEach(function(c) {
    var m = _fq8CapMeta[c.cap - 1];
    var color = _fq8CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _fq8CapMeta[pior.cap - 1];
      var colorp = _fq8CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="fq8TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="fq8ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function fq8TreinarCap(cap) {
  _fq8Prat.cap = cap;
  _fq8Prat.st = 0;
  _fq8Prat.sel = { caps: [cap], stsByCap: {} };
  fq8SwitchTab('exercicios', null);
}

function fq8ProgReset() {
  ...
  for (var cap = 1; cap <= 8; cap++) { try { localStorage.removeItem('edupt_fq8_cap' + cap); } catch (e) {} }
  // Remove só os capítulos do 8.º ano (fq8cap*) do ProgressManager, preservando o 7.º.
  try {
    var raw = localStorage.getItem('edupt_progress_v2');
    if (raw) {
      var d = JSON.parse(raw);
      if (d.caps) { Object.keys(d.caps).forEach(function(k) { if (k.indexOf('fq8cap') === 0) delete d.caps[k]; }); }
...
function fq8ProgDownloadPDF() {
  ...
  var html = '<div style="font-family:Arial,sans-serif;max-width:680px;margin:0 auto;padding:24px">'
    + '<h1 style="font-size:20px;margin:0 0 4px">Relatório de Progresso · Físico-Química 8.º Ano</h1>'
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-fq8.pdf');
...
function fq8FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('fq8-fichas-atr','fq8AtribuirFicha');
  var el = document.getElementById('fq8-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com gerador
  var temAlgum = false;
  for (var k in _fq8gf.caps) { if (_fq8gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _fq8CapMeta.forEach(function(m) {
    var hasGen = !!_fq8Gerador(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _fq8gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_fq8gf.caps[m.n];
    var color = _fq8CapColors[m.n] || '#516860';
...
function fq8gfToggleType(btn) {
  ...
  _fq8gf.tipos[t] = !_fq8gf.tipos[t];
  btn.classList.toggle('active', _fq8gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _fq8gf.tipos[t] ? '✓' : '';
}

function fq8gfSetDif(btn) {
  _fq8gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('fq8-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function fq8gfSetQty(btn) {
  _fq8gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('fq8-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _fq8gfExBloco(exs, startNum) {
  var h = '';
...
function _fq8gfGenExs(cap, n) {
  ...
  var banco = (typeof _fq8Banco !== 'undefined' && _fq8Banco[cap]) ? _fq8Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _fq8gf.dif);
  }
  return geradas;
}

function fq8gfGerar(formato) {
  // capítulos selecionados
  var capsSel = [];
  _fq8CapMeta.forEach(function(m) { if (_fq8gf.caps[m.n] && _fq8Gerador(m.n)) capsSel.push(m.n); });
  var status = document.getElementById('fq8-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _fq8gf.tipos.resumo || _fq8gf.tipos.exercicios || _fq8gf.tipos.teste || _fq8gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_fq8gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _fq8PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _fq8CapMeta[cap - 1];
    var color = _fq8CapColors[cap] || '#36527a';
    ...
    if (_fq8gf.tipos.resumo) {
      var cards = _fq8Cards[cap] || [];
      if (cards.length) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_fq8gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) { return _fq8CapMeta[c - 1].label; }).join(', ');
  var html = '<div style="font-family:Arial,sans-serif;max-width:740px;margin:0 auto;padding:24px;color:#222">'
    ...
    + '<div><h1 style="font-size:18px;margin:0">Ficha de Trabalho · Físico-Química 8.º Ano</h1>'
  ...
  var _temasNomes = capsSel.map(function(c){ return _fq8CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('8', 'matematica', _temasNomes, formato) : ('ficha-fq8.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
// ═══ INIT ═══
function _fq8Init() {
  if (typeof _addStatsTeoriaVisuais === 'function') _addStatsTeoriaVisuais(_fq8Cards[8], '#9a5e96'); // cap8 Dados
  if (typeof _addGeoTeoriaVisuais === 'function') _addGeoTeoriaVisuais(_fq8Cards[4], '#b06a6a', { pitagoras: true }); // cap4 Pitágoras
  // arranca na tab Teoria com o cap 1 selecionado
  fq8BuildResumoNav();
}
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _fq8Init);
else _fq8Init();
...
/* ════════════════════════════════════════════════════════════════
   ...
   Reutiliza helpers _f7q1 (rnd_f7q1, etc.) já definidos no fq8.js.
   ════════════════════════════════════════════════════════════════ */
...
/* atribuir: deep-link fq8 */
function _fq8DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_fq8gf){ _fq8gf.caps={}; cs.forEach(function(n){ _fq8gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _fq8gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ fq8SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_fq8Prat) _fq8Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ fq8SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof gTabSwitch==='function')setTimeout(function(){try{gTabSwitch('fq8-jogos-app',jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_fq8TarefaAtiva=p.get('tarefa');_fq8TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _fq8Prat.cap=cap; _fq8Prat.st=st; _fq8Prat.nivel=nivel; _fq8Prat.sel={caps:[cap],stsByCap:(st?(function(){var o={};o[cap]=[st];return o;})():{})}; setTimeout(function(){ fq8SwitchTab('exercicios',null); if(typeof fq8GerarExercicios==='function') fq8GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_fq8DeepLinkAuto,300);});else setTimeout(_fq8DeepLinkAuto,300);

function fq8AtribuirFicha(){
  var caps=[]; _fq8CapMeta.forEach(function(m){ if(_fq8gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('fq8-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_fq8CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'fq8', cursoNome:'Físico-Química 8.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_fq8gf.dif };
}

function fq8EntregarTarefa(){
  if(!_fq8TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_fq8TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_fq8TarefaResp[qid]; if(ok)certas++; var ex=_fq8Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_fq8TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _fq8TarefaAtiva=null; _fq8TarefaResp={}; fq8GerarExercicios();
