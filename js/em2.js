/* ════════════════════════════════════════════════════════════════
   ...
   Sem módulos realojados: a prática é banco-driven (_em2Banco).
...
// Subtemas por domínio (2.º ano · Estudo do Meio)
var _em2Subtemas = {
...
var _em2Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function em2SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('em2p-' + tab);
  ...
  if (titles[tab]) document.title = 'Estudo do Meio 2.º · ' + titles[tab] + ' · 3ponto14';

  if (tab === 'resumo') em2BuildResumoNav();
  else if (tab === 'exercicios') em2BuildPraticarNav();
  else if (tab === 'quiz') em2QuizBuildNav();
  else if (tab === 'flashcards') em2FcBuildNav();
  else if (tab === 'teste') em2TesteBuildNav();
  else if (tab === 'jogos') em2JogosInit();
  else if (tab === 'jogar') { if (typeof LudicoEm2 !== 'undefined') LudicoEm2.init('em2-ludico-menu', 'em2-ludico-jogo'); }
  else if (tab === 'fichas') em2FichasBuildNav();
  else if (tab === 'progresso') em2RenderProgresso();
}

// Abre um sub-modo de prática a partir dos cartões do menu Praticar.
function em2OpenPraticar(modo) { em2SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function em2BuildResumoNav() {
  var capRow = document.getElementById('em2-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _em2Sel['resumo'] || 1;
  var h = '';
  _em2CapMeta.forEach(function(m) {
    var color = _em2CapColors[m.n] || '#516860';
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="em2ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  em2ResumoShowSts(activeCap);
  em2RenderResumoInline();
}

function em2ResumoSelectCap(cap, btn) {
  _em2Sel['resumo'] = cap;
  _em2Sel['resumo-st'] = 0;
  var capRow = document.getElementById('em2-resumo-cap-row');
  if (capRow) {
  ...
  if (btn) {
    var color = _em2CapColors[cap] || '#516860';
    btn.classList.add('active');
    btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff';
  }
  em2ResumoShowSts(cap);
  em2RenderResumoInline();
}

function em2ResumoShowSts(cap) {
  var stRow = document.getElementById('em2-resumo-st-row');
  if (!stRow) return;
  var sts = _em2Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  // conta cards por subtema para esconder chips de subtemas sem teoria
  var _cc = (typeof _em2Cards !== 'undefined' && _em2Cards[cap]) ? _em2Cards[cap] : [];
  var _nSt = {}; _cc.forEach(function(c){ if (c.st) _nSt[c.st] = (_nSt[c.st]||0)+1; });
  var _stSel = _em2Sel['resumo-st'] || 0;
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_stSel === 0 ? ' active' : '') + '" data-st="0" onclick="em2ResumoSelectSt(this,' + cap + ',0)">Todos</button>';
  sts.forEach(function(st, i) {
    if (!_nSt[i + 1]) return; // subtema sem flashcards → não mostra o chip
    var isActive = _stSel === (i + 1);
    h += '<button class="resumo-st-btn' + (isActive ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="em2ResumoSelectSt(this,' + cap + ',' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function em2ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('em2-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _em2Sel['resumo'] = cap;
  _em2Sel['resumo-st'] = stIdx;
  em2RenderResumoInline();
}

function em2RenderResumoInline() {
  var cap = _em2Sel['resumo'] || 1;
  var dest = document.getElementById('em2-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _em2CapColors[cap] || '#516860';
    return c;
  })();
  ...
  var allCards = _em2Cards[cap] || [];

  // Filtrar por subtema, se selecionado
  var stIdx = _em2Sel['resumo-st'] || 0;
  ...
  _em2PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
// Português não tem geradores procedurais — a prática usa o _em2Banco.
// Só os caps com gerador são "praticáveis".
// Português não tem geradores procedurais (as perguntas são de análise/banco).
// _em2Gerador devolve sempre null → o hub usa apenas o _em2Banco.
function _em2Gerador(cap) {
  return null;
}
// Um domínio está "disponível" para prática se tiver questões no banco.
function _em2TemConteudo(cap) {
  return !!(_em2Banco[cap] && _em2Banco[cap].length);
}
// Nº de subtemas por domínio (espelha _em2Subtemas) — usado na seleção de prática.
var _em2TemasCount = { 1: 4, 2: 5, 3: 3, 4: 3, 5: 3 };

// Estado da prática
var _em2Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _em2TarefaAtiva=null, _em2TarefaResp={};

function _em2PratStorageKey(cap) { return 'edupt_em2_cap' + cap; }

function em2BuildPraticarNav() {
  var capRow = document.getElementById('em2-praticar-cap-row');
  if (!capRow) return;
  var activeCap = _em2Prat.cap || 1;
  var h = '';
  _em2CapMeta.forEach(function(m) {
    var hasGen = _em2TemConteudo(m.n);
    var color = _em2CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    var disabledStyle = hasGen ? '' : 'opacity:.45;cursor:not-allowed';
    var onclick = hasGen ? 'em2PraticarSelectCap(' + m.n + ',this)' : '';
  ...
  capRow.innerHTML = h;
  em2PraticarShowSts(activeCap);
  em2GerarExercicios(); // renderiza logo o domínio ativo (módulo ou banco)
}

function em2PraticarShowSts(cap) {
  var stRow = document.getElementById('em2-praticar-st-row');
  if (!stRow) return;
  var sts = _em2Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_em2Prat.st === 0 ? ' active' : '') + '" data-st="0" onclick="em2PraticarSelectSt(this,0)">Todos</button>';
  sts.forEach(function(st, i) {
    h += '<button class="resumo-st-btn' + (_em2Prat.st === (i + 1) ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="em2PraticarSelectSt(this,' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function em2PraticarSelectCap(cap, btn) {
  if (!_em2TemConteudo(cap)) return;
  _em2Prat.cap = cap;
  _em2Prat.st = 0;
  var capRow = document.getElementById('em2-praticar-cap-row');
  ...
  if (btn) { var color = _em2CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff'; }
  em2PraticarShowSts(cap);
  em2GerarExercicios();
}

function em2PraticarSelectSt(btn, stIdx) {
  var stRow = document.getElementById('em2-praticar-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _em2Prat.st = stIdx;
...
function em2PraticarSetNivel(nivel, btn) {
  _em2Prat.nivel = nivel;
  var grp = document.getElementById('em2-praticar-nivel');
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _em2PratModulo(cap, dest) {
  // O 7.º ano não tem módulos realojados: a prática é toda banco-driven.
  return false;
}

function em2GerarExercicios() {
  var dest = document.getElementById('em2-praticar-content');
  if (!dest) return;
  var cap = _em2Prat.cap, gen = _em2Gerador(cap);
  ...
  var lvlBar = document.getElementById('em2-praticar-levelbar');
  var stRow = document.getElementById('em2-praticar-st-row');
  if (_em2PratModulo(cap, dest)) {
    if (lvlBar) lvlBar.style.display = 'none';
    if (stRow) stRow.style.display = 'none';
    if (_em2TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_em2TarefaAtiva, 'Trabalho concluído');
    return;
  }
  if (lvlBar) lvlBar.style.display = '';

  // Sem gerador procedural nem banco → nada a mostrar
  if (!gen && !(_em2Banco[cap] && _em2Banco[cap].length)) { dest.innerHTML = ''; return; }

  // Que temas usar?
  var temas;
  if (_em2Prat.st > 0 && _em2SubtemaTemas[cap] && _em2SubtemaTemas[cap][_em2Prat.st]) {
    temas = _em2SubtemaTemas[cap][_em2Prat.st];
  } else {
    temas = [];
    for (var t = 1; t <= (_em2TemasCount[cap] || 1); t++) temas.push(String(t));
  }
  ...
  if (gen) {
    for (var i = 0; i < QTD; i++) {
      ...
      var ex = gen(tema, tipos[i % tipos.length], _em2Prat.nivel);
      if (ex) geradas.push(ex);
    }
  }
  var banco = (typeof _em2Banco !== 'undefined' && _em2Banco[cap]) ? _em2Banco[cap].filter(function (q) { return temas.indexOf(q.t) !== -1; }) : [];
  var exs;
  if (!gen) {
    ...
    exs = (typeof _mixBancoGeradas === 'function') ? _mixBancoGeradas(banco, geradas, QTD, 3, _em2Prat.nivel)
      : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  }
  _em2Prat.exs = exs;
  _em2Prat.answered = {};
  _em2Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="em2-prat-scorebar">'
    + '<div><div class="score-num" id="em2-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="em2-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="em2-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="em2GerarExercicios()">↺ Novas questões</button>'
    + (_em2TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="em2EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'em2CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="em2-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _em2CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _sn = (_em2Subtemas[cap] && _em2Prat.st > 0) ? (_em2Subtemas[cap][_em2Prat.st - 1] || '') : '';
    Atribuir.montar('em2-atribuir', { curso: 'em2', cursoNome: 'Estudo do Meio 2.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_em2Prat.st || ''), subNome: _sn, tipo: 'quiz', nivel: _em2Prat.nivel });
  }
}

function em2CheckEx(qid, tipo, val, btn) {
  if (_em2Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _em2Prat.answered[qid] = true;
  if (r.correct) _em2Prat.score.correct++;
  _em2Prat.score.total++; if(_em2TarefaAtiva){_em2TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('em2-prat-score'); if (se) se.textContent = _em2Prat.score.correct;
  var te = document.getElementById('em2-prat-total'); if (te) te.textContent = '/ ' + _em2Prat.score.total;
  var pe = document.getElementById('em2-prat-prog'); if (pe && _em2Prat.exs.length) pe.style.width = (_em2Prat.score.total / _em2Prat.exs.length * 100) + '%';
  em2SaveProgress(_em2Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function em2SaveProgress(cap, correct) {
  try {
    var key = _em2PratStorageKey(cap);
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _em2BuildCapRow(rowId, activeCap, onclickName) {
...
function _em2SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
...
function _em2FichaSlice(banco, n, dif) {
...
function _em2BuildMcQuestion(cap) {
  var qs = _em2FichaBanco(cap).filter(function (q) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _em2Quiz = { cap: 1, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function em2QuizBuildNav() {
  if (!_em2TemConteudo(_em2Quiz.cap)) _em2Quiz.cap = 1;
  _em2BuildCapRow('em2-quiz-cap-row', _em2Quiz.cap, 'em2QuizSelectCap');
  em2QuizStart();
}

function em2QuizSelectCap(cap, btn) {
  if (!_em2TemConteudo(cap)) return;
  _em2SetActiveCapBtn('em2-quiz-cap-row', btn, cap);
  _em2Quiz.cap = cap;
  em2QuizStart();
}

function em2QuizStart() {
  _em2Quiz.lives = 3; _em2Quiz.streak = 0; _em2Quiz.maxStreak = 0;
  _em2Quiz.score = 0; _em2Quiz.total = 0; _em2Quiz.answered = false;
  em2QuizNext();
}

function em2QuizNext() {
  var app = document.getElementById('em2-quiz-app');
  if (!app) return;
  if (_em2Quiz.lives <= 0) { em2QuizGameOver(app); return; }
  var ex = _em2BuildMcQuestion(_em2Quiz.cap);
  ...
  _em2Quiz.current = ex; _em2Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _em2Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="em2qopt-' + idx + '" onclick="em2QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_em2Quiz.streak > 1 ? '🔥 ' + _em2Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _em2Quiz.score + ' / ' + _em2Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="em2-quiz-fb" style="min-height:2.5rem"></div>';
}

function em2QuizAnswer(idx) {
  if (_em2Quiz.answered) return;
  _em2Quiz.answered = true;
  var ex = _em2Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('em2qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('em2-quiz-fb');
  if (correct) {
    _em2Quiz.score++; _em2Quiz.streak++;
    if (_em2Quiz.streak > _em2Quiz.maxStreak) _em2Quiz.maxStreak = _em2Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _em2Quiz.lives--; _em2Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  em2SaveProgress(_em2Quiz.cap, correct);
  setTimeout(function(){ em2QuizNext(); }, 1400);
}

function em2QuizGameOver(app) {
  var pct = _em2Quiz.total > 0 ? Math.round(_em2Quiz.score / _em2Quiz.total * 100) : 0;
  _em2PM(_em2Quiz.cap, 'quiz', { pontuacao: _em2Quiz.score, total: _em2Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _em2Quiz.score + ' certas em ' + _em2Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_em2Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="em2QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _em2Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function em2FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_em2Cards[_em2Fc.cap] && _em2Cards[_em2Fc.cap].length)) {
    for (var c = 1; c <= _em2CapMeta.length; c++) { if (_em2Cards[c] && _em2Cards[c].length) { _em2Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('em2-fc-cap-row');
  if (row) {
    var h = '';
    _em2CapMeta.forEach(function(m) {
      var has = !!(_em2Cards[m.n] && _em2Cards[m.n].length);
      var color = _em2CapColors[m.n] || '#516860';
      var isActive = _em2Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('em2FcSelectCap(' + m.n + ',this)') : '';
  ...
  em2FcSelectCap(_em2Fc.cap, null);
}

function em2FcSelectCap(cap, btn) {
  if (!(_em2Cards[cap] && _em2Cards[cap].length)) return;
  if (btn) _em2SetActiveCapBtn('em2-fc-cap-row', btn, cap);
  _em2Fc.cap = cap;
  _em2Fc.cards = _em2Cards[cap].slice();
  _em2Fc.idx = 0; _em2Fc.flipped = false;
  _em2PM(cap, 'flashcard');
  em2FcRender();
}

function em2FcRender() {
  var app = document.getElementById('em2-fc-app');
  if (!app) return;
  var cards = _em2Fc.cards;
  ...
  var card = cards[_em2Fc.idx];
  var color = _em2CapColors[_em2Fc.cap] || '#516860';
  var pct = Math.round((_em2Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_em2Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="em2FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_em2Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_em2Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="em2FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="em2FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="em2FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function em2FcFlip() { _em2Fc.flipped = !_em2Fc.flipped; em2FcRender(); }
function em2FcNext() { _em2Fc.idx = (_em2Fc.idx + 1) % (_em2Fc.cards.length || 1); _em2Fc.flipped = false; em2FcRender(); }
function em2FcPrev() { _em2Fc.idx = (_em2Fc.idx - 1 + (_em2Fc.cards.length || 1)) % (_em2Fc.cards.length || 1); _em2Fc.flipped = false; em2FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _em2Teste = { cap: 1, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function em2TesteBuildNav() {
  if (!_em2TemConteudo(_em2Teste.cap)) _em2Teste.cap = 1;
  _em2BuildCapRow('em2-teste-cap-row', _em2Teste.cap, 'em2TesteSelectCap');
  em2TesteRenderConfig();
}

function em2TesteSelectCap(cap, btn) {
  if (!_em2TemConteudo(cap)) return;
  _em2SetActiveCapBtn('em2-teste-cap-row', btn, cap);
  _em2Teste.cap = cap;
  em2TesteRenderConfig();
}

function em2TesteRenderConfig() {
  var app = document.getElementById('em2-teste-app');
  if (!app) return;
  if (_em2Teste.timer) { clearInterval(_em2Teste.timer); _em2Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="em2-teste-nivel">' +
            '<button class="gen-level-btn' + (_em2Teste.nivel==='facil'?' active':'') + '" onclick="em2TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_em2Teste.nivel==='medio'?' active':'') + '" onclick="em2TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_em2Teste.nivel==='dificil'?' active':'') + '" onclick="em2TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="em2-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="em2-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="em2TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function em2TesteSetNivel(nivel, btn) {
  _em2Teste.nivel = nivel;
  var grp = document.getElementById('em2-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function em2TesteStart() {
  var gen = _em2Gerador(_em2Teste.cap);
  var qtdEl = document.getElementById('em2-teste-qtd'), tempoEl = document.getElementById('em2-teste-tempo');
  _em2Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _em2Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var exs = [];
  if (gen) {
    var nTemas = _em2TemasCount[_em2Teste.cap] || 1;
    var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
    for (var i = 0; i < _em2Teste.qtd; i++) {
      var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _em2Teste.nivel);
      if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
    }
  } else {
    // PT: teste sai do banco alargado do domínio
    exs = _em2FichaSlice(_em2FichaBanco(_em2Teste.cap), _em2Teste.qtd, _em2Teste.nivel);
  }
  if (!exs.length) return;
  _em2Teste.exs = exs; _em2Teste.answered = {}; _em2Teste.score = { correct: 0, total: 0 };
  _em2Teste.restante = _em2Teste.tempo;

  var app = document.getElementById('em2-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'em2TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="em2-teste-timer">' + _em2FmtTime(_em2Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="em2-teste-answered">0 / ' + _em2Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="em2TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="em2-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _em2Teste.timer = setInterval(function() {
    _em2Teste.restante--;
    var t = document.getElementById('em2-teste-timer');
    if (t) t.textContent = _em2FmtTime(_em2Teste.restante);
    if (_em2Teste.restante <= 0) em2TesteFinish();
  }, 1000);
}

function _em2FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function em2TesteCheck(qid, tipo, val, btn) {
  if (_em2Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _em2Teste.answered[qid] = true;
  if (r.correct) _em2Teste.score.correct++;
  _em2Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('em2-teste-answered');
  if (a) a.textContent = _em2Teste.score.total + ' / ' + _em2Teste.qtd;
  em2SaveProgress(_em2Teste.cap, r.correct);
  if (_em2Teste.score.total >= _em2Teste.qtd) setTimeout(em2TesteFinish, 600);
}

function em2TesteFinish() {
  if (_em2Teste.timer) { clearInterval(_em2Teste.timer); _em2Teste.timer = null; }
  var app = document.getElementById('em2-teste-app');
  if (!app) return;
  var total = _em2Teste.qtd;
  var correct = _em2Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _em2PM(_em2Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="em2TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function em2JogosInit() {
  (function(){ var pj=document.getElementById('em2p-jogos'); if(pj && !document.getElementById('em2-jogos-atr')){ var d=document.createElement('div'); d.id='em2-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('em2-jogos-atr',{curso:'em2',cursoNome:'Português 7.º',tipo:'jogo',nivel:'',caps:_em2CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  if (typeof ptJogosRender === 'function') {
    if (typeof ptJogosConfig === 'function') {
      ptJogosConfig({
        ...
        fonte: function () {
          var out = [];
          [1, 2, 3, 4, 5].forEach(function (c) { out = out.concat(_em2FichaBanco(c)); });
          return out;
        },
        forca: PT7_FORCA
      });
    ...
    var app = document.getElementById('em2-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_em2_capN (gravado por em2SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _em2ProgGetCaps() {
  var out = [];
  var nCaps = _em2CapMeta.length;
  for (var cap = 1; cap <= nCaps; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_em2_cap' + cap) || '{}');
...
function em2RenderProgresso() {
  var el = document.getElementById('em2-progresso-content');
  if (!el) return;
  var caps = _em2ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="em2ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m9c1-base);border:1.5px solid var(--m9c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="em2ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  caps.forEach(function(c) {
    var m = _em2CapMeta[c.cap - 1];
    var color = _em2CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _em2CapMeta[pior.cap - 1];
      var colorp = _em2CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="em2TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="em2ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function em2TreinarCap(cap) {
  _em2Prat.cap = cap;
  _em2Prat.st = 0;
  em2SwitchTab('exercicios', null);
}

function em2ProgReset() {
  ...
  for (var cap = 1; cap <= _em2CapMeta.length; cap++) { try { localStorage.removeItem('edupt_em2_cap' + cap); } catch (e) {} }
...
function em2ProgDownloadPDF() {
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-em2.pdf');
...
function _em2gfStsSel(cap) {
  var sel = _em2gf.sts[cap];
  if (!sel) return null;
  var out = [];
...
function _em2gfTemasSel(cap) {
  var sts = _em2gfStsSel(cap);
  if (!sts) return null;
  var mapa = _em2SubtemaTemas[cap] || {};
  var temas = [];
...
function em2FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('em2-fichas-atr','em2AtribuirFicha');
  var el = document.getElementById('em2-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com conteúdo
  var temAlgum = false;
  for (var k in _em2gf.caps) { if (_em2gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _em2CapMeta.forEach(function(m) {
    var hasGen = _em2TemConteudo(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _em2gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_em2gf.caps[m.n];
    var color = _em2CapColors[m.n] || '#516860';
    var style = sel ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    h += '<button class="gf-cap-btn' + (sel ? ' active' : '') + '" data-cap="' + m.n + '" onclick="em2gfToggleCap(' + m.n + ',this)" style="' + style + '">' + m.icon + ' ' + m.label + '</button>';
    // tópicos do capítulo selecionado
    if (sel) {
      var sts = _em2Subtemas[m.n] || [];
      if (sts.length) {
        var escolhidos = _em2gf.sts[m.n] || {};
        var algumSt = _em2gfStsSel(m.n) !== null;
        ...
        h += '<button onclick="em2gfToggleSt(' + m.n + ',0,this)" style="' + _em2gfStStyle(!algumSt, color) + '">Todos</button>';
        sts.forEach(function (st, i) {
          var on = !!escolhidos[i + 1];
          h += '<button onclick="em2gfToggleSt(' + m.n + ',' + (i + 1) + ',this)" style="' + _em2gfStStyle(on, color) + '">' + st + '</button>';
        });
        h += '</div>';
      }
    }
...
function _em2gfStStyle(on, color) {
...
function em2gfToggleCap(cap, btn) {
  _em2gf.caps[cap] = !_em2gf.caps[cap];
  if (!_em2gf.caps[cap]) delete _em2gf.sts[cap]; // desmarcar cap limpa os tópicos
  em2FichasBuildNav(); // re-render para mostrar/esconder a linha de tópicos
}

// idx 0 = «Todos» (limpa a seleção); idx 1..n alterna o tópico.
function em2gfToggleSt(cap, idx, btn) {
  if (idx === 0) {
    delete _em2gf.sts[cap];
  } else {
    if (!_em2gf.sts[cap]) _em2gf.sts[cap] = {};
    _em2gf.sts[cap][idx] = !_em2gf.sts[cap][idx];
    if (_em2gfStsSel(cap) === null) delete _em2gf.sts[cap]; // tudo desmarcado = todos
  }
  em2FichasBuildNav();
}

function em2gfToggleType(btn) {
  var t = btn.getAttribute('data-type');
  _em2gf.tipos[t] = !_em2gf.tipos[t];
  btn.classList.toggle('active', _em2gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _em2gf.tipos[t] ? '✓' : '';
}

function em2gfSetDif(btn) {
  _em2gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('em2-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function em2gfSetQty(btn) {
  _em2gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('em2-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _em2gfExBloco(exs, startNum) {
  var h = '';
...
function _em2gfGenExs(cap, n) {
  var gen = _em2Gerador(cap);
  if (!gen) {
    ...
    var pool = _em2FichaBanco(cap);
    var temasSel = _em2gfTemasSel(cap);
    if (temasSel) {
      var filtrado = pool.filter(function (q) { return temasSel.indexOf(q.t) !== -1; });
      if (filtrado.length) pool = filtrado;
    }
    return _em2FichaSlice(pool, n, _em2gf.dif);
  }
  var nTemas = _em2TemasCount[cap] || 1;
  ...
  for (var i = 0, tent = 0; geradas.length < n && tent < n * 6; tent++) {
    var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _em2gf.dif);
    i++;
    if (!ex) continue;
  ...
  var banco = (typeof _em2Banco !== 'undefined' && _em2Banco[cap]) ? _em2Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _em2gf.dif);
  }
  return geradas;
}

function em2gfGerar(formato) {
  // capítulos selecionados (conta o banco alargado — em PT não há geradores)
  var capsSel = [];
  _em2CapMeta.forEach(function(m) { if (_em2gf.caps[m.n] && (_em2Gerador(m.n) || _em2FichaBanco(m.n).length)) capsSel.push(m.n); });
  var status = document.getElementById('em2-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _em2gf.tipos.resumo || _em2gf.tipos.exercicios || _em2gf.tipos.teste || _em2gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_em2gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _em2PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _em2CapMeta[cap - 1];
    var color = _em2CapColors[cap] || '#36527a';
    ...
    if (_em2gf.tipos.resumo) {
      var cards = _em2Cards[cap] || [];
      var stsR = _em2gfStsSel(cap);
      if (stsR && cards.length) {
        var palavras = [];
        stsR.forEach(function (i) {
          String((_em2Subtemas[cap] || [])[i - 1] || '').toLowerCase().split(/\s+/).forEach(function (w) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_em2gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) {
    var nome = _em2CapMeta[c - 1].label;
    var sts = _em2gfStsSel(c);
    if (sts) {
      var nomes = sts.map(function (i) { return (_em2Subtemas[c] || [])[i - 1] || ''; }).filter(function (x) { return x; });
  ...
  var _temasNomes = capsSel.map(function(c){ return _em2CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('9', 'portugues', _temasNomes, formato) : ('ficha-em2.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _em2Init);
else _em2Init();
...
_em2Banco[2] = _em2Banco[2].concat([
...
_em2Banco[1] = _em2Banco[1].concat([
...
_em2Banco[3] = _em2Banco[3].concat([
  // F1-F2 · Publicidade
...
/* atribuir: deep-link em2 */
function _em2DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_em2gf){ _em2gf.caps={}; cs.forEach(function(n){ _em2gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _em2gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ em2SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_em2Prat) _em2Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ em2SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_em2TarefaAtiva=p.get('tarefa');_em2TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _em2Prat.cap=cap; _em2Prat.st=st; _em2Prat.nivel=nivel; setTimeout(function(){ em2SwitchTab('exercicios',null); if(typeof em2GerarExercicios==='function') em2GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_em2DeepLinkAuto,300);});else setTimeout(_em2DeepLinkAuto,300);

function em2AtribuirFicha(){
  var caps=[]; _em2CapMeta.forEach(function(m){ if(_em2gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('em2-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_em2CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'em2', cursoNome:'Português 7.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_em2gf.dif };
}

function em2EntregarTarefa(){
  if(!_em2TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_em2TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_em2TarefaResp[qid]; if(ok)certas++; var ex=_em2Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_em2TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _em2TarefaAtiva=null; _em2TarefaResp={}; em2GerarExercicios();
