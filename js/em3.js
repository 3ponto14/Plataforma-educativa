/* ════════════════════════════════════════════════════════════════
   ...
   Sem módulos realojados: a prática é banco-driven (_em3Banco).
...
// Subtemas por domínio (3.º ano · Estudo do Meio)
var _em3Subtemas = {
...
var _em3Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function em3SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('em3p-' + tab);
  ...
  if (titles[tab]) document.title = 'Estudo do Meio 3.º · ' + titles[tab] + ' · 3ponto14';

  if (tab === 'resumo') em3BuildResumoNav();
  else if (tab === 'exercicios') em3BuildPraticarNav();
  else if (tab === 'quiz') em3QuizBuildNav();
  else if (tab === 'flashcards') em3FcBuildNav();
  else if (tab === 'teste') em3TesteBuildNav();
  else if (tab === 'jogos') em3JogosInit();
  else if (tab === 'jogar') { if (typeof LudicoEm3 !== 'undefined') LudicoEm3.init('em3-ludico-menu', 'em3-ludico-jogo'); }
  else if (tab === 'fichas') em3FichasBuildNav();
  else if (tab === 'progresso') em3RenderProgresso();
}

// Abre um sub-modo de prática a partir dos cartões do menu Praticar.
function em3OpenPraticar(modo) { em3SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function em3BuildResumoNav() {
  var capRow = document.getElementById('em3-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _em3Sel['resumo'] || 1;
  var h = '';
  _em3CapMeta.forEach(function(m) {
    var color = _em3CapColors[m.n] || '#516860';
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="em3ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  em3ResumoShowSts(activeCap);
  em3RenderResumoInline();
}

function em3ResumoSelectCap(cap, btn) {
  _em3Sel['resumo'] = cap;
  _em3Sel['resumo-st'] = 0;
  var capRow = document.getElementById('em3-resumo-cap-row');
  if (capRow) {
  ...
  if (btn) {
    var color = _em3CapColors[cap] || '#516860';
    btn.classList.add('active');
    btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff';
  }
  em3ResumoShowSts(cap);
  em3RenderResumoInline();
}

function em3ResumoShowSts(cap) {
  var stRow = document.getElementById('em3-resumo-st-row');
  if (!stRow) return;
  var sts = _em3Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  // conta cards por subtema para esconder chips de subtemas sem teoria
  var _cc = (typeof _em3Cards !== 'undefined' && _em3Cards[cap]) ? _em3Cards[cap] : [];
  var _nSt = {}; _cc.forEach(function(c){ if (c.st) _nSt[c.st] = (_nSt[c.st]||0)+1; });
  var _stSel = _em3Sel['resumo-st'] || 0;
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_stSel === 0 ? ' active' : '') + '" data-st="0" onclick="em3ResumoSelectSt(this,' + cap + ',0)">Todos</button>';
  sts.forEach(function(st, i) {
    if (!_nSt[i + 1]) return; // subtema sem flashcards → não mostra o chip
    var isActive = _stSel === (i + 1);
    h += '<button class="resumo-st-btn' + (isActive ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="em3ResumoSelectSt(this,' + cap + ',' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function em3ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('em3-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _em3Sel['resumo'] = cap;
  _em3Sel['resumo-st'] = stIdx;
  em3RenderResumoInline();
}

function em3RenderResumoInline() {
  var cap = _em3Sel['resumo'] || 1;
  var dest = document.getElementById('em3-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _em3CapColors[cap] || '#516860';
    return c;
  })();
  ...
  var allCards = _em3Cards[cap] || [];

  // Filtrar por subtema, se selecionado
  var stIdx = _em3Sel['resumo-st'] || 0;
  ...
  _em3PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
// Português não tem geradores procedurais — a prática usa o _em3Banco.
// Só os caps com gerador são "praticáveis".
// Português não tem geradores procedurais (as perguntas são de análise/banco).
// _em3Gerador devolve sempre null → o hub usa apenas o _em3Banco.
function _em3Gerador(cap) {
  return null;
}
// Um domínio está "disponível" para prática se tiver questões no banco.
function _em3TemConteudo(cap) {
  return !!(_em3Banco[cap] && _em3Banco[cap].length);
}
// Nº de subtemas por domínio (espelha _em3Subtemas) — usado na seleção de prática.
var _em3TemasCount = { 1: 4, 2: 5, 3: 3, 4: 3, 5: 3 };

// Estado da prática
var _em3Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _em3TarefaAtiva=null, _em3TarefaResp={};

function _em3PratStorageKey(cap) { return 'edupt_em3_cap' + cap; }

function em3BuildPraticarNav() {
  var capRow = document.getElementById('em3-praticar-cap-row');
  if (!capRow) return;
  var activeCap = _em3Prat.cap || 1;
  var h = '';
  _em3CapMeta.forEach(function(m) {
    var hasGen = _em3TemConteudo(m.n);
    var color = _em3CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    var disabledStyle = hasGen ? '' : 'opacity:.45;cursor:not-allowed';
    var onclick = hasGen ? 'em3PraticarSelectCap(' + m.n + ',this)' : '';
  ...
  capRow.innerHTML = h;
  em3PraticarShowSts(activeCap);
  em3GerarExercicios(); // renderiza logo o domínio ativo (módulo ou banco)
}

function em3PraticarShowSts(cap) {
  var stRow = document.getElementById('em3-praticar-st-row');
  if (!stRow) return;
  var sts = _em3Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_em3Prat.st === 0 ? ' active' : '') + '" data-st="0" onclick="em3PraticarSelectSt(this,0)">Todos</button>';
  sts.forEach(function(st, i) {
    h += '<button class="resumo-st-btn' + (_em3Prat.st === (i + 1) ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="em3PraticarSelectSt(this,' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function em3PraticarSelectCap(cap, btn) {
  if (!_em3TemConteudo(cap)) return;
  _em3Prat.cap = cap;
  _em3Prat.st = 0;
  var capRow = document.getElementById('em3-praticar-cap-row');
  ...
  if (btn) { var color = _em3CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff'; }
  em3PraticarShowSts(cap);
  em3GerarExercicios();
}

function em3PraticarSelectSt(btn, stIdx) {
  var stRow = document.getElementById('em3-praticar-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _em3Prat.st = stIdx;
...
function em3PraticarSetNivel(nivel, btn) {
  _em3Prat.nivel = nivel;
  var grp = document.getElementById('em3-praticar-nivel');
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _em3PratModulo(cap, dest) {
  // O 7.º ano não tem módulos realojados: a prática é toda banco-driven.
  return false;
}

function em3GerarExercicios() {
  var dest = document.getElementById('em3-praticar-content');
  if (!dest) return;
  var cap = _em3Prat.cap, gen = _em3Gerador(cap);
  ...
  var lvlBar = document.getElementById('em3-praticar-levelbar');
  var stRow = document.getElementById('em3-praticar-st-row');
  if (_em3PratModulo(cap, dest)) {
    if (lvlBar) lvlBar.style.display = 'none';
    if (stRow) stRow.style.display = 'none';
    if (_em3TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_em3TarefaAtiva, 'Trabalho concluído');
    return;
  }
  if (lvlBar) lvlBar.style.display = '';

  // Sem gerador procedural nem banco → nada a mostrar
  if (!gen && !(_em3Banco[cap] && _em3Banco[cap].length)) { dest.innerHTML = ''; return; }

  // Que temas usar?
  var temas;
  if (_em3Prat.st > 0 && _em3SubtemaTemas[cap] && _em3SubtemaTemas[cap][_em3Prat.st]) {
    temas = _em3SubtemaTemas[cap][_em3Prat.st];
  } else {
    temas = [];
    for (var t = 1; t <= (_em3TemasCount[cap] || 1); t++) temas.push(String(t));
  }
  ...
  if (gen) {
    for (var i = 0; i < QTD; i++) {
      ...
      var ex = gen(tema, tipos[i % tipos.length], _em3Prat.nivel);
      if (ex) geradas.push(ex);
    }
  }
  var banco = (typeof _em3Banco !== 'undefined' && _em3Banco[cap]) ? _em3Banco[cap].filter(function (q) { return temas.indexOf(q.t) !== -1; }) : [];
  var exs;
  if (!gen) {
    ...
    exs = (typeof _mixBancoGeradas === 'function') ? _mixBancoGeradas(banco, geradas, QTD, 3, _em3Prat.nivel)
      : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  }
  _em3Prat.exs = exs;
  _em3Prat.answered = {};
  _em3Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="em3-prat-scorebar">'
    + '<div><div class="score-num" id="em3-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="em3-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="em3-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="em3GerarExercicios()">↺ Novas questões</button>'
    + (_em3TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="em3EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'em3CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="em3-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _em3CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _sn = (_em3Subtemas[cap] && _em3Prat.st > 0) ? (_em3Subtemas[cap][_em3Prat.st - 1] || '') : '';
    Atribuir.montar('em3-atribuir', { curso: 'em3', cursoNome: 'Estudo do Meio 3.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_em3Prat.st || ''), subNome: _sn, tipo: 'quiz', nivel: _em3Prat.nivel });
  }
}

function em3CheckEx(qid, tipo, val, btn) {
  if (_em3Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _em3Prat.answered[qid] = true;
  if (r.correct) _em3Prat.score.correct++;
  _em3Prat.score.total++; if(_em3TarefaAtiva){_em3TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('em3-prat-score'); if (se) se.textContent = _em3Prat.score.correct;
  var te = document.getElementById('em3-prat-total'); if (te) te.textContent = '/ ' + _em3Prat.score.total;
  var pe = document.getElementById('em3-prat-prog'); if (pe && _em3Prat.exs.length) pe.style.width = (_em3Prat.score.total / _em3Prat.exs.length * 100) + '%';
  em3SaveProgress(_em3Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function em3SaveProgress(cap, correct) {
  try {
    var key = _em3PratStorageKey(cap);
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _em3BuildCapRow(rowId, activeCap, onclickName) {
...
function _em3SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
...
function _em3FichaSlice(banco, n, dif) {
...
function _em3BuildMcQuestion(cap) {
  var qs = _em3FichaBanco(cap).filter(function (q) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _em3Quiz = { cap: 1, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function em3QuizBuildNav() {
  if (!_em3TemConteudo(_em3Quiz.cap)) _em3Quiz.cap = 1;
  _em3BuildCapRow('em3-quiz-cap-row', _em3Quiz.cap, 'em3QuizSelectCap');
  em3QuizStart();
}

function em3QuizSelectCap(cap, btn) {
  if (!_em3TemConteudo(cap)) return;
  _em3SetActiveCapBtn('em3-quiz-cap-row', btn, cap);
  _em3Quiz.cap = cap;
  em3QuizStart();
}

function em3QuizStart() {
  _em3Quiz.lives = 3; _em3Quiz.streak = 0; _em3Quiz.maxStreak = 0;
  _em3Quiz.score = 0; _em3Quiz.total = 0; _em3Quiz.answered = false;
  em3QuizNext();
}

function em3QuizNext() {
  var app = document.getElementById('em3-quiz-app');
  if (!app) return;
  if (_em3Quiz.lives <= 0) { em3QuizGameOver(app); return; }
  var ex = _em3BuildMcQuestion(_em3Quiz.cap);
  ...
  _em3Quiz.current = ex; _em3Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _em3Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="em3qopt-' + idx + '" onclick="em3QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_em3Quiz.streak > 1 ? '🔥 ' + _em3Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _em3Quiz.score + ' / ' + _em3Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="em3-quiz-fb" style="min-height:2.5rem"></div>';
}

function em3QuizAnswer(idx) {
  if (_em3Quiz.answered) return;
  _em3Quiz.answered = true;
  var ex = _em3Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('em3qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('em3-quiz-fb');
  if (correct) {
    _em3Quiz.score++; _em3Quiz.streak++;
    if (_em3Quiz.streak > _em3Quiz.maxStreak) _em3Quiz.maxStreak = _em3Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _em3Quiz.lives--; _em3Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  em3SaveProgress(_em3Quiz.cap, correct);
  setTimeout(function(){ em3QuizNext(); }, 1400);
}

function em3QuizGameOver(app) {
  var pct = _em3Quiz.total > 0 ? Math.round(_em3Quiz.score / _em3Quiz.total * 100) : 0;
  _em3PM(_em3Quiz.cap, 'quiz', { pontuacao: _em3Quiz.score, total: _em3Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _em3Quiz.score + ' certas em ' + _em3Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_em3Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="em3QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _em3Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function em3FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_em3Cards[_em3Fc.cap] && _em3Cards[_em3Fc.cap].length)) {
    for (var c = 1; c <= _em3CapMeta.length; c++) { if (_em3Cards[c] && _em3Cards[c].length) { _em3Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('em3-fc-cap-row');
  if (row) {
    var h = '';
    _em3CapMeta.forEach(function(m) {
      var has = !!(_em3Cards[m.n] && _em3Cards[m.n].length);
      var color = _em3CapColors[m.n] || '#516860';
      var isActive = _em3Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('em3FcSelectCap(' + m.n + ',this)') : '';
  ...
  em3FcSelectCap(_em3Fc.cap, null);
}

function em3FcSelectCap(cap, btn) {
  if (!(_em3Cards[cap] && _em3Cards[cap].length)) return;
  if (btn) _em3SetActiveCapBtn('em3-fc-cap-row', btn, cap);
  _em3Fc.cap = cap;
  _em3Fc.cards = _em3Cards[cap].slice();
  _em3Fc.idx = 0; _em3Fc.flipped = false;
  _em3PM(cap, 'flashcard');
  em3FcRender();
}

function em3FcRender() {
  var app = document.getElementById('em3-fc-app');
  if (!app) return;
  var cards = _em3Fc.cards;
  ...
  var card = cards[_em3Fc.idx];
  var color = _em3CapColors[_em3Fc.cap] || '#516860';
  var pct = Math.round((_em3Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_em3Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="em3FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_em3Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_em3Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="em3FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="em3FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="em3FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function em3FcFlip() { _em3Fc.flipped = !_em3Fc.flipped; em3FcRender(); }
function em3FcNext() { _em3Fc.idx = (_em3Fc.idx + 1) % (_em3Fc.cards.length || 1); _em3Fc.flipped = false; em3FcRender(); }
function em3FcPrev() { _em3Fc.idx = (_em3Fc.idx - 1 + (_em3Fc.cards.length || 1)) % (_em3Fc.cards.length || 1); _em3Fc.flipped = false; em3FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _em3Teste = { cap: 1, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function em3TesteBuildNav() {
  if (!_em3TemConteudo(_em3Teste.cap)) _em3Teste.cap = 1;
  _em3BuildCapRow('em3-teste-cap-row', _em3Teste.cap, 'em3TesteSelectCap');
  em3TesteRenderConfig();
}

function em3TesteSelectCap(cap, btn) {
  if (!_em3TemConteudo(cap)) return;
  _em3SetActiveCapBtn('em3-teste-cap-row', btn, cap);
  _em3Teste.cap = cap;
  em3TesteRenderConfig();
}

function em3TesteRenderConfig() {
  var app = document.getElementById('em3-teste-app');
  if (!app) return;
  if (_em3Teste.timer) { clearInterval(_em3Teste.timer); _em3Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="em3-teste-nivel">' +
            '<button class="gen-level-btn' + (_em3Teste.nivel==='facil'?' active':'') + '" onclick="em3TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_em3Teste.nivel==='medio'?' active':'') + '" onclick="em3TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_em3Teste.nivel==='dificil'?' active':'') + '" onclick="em3TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="em3-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="em3-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="em3TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function em3TesteSetNivel(nivel, btn) {
  _em3Teste.nivel = nivel;
  var grp = document.getElementById('em3-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function em3TesteStart() {
  var gen = _em3Gerador(_em3Teste.cap);
  var qtdEl = document.getElementById('em3-teste-qtd'), tempoEl = document.getElementById('em3-teste-tempo');
  _em3Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _em3Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var exs = [];
  if (gen) {
    var nTemas = _em3TemasCount[_em3Teste.cap] || 1;
    var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
    for (var i = 0; i < _em3Teste.qtd; i++) {
      var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _em3Teste.nivel);
      if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
    }
  } else {
    // PT: teste sai do banco alargado do domínio
    exs = _em3FichaSlice(_em3FichaBanco(_em3Teste.cap), _em3Teste.qtd, _em3Teste.nivel);
  }
  if (!exs.length) return;
  _em3Teste.exs = exs; _em3Teste.answered = {}; _em3Teste.score = { correct: 0, total: 0 };
  _em3Teste.restante = _em3Teste.tempo;

  var app = document.getElementById('em3-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'em3TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="em3-teste-timer">' + _em3FmtTime(_em3Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="em3-teste-answered">0 / ' + _em3Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="em3TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="em3-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _em3Teste.timer = setInterval(function() {
    _em3Teste.restante--;
    var t = document.getElementById('em3-teste-timer');
    if (t) t.textContent = _em3FmtTime(_em3Teste.restante);
    if (_em3Teste.restante <= 0) em3TesteFinish();
  }, 1000);
}

function _em3FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function em3TesteCheck(qid, tipo, val, btn) {
  if (_em3Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _em3Teste.answered[qid] = true;
  if (r.correct) _em3Teste.score.correct++;
  _em3Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('em3-teste-answered');
  if (a) a.textContent = _em3Teste.score.total + ' / ' + _em3Teste.qtd;
  em3SaveProgress(_em3Teste.cap, r.correct);
  if (_em3Teste.score.total >= _em3Teste.qtd) setTimeout(em3TesteFinish, 600);
}

function em3TesteFinish() {
  if (_em3Teste.timer) { clearInterval(_em3Teste.timer); _em3Teste.timer = null; }
  var app = document.getElementById('em3-teste-app');
  if (!app) return;
  var total = _em3Teste.qtd;
  var correct = _em3Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _em3PM(_em3Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="em3TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function em3JogosInit() {
  (function(){ var pj=document.getElementById('em3p-jogos'); if(pj && !document.getElementById('em3-jogos-atr')){ var d=document.createElement('div'); d.id='em3-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('em3-jogos-atr',{curso:'em3',cursoNome:'Português 7.º',tipo:'jogo',nivel:'',caps:_em3CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  if (typeof ptJogosRender === 'function') {
    if (typeof ptJogosConfig === 'function') {
      ptJogosConfig({
        ...
        fonte: function () {
          var out = [];
          [1, 2, 3, 4, 5].forEach(function (c) { out = out.concat(_em3FichaBanco(c)); });
          return out;
        },
        forca: PT7_FORCA
      });
    ...
    var app = document.getElementById('em3-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_em3_capN (gravado por em3SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _em3ProgGetCaps() {
  var out = [];
  var nCaps = _em3CapMeta.length;
  for (var cap = 1; cap <= nCaps; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_em3_cap' + cap) || '{}');
...
function em3RenderProgresso() {
  var el = document.getElementById('em3-progresso-content');
  if (!el) return;
  var caps = _em3ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="em3ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m9c1-base);border:1.5px solid var(--m9c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="em3ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  caps.forEach(function(c) {
    var m = _em3CapMeta[c.cap - 1];
    var color = _em3CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _em3CapMeta[pior.cap - 1];
      var colorp = _em3CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="em3TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="em3ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function em3TreinarCap(cap) {
  _em3Prat.cap = cap;
  _em3Prat.st = 0;
  em3SwitchTab('exercicios', null);
}

function em3ProgReset() {
  ...
  for (var cap = 1; cap <= _em3CapMeta.length; cap++) { try { localStorage.removeItem('edupt_em3_cap' + cap); } catch (e) {} }
...
function em3ProgDownloadPDF() {
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-em3.pdf');
...
function _em3gfStsSel(cap) {
  var sel = _em3gf.sts[cap];
  if (!sel) return null;
  var out = [];
...
function _em3gfTemasSel(cap) {
  var sts = _em3gfStsSel(cap);
  if (!sts) return null;
  var mapa = _em3SubtemaTemas[cap] || {};
  var temas = [];
...
function em3FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('em3-fichas-atr','em3AtribuirFicha');
  var el = document.getElementById('em3-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com conteúdo
  var temAlgum = false;
  for (var k in _em3gf.caps) { if (_em3gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _em3CapMeta.forEach(function(m) {
    var hasGen = _em3TemConteudo(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _em3gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_em3gf.caps[m.n];
    var color = _em3CapColors[m.n] || '#516860';
    var style = sel ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    h += '<button class="gf-cap-btn' + (sel ? ' active' : '') + '" data-cap="' + m.n + '" onclick="em3gfToggleCap(' + m.n + ',this)" style="' + style + '">' + m.icon + ' ' + m.label + '</button>';
    // tópicos do capítulo selecionado
    if (sel) {
      var sts = _em3Subtemas[m.n] || [];
      if (sts.length) {
        var escolhidos = _em3gf.sts[m.n] || {};
        var algumSt = _em3gfStsSel(m.n) !== null;
        ...
        h += '<button onclick="em3gfToggleSt(' + m.n + ',0,this)" style="' + _em3gfStStyle(!algumSt, color) + '">Todos</button>';
        sts.forEach(function (st, i) {
          var on = !!escolhidos[i + 1];
          h += '<button onclick="em3gfToggleSt(' + m.n + ',' + (i + 1) + ',this)" style="' + _em3gfStStyle(on, color) + '">' + st + '</button>';
        });
        h += '</div>';
      }
    }
...
function _em3gfStStyle(on, color) {
...
function em3gfToggleCap(cap, btn) {
  _em3gf.caps[cap] = !_em3gf.caps[cap];
  if (!_em3gf.caps[cap]) delete _em3gf.sts[cap]; // desmarcar cap limpa os tópicos
  em3FichasBuildNav(); // re-render para mostrar/esconder a linha de tópicos
}

// idx 0 = «Todos» (limpa a seleção); idx 1..n alterna o tópico.
function em3gfToggleSt(cap, idx, btn) {
  if (idx === 0) {
    delete _em3gf.sts[cap];
  } else {
    if (!_em3gf.sts[cap]) _em3gf.sts[cap] = {};
    _em3gf.sts[cap][idx] = !_em3gf.sts[cap][idx];
    if (_em3gfStsSel(cap) === null) delete _em3gf.sts[cap]; // tudo desmarcado = todos
  }
  em3FichasBuildNav();
}

function em3gfToggleType(btn) {
  var t = btn.getAttribute('data-type');
  _em3gf.tipos[t] = !_em3gf.tipos[t];
  btn.classList.toggle('active', _em3gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _em3gf.tipos[t] ? '✓' : '';
}

function em3gfSetDif(btn) {
  _em3gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('em3-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function em3gfSetQty(btn) {
  _em3gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('em3-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _em3gfExBloco(exs, startNum) {
  var h = '';
...
function _em3gfGenExs(cap, n) {
  var gen = _em3Gerador(cap);
  if (!gen) {
    ...
    var pool = _em3FichaBanco(cap);
    var temasSel = _em3gfTemasSel(cap);
    if (temasSel) {
      var filtrado = pool.filter(function (q) { return temasSel.indexOf(q.t) !== -1; });
      if (filtrado.length) pool = filtrado;
    }
    return _em3FichaSlice(pool, n, _em3gf.dif);
  }
  var nTemas = _em3TemasCount[cap] || 1;
  ...
  for (var i = 0, tent = 0; geradas.length < n && tent < n * 6; tent++) {
    var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _em3gf.dif);
    i++;
    if (!ex) continue;
  ...
  var banco = (typeof _em3Banco !== 'undefined' && _em3Banco[cap]) ? _em3Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _em3gf.dif);
  }
  return geradas;
}

function em3gfGerar(formato) {
  // capítulos selecionados (conta o banco alargado — em PT não há geradores)
  var capsSel = [];
  _em3CapMeta.forEach(function(m) { if (_em3gf.caps[m.n] && (_em3Gerador(m.n) || _em3FichaBanco(m.n).length)) capsSel.push(m.n); });
  var status = document.getElementById('em3-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _em3gf.tipos.resumo || _em3gf.tipos.exercicios || _em3gf.tipos.teste || _em3gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_em3gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _em3PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _em3CapMeta[cap - 1];
    var color = _em3CapColors[cap] || '#36527a';
    ...
    if (_em3gf.tipos.resumo) {
      var cards = _em3Cards[cap] || [];
      var stsR = _em3gfStsSel(cap);
      if (stsR && cards.length) {
        var palavras = [];
        stsR.forEach(function (i) {
          String((_em3Subtemas[cap] || [])[i - 1] || '').toLowerCase().split(/\s+/).forEach(function (w) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_em3gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) {
    var nome = _em3CapMeta[c - 1].label;
    var sts = _em3gfStsSel(c);
    if (sts) {
      var nomes = sts.map(function (i) { return (_em3Subtemas[c] || [])[i - 1] || ''; }).filter(function (x) { return x; });
  ...
  var _temasNomes = capsSel.map(function(c){ return _em3CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('9', 'portugues', _temasNomes, formato) : ('ficha-em3.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _em3Init);
else _em3Init();
...
_em3Banco[2] = _em3Banco[2].concat([
...
_em3Banco[1] = _em3Banco[1].concat([
...
_em3Banco[3] = _em3Banco[3].concat([
  // F1-F2 · Publicidade
...
/* atribuir: deep-link em3 */
function _em3DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_em3gf){ _em3gf.caps={}; cs.forEach(function(n){ _em3gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _em3gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ em3SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_em3Prat) _em3Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ em3SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_em3TarefaAtiva=p.get('tarefa');_em3TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _em3Prat.cap=cap; _em3Prat.st=st; _em3Prat.nivel=nivel; setTimeout(function(){ em3SwitchTab('exercicios',null); if(typeof em3GerarExercicios==='function') em3GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_em3DeepLinkAuto,300);});else setTimeout(_em3DeepLinkAuto,300);

function em3AtribuirFicha(){
  var caps=[]; _em3CapMeta.forEach(function(m){ if(_em3gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('em3-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_em3CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'em3', cursoNome:'Português 7.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_em3gf.dif };
}

function em3EntregarTarefa(){
  if(!_em3TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_em3TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_em3TarefaResp[qid]; if(ok)certas++; var ex=_em3Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_em3TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _em3TarefaAtiva=null; _em3TarefaResp={}; em3GerarExercicios();
