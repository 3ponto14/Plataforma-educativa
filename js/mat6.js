...
var _mat6TarefaAtiva=null, _mat6TarefaResp={};

function _mat6PratStorageKey(cap) { return 'edupt_mat6_cap' + cap; }

function _mat6SelCfg() {
  return { capMeta: _mat6CapMeta, capColors: _mat6CapColors, subtemas: _mat6Subtemas,
    subtemaTemas: _mat6SubtemaTemas, temasCount: _mat6TemasCount, gerador: _mat6Gerador };
}
function _mat6PratSel() {
  if (!_mat6Prat.sel) {
    _mat6Prat.sel = _selNew();
    if (_mat6Prat.cap) { _mat6Prat.sel.caps = [_mat6Prat.cap]; if (_mat6Prat.st) _mat6Prat.sel.stsByCap[_mat6Prat.cap] = [_mat6Prat.st]; }
  }
  return _mat6Prat.sel;
}
...
function mat6BuildPraticarNav() {
  ...
  capRow.innerHTML = _selBarsHTML(_mat6PratSel(), _mat6SelCfg(), 'mat6PratToggleCap', 'mat6PratToggleSt');
}

function mat6PratToggleCap(cap) {
  _selToggleCap(_mat6PratSel(), _mat6SelCfg(), cap);
  mat6BuildPraticarNav();
  mat6GerarExercicios();
}
function mat6PratToggleSt(cap, st) {
  _selToggleSt(_mat6PratSel(), cap, st);
  mat6BuildPraticarNav();
  mat6GerarExercicios();
}
...
function mat6GerarExercicios() {
  ...
  if (!dest) return;
  var pares = _selPares(_mat6PratSel(), _mat6SelCfg());
  ...
  var cap = _mat6PratSel().caps[0] || (pares[0] && pares[0].cap) || 1;
  ...
  for (var i = 0; i < QTD; i++) {
    ...
    var gen = _mat6Gerador(par.cap);
    if (!gen) continue;
    var ex = gen(par.tema, tipos[i % tipos.length], _mat6Prat.nivel);
    if (ex) geradas.push(ex);
  }
  var temasPorCap = {};
  pares.forEach(function (pr) { (temasPorCap[pr.cap] = temasPorCap[pr.cap] || []).push(pr.tema); });
  var banco = [];
  if (typeof _mat6Banco !== "undefined") {
    Object.keys(temasPorCap).forEach(function (c) {
      if (_mat6Banco[c]) banco = banco.concat(_mat6Banco[c].filter(function (q) { return temasPorCap[c].indexOf(String(q.t)) !== -1; }));
    });
  }
  var exs = (typeof _mixBancoGeradas === "function") ? _mixBancoGeradas(banco, geradas, QTD, 2, _mat6Prat.nivel)
  ...
  var scoreBar = '<div class="score-bar" id="mat6-prat-scorebar">'
    ...
    + (_mat6TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="mat6EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  ...
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    ...
    var _selA = _mat6PratSel();
    var _stA = (_selA.caps.length === 1 && (_selA.stsByCap[cap] || []).length === 1) ? _selA.stsByCap[cap][0] : 0;
    var _sn = (_mat6Subtemas[cap] && _stA > 0) ? (_mat6Subtemas[cap][_stA - 1] || '') : '';
    Atribuir.montar('mat6-atribuir', { curso: 'mat6', cursoNome: 'Matemática 6.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_stA || ''), subNome: _sn, tipo: 'quiz', nivel: _mat6Prat.nivel });
  }
}
...
function mat6CheckEx(qid, tipo, val, btn) {
  ...
  _mat6Prat.score.total++; if(_mat6TarefaAtiva){_mat6TarefaResp[qid]=!!r.correct;}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _mat6Quiz = { cap: 1, sel: null, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function _mat6QuizSel() { if (!_mat6Quiz.sel) _mat6Quiz.sel = _selNew(); return _mat6Quiz.sel; }

function mat6QuizBuildNav() {
  var row = document.getElementById('mat6-quiz-cap-row');
  if (row) row.innerHTML = _selBarsHTML(_mat6QuizSel(), _mat6SelCfg(), 'mat6QuizToggleCap', 'mat6QuizToggleSt');
  var st = document.getElementById('mat6-quiz-st-row'); if (st) st.style.display = 'none';
  mat6QuizStart();
}

function mat6QuizToggleCap(cap) {
  _selToggleCap(_mat6QuizSel(), _mat6SelCfg(), cap);
  mat6QuizBuildNav();
}
function mat6QuizToggleSt(cap, st) {
  _selToggleSt(_mat6QuizSel(), cap, st);
  mat6QuizBuildNav();
}
...
function mat6QuizNext() {
  ...
  var _qpares = _selPares(_mat6QuizSel(), _mat6SelCfg());
  _mat6Quiz.cap = (_mat6QuizSel().caps[0]) || (_qpares[0] && _qpares[0].cap) || 1;
  var ex = _selMcQuestion(_mat6QuizSel(), _mat6SelCfg(), _mat6FillToMc);
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _mat6Teste = { cap: 1, sel: null, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function _mat6TesteSel() { if (!_mat6Teste.sel) _mat6Teste.sel = _selNew(); return _mat6Teste.sel; }

function mat6TesteBuildNav() {
  var row = document.getElementById('mat6-teste-cap-row');
  if (row) row.innerHTML = _selBarsHTML(_mat6TesteSel(), _mat6SelCfg(), 'mat6TesteToggleCap', 'mat6TesteToggleSt');
  var st = document.getElementById('mat6-teste-st-row'); if (st) st.style.display = 'none';
  mat6TesteRenderConfig();
}

function mat6TesteToggleCap(cap) {
  _selToggleCap(_mat6TesteSel(), _mat6SelCfg(), cap);
  mat6TesteBuildNav();
}
function mat6TesteToggleSt(cap, st) {
  _selToggleSt(_mat6TesteSel(), cap, st);
  mat6TesteBuildNav();
}
...
function mat6TesteStart() {
  var pares = _selPares(_mat6TesteSel(), _mat6SelCfg());
  ...
  _mat6Teste.cap = (_mat6TesteSel().caps[0]) || (pares[0] && pares[0].cap) || 1;
  ...
  for (var i = 0; i < _mat6Teste.qtd; i++) {
    var par = pares[i % pares.length];
    var gen = _mat6Gerador(par.cap); if (!gen) continue;
    var ex = gen(par.tema, tipos[i % tipos.length], _mat6Teste.nivel);
...
function mat6JogosInit() {
  (function(){ var pj=document.getElementById('mat6p-jogos'); if(pj && !document.getElementById('mat6-jogos-atr')){ var d=document.createElement('div'); d.id='mat6-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('mat6-jogos-atr',{curso:'mat6',cursoNome:'Matemática 6.º',tipo:'jogo',nivel:'',caps:_mat6CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
...
function mat6TreinarCap(cap) {
  _mat6Prat.cap = cap;
  _mat6Prat.st = 0;
  _mat6Prat.sel = { caps: [cap], stsByCap: {} };
...
function mat6FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('mat6-fichas-atr','mat6AtribuirFicha');
...
function _mat6gfGenExs(cap, n) {
  ...
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _mat6gf.dif);
  }
  return geradas;
}
...
function mat6gfGerar(formato) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_mat6gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
...
function _mat6DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_mat6gf){ _mat6gf.caps={}; cs.forEach(function(n){ _mat6gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _mat6gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ mat6SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_mat6Prat) _mat6Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ mat6SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof gTabSwitch==='function')setTimeout(function(){try{gTabSwitch('mat6-jogos-app',jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_mat6TarefaAtiva=p.get('tarefa');_mat6TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _mat6Prat.cap=cap; _mat6Prat.st=st; _mat6Prat.nivel=nivel; _mat6Prat.sel={caps:[cap],stsByCap:(st?(function(){var o={};o[cap]=[st];return o;})():{})}; setTimeout(function(){ mat6SwitchTab('exercicios',null); if(typeof mat6GerarExercicios==='function') mat6GerarExercicios(); },350); }catch(e){} }
...
function mat6AtribuirFicha(){
  var caps=[]; _mat6CapMeta.forEach(function(m){ if(_mat6gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('mat6-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_mat6CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'mat6', cursoNome:'Matemática 6.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_mat6gf.dif };
}

function mat6EntregarTarefa(){
  if(!_mat6TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_mat6TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_mat6TarefaResp[qid]; if(ok)certas++; var ex=_mat6Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_mat6TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _mat6TarefaAtiva=null; _mat6TarefaResp={}; mat6GerarExercicios();
