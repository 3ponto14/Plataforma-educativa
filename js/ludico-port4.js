...
/* ════════════════════════════════════════════════════════════════
   ludico-port4.js — Atividades lúdicas do 1.º ano de Português.
...
(function () {
  ...
  function _menu(hostId) {
    ...
    for (var i = 0; i < keys.length; i++) {
      var m = MUNDOS[keys[i]];
      html += '<button onclick="LudicoPort4.abrir(\'' + keys[i] + '\')" '
  ...
  function abrir(chave) {
    ...
    if (menu) menu.innerHTML = '<button onclick="LudicoPort4.voltar()" style="background:none;border:none;color:#6a5a8a;font-weight:800;cursor:pointer;font-size:1rem;font-family:Montserrat,sans-serif;margin-bottom:.4rem"><i class="ph ph-arrow-left"></i> Escolher outro jogo</button>';
  ...
  window.LudicoPort4 = { init: init, abrir: abrir, voltar: voltar };
})();
