...
var _port7Cards = {
  ...
  5: [
    { st: 1, tag: 'Figura', q: 'Comparação', a: 'Comparas duas coisas usando uma palavra como «como» ou «tal como». Ex.: «veloz <b>como</b> um relâmpago». Cria uma imagem rápida na cabeça. Truque para a reconhecer: procura o «como».' },
    { st: 1, tag: 'Figura', q: 'Personificação', a: 'Dás vida humana a coisas ou animais — fazes o vento «uivar», a cidade «dormir». Ex.: «o vento <b>uivava</b> lá fora». Torna a cena mais viva, como num desenho animado.' },
    { st: 1, tag: 'Figura', q: 'Metáfora', a: 'É como uma comparação, mas <b>sem</b> o «como» — dizes que uma coisa <b>é</b> a outra. Ex.: «este quarto é uma gelataria» (= está gelado). Dá mais força do que a comparação.' },
    { st: 2, tag: 'Figura', q: 'Onomatopeia', a: 'É uma palavra que <b>imita um som</b>: trrrim!, miau, bzzz, cocorococó. Serve para «ouvirmos» a cena enquanto lemos.' },
    { st: 1, tag: 'Figura', q: 'Anáfora', a: 'Repetes a mesma palavra no <b>início</b> de versos ou frases seguidas. Ex.: «<b>Era</b> a manhã… / <b>Era</b> a luz…». A repetição cria ritmo e martela a ideia.' },
    { st: 1, tag: 'Figura', q: 'Enumeração', a: 'Fazes uma lista de coisas seguidas para reforçar uma ideia. Ex.: «trouxe pão, queijo, fruta e sumo». Dá sensação de quantidade.' },
    { st: 1, tag: 'Figura', q: 'Hipérbole', a: 'É um exagero de propósito, para dar ênfase. Ex.: «já te chamei <b>mil</b> vezes!». Toda a gente percebe que não é a sério — é só para realçar.' },
    ...
    { st: 1, tag: 'Figura', q: 'Antítese', a: 'Juntas duas ideias <b>opostas</b> na mesma frase, para criar contraste. Ex.: «era o <b>melhor</b> dos tempos, era o <b>pior</b> dos tempos». O choque entre os contrários chama a atenção.' },
    { st: 1, tag: 'Estratégia', q: 'Como descobrir uma figura de estilo num verso?', a: 'Procura palavras que não estão à letra e repetições ou sons. Pergunta-te: há uma comparação? um exagero? algo «humano» numa coisa que não é pessoa? um som imitado? Quando uma destas bater, encontraste a figura.' },
    { st: 2, tag: 'Definição', q: 'O que são recursos expressivos?', a: 'São maneiras especiais de usar as palavras (imagens, sons, repetições) para dar mais <b>força, beleza ou emoção</b> ao texto. É o que faz a diferença entre «estava frio» e «o quarto era uma gelataria».' },
    { st: 2, tag: 'Definição', q: 'Qual é o efeito de uma figura de estilo?', a: 'Cada figura serve para algo: a hipérbole <b>realça</b>, a personificação <b>dá vida</b>, a comparação <b>ajuda a imaginar</b>. Quando te perguntarem, diz sempre <b>o que ela faz ali</b>, não só o nome — é isso que conta.' },
...
function port7SwitchTab(tab, btn) {
  ...
  else if (tab === 'aprender') { if (typeof port7AprenderInit === 'function') port7AprenderInit(); }
  else if (tab === 'fichas') port7FichasBuildNav();
  else if (tab === 'progresso') port7RenderProgresso();
}

/* Aba APRENDER: catálogo de explicadores lúdicos de Português 7.º + abrir/voltar. */
var _port7AprenderWired = false;
function port7AprenderInit() {
  if (typeof APRENDER === 'undefined') {
    var c = document.getElementById('port7-aprender-cat');
    if (c) c.innerHTML = '<p style="color:var(--ink4);padding:2rem;text-align:center">Explicadores indisponíveis (motor não carregado).</p>';
    return;
  }
  var host = document.getElementById('port7-aprender-host');
  var cat = document.getElementById('port7-aprender-cat');
  if (!host || !cat) return;
  APRENDER.catalogo('port7-aprender-cat', function (id, t) {
    return /port7/.test(id);
  });
  host.style.display = 'none';
  if (_port7AprenderWired) return;
  _port7AprenderWired = true;
...
function port7GerarExercicios() {
  ...
  if (_port7PratModulo(cap, dest)) {
    ...
    if (_port7TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_port7TarefaAtiva, 'Trabalho concluído');
    return;
  }
...
function port7gfGerar(formato) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_port7gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
...
var _port7Banco = {
  ...
  1: [
    ...
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'O ensinamento ou lição que se retira de uma fábula chama-se:', opcoes: ['moralidade', 'desenlace', 'prólogo', 'refrão'], resposta: 'moralidade', expl: 'A moralidade é a lição de vida com que a fábula termina, muitas vezes explícita.', tema: 'T1 · Narrativas tradicionais' },
    { t: '3', tipo: 'mc', dif: 'facil', enun: 'No poema «Pelo sonho é que vamos», de Sebastião da Gama, a mensagem é:', opcoes: ['o importante é sonhar, mesmo sem garantia de êxito', 'devemos desistir dos sonhos', 'só os ricos podem sonhar', 'o sonho é uma perda de tempo'], resposta: 'o importante é sonhar, mesmo sem garantia de êxito', expl: '«Chegamos? Não chegamos? […] Partimos. Vamos. Somos.»: vale a pena seguir o sonho, haja ou não chegada.', tema: 'T3 · Poesia' },
    { t: '3', tipo: 'mc', dif: 'medio', enun: 'Quantos versos tem uma quadra?', opcoes: ['quatro', 'dois', 'três', 'seis'], resposta: 'quatro', expl: 'Quadra = estrofe de 4 versos; é a mais frequente na poesia tradicional portuguesa.', tema: 'T3 · Poesia' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'A introdução de uma personagem nova numa narrativa, com mudança na ação, é uma:', opcoes: ['peripécia', 'didascália', 'rima', 'moralidade'], resposta: 'peripécia', expl: 'Peripécias são os acontecimentos que fazem avançar e complicar a ação ao longo da narrativa.', tema: 'T2 · Narrativas de autor' },
    { t: '4', tipo: 'mc', dif: 'facil', enun: 'No teatro, a entrada ou saída de uma personagem marca o início de uma nova:', opcoes: ['cena', 'estrofe', 'moralidade', 'rima'], resposta: 'cena', expl: 'A cena delimita-se pela entrada/saída de personagens; o ato é uma divisão maior (muda o cenário/tempo).', tema: 'T4 · Texto dramático' }
  ],
  // ── Domínio 2 · Gramática ──
  2: [
    ...
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'Em «O João correu rapidamente.», o predicado é:', opcoes: ['correu rapidamente', 'O João', 'rapidamente', 'João'], resposta: 'correu rapidamente', expl: 'O predicado é tudo o que se diz do sujeito: o verbo e os seus complementos/modificadores.', tema: 'T2 · Funções Sintáticas' },
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'A palavra «felizmente» pertence à classe dos:', opcoes: ['advérbios', 'adjetivos', 'nomes', 'verbos'], resposta: 'advérbios', expl: 'Termina em -mente e indica o modo: é um advérbio (invariável).', tema: 'T1 · Classes de Palavras' },
    { t: '4', tipo: 'mc', dif: 'medio', enun: 'A forma verbal «cantava» está no:', opcoes: ['pretérito imperfeito do indicativo', 'presente do indicativo', 'futuro do indicativo', 'imperativo'], resposta: 'pretérito imperfeito do indicativo', expl: 'O imperfeito exprime uma ação passada que durava ou se repetia («cantava todos os dias»).', tema: 'T4 · Modos Verbais' },
    { t: '3', tipo: 'mc', dif: 'medio', enun: 'Em «Ou estudas ou reprovas.», a conjunção «ou… ou» liga orações coordenadas:', opcoes: ['disjuntivas', 'copulativas', 'adversativas', 'conclusivas'], resposta: 'disjuntivas', expl: '«Ou… ou» exprime alternativa/exclusão: coordenação disjuntiva.', tema: 'T3 · Orações' },
    { t: '5', tipo: 'mc', dif: 'facil', enun: 'A palavra «gatinho» é formada por:', opcoes: ['derivação por sufixação', 'derivação por prefixação', 'composição', 'onomatopeia'], resposta: 'derivação por sufixação', expl: 'gato + -inho: acrescenta-se um sufixo no fim da palavra (diminutivo).', tema: 'T5 · Léxico e Variação' }
  ],
  // ── Domínio 3 · Leitura ──
  3: [
    ...
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'A ideia principal de um parágrafo é:', opcoes: ['a informação mais importante que ele transmite', 'a primeira palavra do parágrafo', 'o número de linhas', 'a frase mais comprida'], resposta: 'a informação mais importante que ele transmite', expl: 'Cada parágrafo desenvolve normalmente uma ideia principal; o resto são pormenores que a apoiam.', tema: 'T1 · Compreensão' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'Numa notícia, o «lead» (primeiro parágrafo) responde sobretudo a:', opcoes: ['o quê, quem, quando, onde, como e porquê', 'apenas ao título', 'à opinião do jornalista', 'à publicidade da página'], resposta: 'o quê, quem, quando, onde, como e porquê', expl: 'O lead resume o essencial da notícia respondendo às perguntas-base do jornalismo.', tema: 'T2 · Tipologias' },
    { t: '2', tipo: 'mc', dif: 'facil', enun: 'Um texto que dá instruções passo a passo (como uma receita) é:', opcoes: ['um texto instrucional', 'uma notícia', 'um poema', 'uma biografia'], resposta: 'um texto instrucional', expl: 'O texto instrucional orienta a ação (receitas, manuais, regras de jogo), por etapas e com verbos no imperativo.', tema: 'T2 · Tipologias' },
    { t: '3', tipo: 'mc', dif: 'medio', enun: 'Em «Custou-me os olhos da cara.», a expressão significa:', opcoes: ['foi muito caro', 'magoei os olhos', 'foi muito barato', 'perdi a visão'], resposta: 'foi muito caro', expl: 'É uma expressão idiomática de sentido figurado: «os olhos da cara» = muito dinheiro.', tema: 'T3 · Sentidos' },
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'Para encontrar uma informação específica num texto longo, deves:', opcoes: ['procurar palavras-chave ligadas à pergunta', 'ler tudo de trás para a frente', 'ler só o título', 'contar os parágrafos'], resposta: 'procurar palavras-chave ligadas à pergunta', expl: 'A leitura de pesquisa (scanning) procura no texto as palavras-chave da pergunta, sem ler tudo.', tema: 'T1 · Compreensão' },
    { t: '2', tipo: 'vf', dif: 'facil', enun: 'A notícia e o anúncio publicitário têm o mesmo objetivo principal.', resposta: 'F', expl: 'A notícia INFORMA sobre um facto; o anúncio quer PERSUADIR (vender ou convencer).', tema: 'T2 · Tipologias' },
    { t: '3', tipo: 'mc', dif: 'dificil', enun: 'Lê: «Quando abriu a porta, o silêncio da casa pesava-lhe nos ombros.» Pode inferir-se que a personagem:', opcoes: ['estava só e sentia-se em baixo', 'tinha muitos convidados em casa', 'estava muito alegre', 'ia dar uma festa'], resposta: 'estava só e sentia-se em baixo', expl: 'O «silêncio» e o «peso nos ombros» (sentido figurado) sugerem solidão e tristeza: inferência apoiada nas pistas.', tema: 'T3 · Sentidos' }
  ],
  // ── Domínio 4 · Escrita ──
  4: [
    ...
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'Numa narrativa, o momento de maior tensão, em que o conflito atinge o auge, é:', opcoes: ['o clímax', 'a situação inicial', 'a apresentação das personagens', 'o título'], resposta: 'o clímax', expl: 'O clímax é o ponto culminante da ação, logo antes do desenlace que resolve o conflito.', tema: 'T1 · Texto narrativo' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'Para introduzir um argumento contrário num texto de opinião, usa-se um conector como:', opcoes: ['no entanto', 'por exemplo', 'em primeiro lugar', 'finalmente'], resposta: 'no entanto', expl: '«No entanto», «contudo» e «por outro lado» introduzem uma ideia que se opõe à anterior (contra-argumento).', tema: 'T2 · Texto de opinião' },
    { t: '3', tipo: 'mc', dif: 'medio', enun: 'Ao descrever uma pessoa, a caracterização que indica o feitio (generoso, tímido…) chama-se:', opcoes: ['psicológica', 'física', 'geográfica', 'temporal'], resposta: 'psicológica', expl: 'Caracterização física = aspeto exterior; psicológica = personalidade, sentimentos e modo de ser.', tema: 'T3 · Texto descritivo' },
    { t: '2', tipo: 'vf', dif: 'facil', enun: 'Num texto de opinião, as conjunções «porque» e «pois» ajudam a justificar as ideias.', resposta: 'V', expl: 'São conectores de causa/explicação: introduzem as razões que sustentam a opinião.', tema: 'T2 · Texto de opinião' },
    { t: '1', tipo: 'mc', dif: 'dificil', enun: 'Qual destas frases mostra melhor a ação em vez de a «contar»?', opcoes: ['«Bateu a porta com força e saiu sem olhar para trás.»', '«Ele estava muito zangado.»', '«A personagem ficou nervosa.»', '«Foi um dia mau.»'], resposta: '«Bateu a porta com força e saiu sem olhar para trás.»', expl: 'Mostrar (com ações concretas) é mais vivo do que dizer («estava zangado»): regra de ouro da escrita narrativa.', tema: 'T1 · Texto narrativo' },
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'A revisão de um texto serve, antes de mais, para:', opcoes: ['corrigir erros e melhorar a clareza', 'aumentar o número de páginas', 'mudar o tema', 'apagar o título'], resposta: 'corrigir erros e melhorar a clareza', expl: 'Rever é reler com olhar crítico: ortografia, pontuação, repetições e ligação entre ideias.', tema: 'T1 · Texto narrativo' },
    { t: '3', tipo: 'mc', dif: 'facil', enun: 'Para ligar as frases de um texto e evitar que pareçam soltas, usam-se:', opcoes: ['conectores (palavras de ligação)', 'mais pontos finais', 'letras maiúsculas', 'mais parágrafos'], resposta: 'conectores (palavras de ligação)', expl: 'Os conectores (depois, porque, mas, por isso…) dão coesão e fazem o texto «fluir».', tema: 'T3 · Texto descritivo' }
  ],
  ...
  5: [
    ...
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'Em «O teu sorriso é o sol da manhã.», a figura de estilo é:', opcoes: ['metáfora', 'comparação', 'onomatopeia', 'enumeração'], resposta: 'metáfora', expl: 'Identifica o sorriso COM o sol, sem usar «como»: é uma metáfora (comparação implícita).', tema: 'T1 · Figuras' },
    { t: '1', tipo: 'mc', dif: 'dificil', enun: 'Qual é a diferença entre comparação e metáfora?', opcoes: ['a comparação usa um conector («como», «tal qual»); a metáfora não', 'a metáfora é sempre mais curta', 'a comparação só existe em poesia', 'não há diferença'], resposta: 'a comparação usa um conector («como», «tal qual»); a metáfora não', expl: '«És forte como um leão» (comparação) vs. «És um leão» (metáfora): a metáfora junta diretamente os elementos.', tema: 'T1 · Figuras' },
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'Em «As flores sorriam no jardim.», a figura de estilo é:', opcoes: ['personificação', 'hipérbole', 'comparação', 'onomatopeia'], resposta: 'personificação', expl: 'Sorrir é próprio de pessoas: atribuído às flores, é personificação.', tema: 'T1 · Figuras' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'Em «Comprou pão, leite, queijo, fruta e bolachas.», o recurso usado é:', opcoes: ['enumeração', 'anáfora', 'comparação', 'hipérbole'], resposta: 'enumeração', expl: 'A enumeração apresenta uma sequência de elementos, separados por vírgulas.', tema: 'T2 · Recursos' },
    { t: '2', tipo: 'vf', dif: 'facil', enun: 'A aliteração é a repetição de sons consonantais ao longo de um verso ou frase.', resposta: 'V', expl: 'Ex.: «o rato roeu a rolha» repete o som «r»: aliteração, que cria musicalidade.', tema: 'T2 · Recursos' },
    { t: '3', tipo: 'mc', dif: 'medio', enun: 'Num poema, dois versos seguidos com o mesmo número de sílabas dizem-se:', opcoes: ['versos isométricos', 'versos brancos', 'versos livres', 'versos soltos'], resposta: 'versos isométricos', expl: 'Isométricos = mesma medida (igual número de sílabas métricas); heterométricos = medidas diferentes.', tema: 'T3 · Versificação' },
    { t: '3', tipo: 'mc', dif: 'facil', enun: 'Versos que não têm rima nem métrica regular chamam-se:', opcoes: ['versos livres', 'quadras', 'redondilhas', 'versos emparelhados'], resposta: 'versos livres', expl: 'O verso livre não obedece a esquema de rima nem a número fixo de sílabas: muito usado na poesia moderna.', tema: 'T3 · Versificação' }
  ]
};
...
function _port7DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_port7gf){ _port7gf.caps={}; cs.forEach(function(n){ _port7gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _port7gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ port7SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_port7Prat) _port7Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ port7SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
