/* ════════════════════════════════════════════════════════════════
   ...
   Sem módulos realojados: a prática é banco-driven (_port3Banco).
...
// Subtemas por domínio (3.º ano · iniciação à leitura e à escrita)
var _port3Subtemas = {
...
var _port3Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function port3SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('port3p-' + tab);
  ...
  if (tab === 'resumo') port3BuildResumoNav();
  else if (tab === 'exercicios') port3BuildPraticarNav();
  else if (tab === 'quiz') port3QuizBuildNav();
  else if (tab === 'flashcards') port3FcBuildNav();
  else if (tab === 'teste') port3TesteBuildNav();
  else if (tab === 'jogos') port3JogosInit();
  else if (tab === 'jogar') { if (typeof LudicoPort3 !== 'undefined') LudicoPort3.init('port3-ludico-menu', 'port3-ludico-jogo'); }
  else if (tab === 'fichas') port3FichasBuildNav();
  else if (tab === 'progresso') port3RenderProgresso();
}

// Abre um sub-modo de prática a partir dos cartões do menu Praticar.
function port3OpenPraticar(modo) { port3SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function port3BuildResumoNav() {
  var capRow = document.getElementById('port3-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _port3Sel['resumo'] || 1;
  var h = '';
  _port3CapMeta.forEach(function(m) {
    var color = _port3CapColors[m.n] || '#516860';
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="port3ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  port3ResumoShowSts(activeCap);
  port3RenderResumoInline();
}

function port3ResumoSelectCap(cap, btn) {
  _port3Sel['resumo'] = cap;
  _port3Sel['resumo-st'] = 0;
  var capRow = document.getElementById('port3-resumo-cap-row');
  if (capRow) {
  ...
  if (btn) {
    var color = _port3CapColors[cap] || '#516860';
    btn.classList.add('active');
    btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff';
  }
  port3ResumoShowSts(cap);
  port3RenderResumoInline();
}

function port3ResumoShowSts(cap) {
  var stRow = document.getElementById('port3-resumo-st-row');
  if (!stRow) return;
  var sts = _port3Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  // conta cards por subtema para esconder chips de subtemas sem teoria
  var _cc = (typeof _port3Cards !== 'undefined' && _port3Cards[cap]) ? _port3Cards[cap] : [];
  var _nSt = {}; _cc.forEach(function(c){ if (c.st) _nSt[c.st] = (_nSt[c.st]||0)+1; });
  var _stSel = _port3Sel['resumo-st'] || 0;
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_stSel === 0 ? ' active' : '') + '" data-st="0" onclick="port3ResumoSelectSt(this,' + cap + ',0)">Todos</button>';
  sts.forEach(function(st, i) {
    if (!_nSt[i + 1]) return; // subtema sem flashcards → não mostra o chip
    var isActive = _stSel === (i + 1);
    h += '<button class="resumo-st-btn' + (isActive ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="port3ResumoSelectSt(this,' + cap + ',' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function port3ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('port3-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _port3Sel['resumo'] = cap;
  _port3Sel['resumo-st'] = stIdx;
  port3RenderResumoInline();
}

function port3RenderResumoInline() {
  var cap = _port3Sel['resumo'] || 1;
  var dest = document.getElementById('port3-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _port3CapColors[cap] || '#516860';
    return c;
  })();
  ...
  var allCards = _port3Cards[cap] || [];

  // Filtrar por subtema, se selecionado
  var stIdx = _port3Sel['resumo-st'] || 0;
  ...
  _port3PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
// Português não tem geradores procedurais — a prática usa o _port3Banco.
// Só os caps com gerador são "praticáveis".
// Português não tem geradores procedurais (as perguntas são de análise/banco).
// _port3Gerador devolve sempre null → o hub usa apenas o _port3Banco.
function _port3Gerador(cap) {
  return null;
}
// Um domínio está "disponível" para prática se tiver questões no banco.
function _port3TemConteudo(cap) {
  return !!(_port3Banco[cap] && _port3Banco[cap].length);
}
// Nº de subtemas por domínio (espelha _port3Subtemas) — usado na seleção de prática.
var _port3TemasCount = { 1: 4, 2: 5, 3: 3, 4: 3, 5: 3 };

// Estado da prática
var _port3Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _port3TarefaAtiva=null, _port3TarefaResp={};

function _port3PratStorageKey(cap) { return 'edupt_port3_cap' + cap; }

function port3BuildPraticarNav() {
  var capRow = document.getElementById('port3-praticar-cap-row');
  if (!capRow) return;
  var activeCap = _port3Prat.cap || 1;
  var h = '';
  _port3CapMeta.forEach(function(m) {
    var hasGen = _port3TemConteudo(m.n);
    var color = _port3CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    var disabledStyle = hasGen ? '' : 'opacity:.45;cursor:not-allowed';
    var onclick = hasGen ? 'port3PraticarSelectCap(' + m.n + ',this)' : '';
  ...
  capRow.innerHTML = h;
  port3PraticarShowSts(activeCap);
  port3GerarExercicios(); // renderiza logo o domínio ativo (módulo ou banco)
}

function port3PraticarShowSts(cap) {
  var stRow = document.getElementById('port3-praticar-st-row');
  if (!stRow) return;
  var sts = _port3Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_port3Prat.st === 0 ? ' active' : '') + '" data-st="0" onclick="port3PraticarSelectSt(this,0)">Todos</button>';
  sts.forEach(function(st, i) {
    h += '<button class="resumo-st-btn' + (_port3Prat.st === (i + 1) ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="port3PraticarSelectSt(this,' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function port3PraticarSelectCap(cap, btn) {
  if (!_port3TemConteudo(cap)) return;
  _port3Prat.cap = cap;
  _port3Prat.st = 0;
  var capRow = document.getElementById('port3-praticar-cap-row');
  ...
  if (btn) { var color = _port3CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff'; }
  port3PraticarShowSts(cap);
  port3GerarExercicios();
}

function port3PraticarSelectSt(btn, stIdx) {
  var stRow = document.getElementById('port3-praticar-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _port3Prat.st = stIdx;
...
function port3PraticarSetNivel(nivel, btn) {
  _port3Prat.nivel = nivel;
  var grp = document.getElementById('port3-praticar-nivel');
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _port3PratModulo(cap, dest) {
  // O 7.º ano não tem módulos realojados: a prática é toda banco-driven.
  return false;
}

function port3GerarExercicios() {
  var dest = document.getElementById('port3-praticar-content');
  if (!dest) return;
  var cap = _port3Prat.cap, gen = _port3Gerador(cap);
  ...
  var lvlBar = document.getElementById('port3-praticar-levelbar');
  var stRow = document.getElementById('port3-praticar-st-row');
  if (_port3PratModulo(cap, dest)) {
    if (lvlBar) lvlBar.style.display = 'none';
    if (stRow) stRow.style.display = 'none';
    if (_port3TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_port3TarefaAtiva, 'Trabalho concluído');
    return;
  }
  if (lvlBar) lvlBar.style.display = '';

  // Sem gerador procedural nem banco → nada a mostrar
  if (!gen && !(_port3Banco[cap] && _port3Banco[cap].length)) { dest.innerHTML = ''; return; }

  // Que temas usar?
  var temas;
  if (_port3Prat.st > 0 && _port3SubtemaTemas[cap] && _port3SubtemaTemas[cap][_port3Prat.st]) {
    temas = _port3SubtemaTemas[cap][_port3Prat.st];
  } else {
    temas = [];
    for (var t = 1; t <= (_port3TemasCount[cap] || 1); t++) temas.push(String(t));
  }
  ...
  if (gen) {
    for (var i = 0; i < QTD; i++) {
      ...
      var ex = gen(tema, tipos[i % tipos.length], _port3Prat.nivel);
      if (ex) geradas.push(ex);
    }
  }
  var banco = (typeof _port3Banco !== 'undefined' && _port3Banco[cap]) ? _port3Banco[cap].filter(function (q) { return temas.indexOf(q.t) !== -1; }) : [];
  var exs;
  if (!gen) {
    ...
    exs = (typeof _mixBancoGeradas === 'function') ? _mixBancoGeradas(banco, geradas, QTD, 3, _port3Prat.nivel)
      : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  }
  _port3Prat.exs = exs;
  _port3Prat.answered = {};
  _port3Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="port3-prat-scorebar">'
    + '<div><div class="score-num" id="port3-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="port3-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="port3-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="port3GerarExercicios()">↺ Novas questões</button>'
    + (_port3TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="port3EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'port3CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="port3-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _port3CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _sn = (_port3Subtemas[cap] && _port3Prat.st > 0) ? (_port3Subtemas[cap][_port3Prat.st - 1] || '') : '';
    Atribuir.montar('port3-atribuir', { curso: 'port3', cursoNome: 'Português 7.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_port3Prat.st || ''), subNome: _sn, tipo: 'quiz', nivel: _port3Prat.nivel });
  }
}

function port3CheckEx(qid, tipo, val, btn) {
  if (_port3Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _port3Prat.answered[qid] = true;
  if (r.correct) _port3Prat.score.correct++;
  _port3Prat.score.total++; if(_port3TarefaAtiva){_port3TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('port3-prat-score'); if (se) se.textContent = _port3Prat.score.correct;
  var te = document.getElementById('port3-prat-total'); if (te) te.textContent = '/ ' + _port3Prat.score.total;
  var pe = document.getElementById('port3-prat-prog'); if (pe && _port3Prat.exs.length) pe.style.width = (_port3Prat.score.total / _port3Prat.exs.length * 100) + '%';
  port3SaveProgress(_port3Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function port3SaveProgress(cap, correct) {
  try {
    var key = _port3PratStorageKey(cap);
...
function _port3PM(cap, tipo, opts) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _port3BuildCapRow(rowId, activeCap, onclickName) {
...
function _port3SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
...
function _port3FichaSlice(banco, n, dif) {
...
function _port3BuildMcQuestion(cap) {
  var qs = _port3FichaBanco(cap).filter(function (q) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _port3Quiz = { cap: 1, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function port3QuizBuildNav() {
  if (!_port3TemConteudo(_port3Quiz.cap)) _port3Quiz.cap = 1;
  _port3BuildCapRow('port3-quiz-cap-row', _port3Quiz.cap, 'port3QuizSelectCap');
  port3QuizStart();
}

function port3QuizSelectCap(cap, btn) {
  if (!_port3TemConteudo(cap)) return;
  _port3SetActiveCapBtn('port3-quiz-cap-row', btn, cap);
  _port3Quiz.cap = cap;
  port3QuizStart();
}

function port3QuizStart() {
  _port3Quiz.lives = 3; _port3Quiz.streak = 0; _port3Quiz.maxStreak = 0;
  _port3Quiz.score = 0; _port3Quiz.total = 0; _port3Quiz.answered = false;
  port3QuizNext();
}

function port3QuizNext() {
  var app = document.getElementById('port3-quiz-app');
  if (!app) return;
  if (_port3Quiz.lives <= 0) { port3QuizGameOver(app); return; }
  var ex = _port3BuildMcQuestion(_port3Quiz.cap);
  ...
  _port3Quiz.current = ex; _port3Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _port3Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="port3qopt-' + idx + '" onclick="port3QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_port3Quiz.streak > 1 ? '🔥 ' + _port3Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _port3Quiz.score + ' / ' + _port3Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="port3-quiz-fb" style="min-height:2.5rem"></div>';
}

function port3QuizAnswer(idx) {
  if (_port3Quiz.answered) return;
  _port3Quiz.answered = true;
  var ex = _port3Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('port3qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('port3-quiz-fb');
  if (correct) {
    _port3Quiz.score++; _port3Quiz.streak++;
    if (_port3Quiz.streak > _port3Quiz.maxStreak) _port3Quiz.maxStreak = _port3Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _port3Quiz.lives--; _port3Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  port3SaveProgress(_port3Quiz.cap, correct);
  setTimeout(function(){ port3QuizNext(); }, 1400);
}

function port3QuizGameOver(app) {
  var pct = _port3Quiz.total > 0 ? Math.round(_port3Quiz.score / _port3Quiz.total * 100) : 0;
  _port3PM(_port3Quiz.cap, 'quiz', { pontuacao: _port3Quiz.score, total: _port3Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _port3Quiz.score + ' certas em ' + _port3Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_port3Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="port3QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _port3Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function port3FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_port3Cards[_port3Fc.cap] && _port3Cards[_port3Fc.cap].length)) {
    for (var c = 1; c <= _port3CapMeta.length; c++) { if (_port3Cards[c] && _port3Cards[c].length) { _port3Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('port3-fc-cap-row');
  if (row) {
    var h = '';
    _port3CapMeta.forEach(function(m) {
      var has = !!(_port3Cards[m.n] && _port3Cards[m.n].length);
      var color = _port3CapColors[m.n] || '#516860';
      var isActive = _port3Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('port3FcSelectCap(' + m.n + ',this)') : '';
  ...
  port3FcSelectCap(_port3Fc.cap, null);
}

function port3FcSelectCap(cap, btn) {
  if (!(_port3Cards[cap] && _port3Cards[cap].length)) return;
  if (btn) _port3SetActiveCapBtn('port3-fc-cap-row', btn, cap);
  _port3Fc.cap = cap;
  _port3Fc.cards = _port3Cards[cap].slice();
  _port3Fc.idx = 0; _port3Fc.flipped = false;
  _port3PM(cap, 'flashcard');
  port3FcRender();
}

function port3FcRender() {
  var app = document.getElementById('port3-fc-app');
  if (!app) return;
  var cards = _port3Fc.cards;
  ...
  var card = cards[_port3Fc.idx];
  var color = _port3CapColors[_port3Fc.cap] || '#516860';
  var pct = Math.round((_port3Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_port3Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="port3FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_port3Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_port3Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="port3FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="port3FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="port3FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function port3FcFlip() { _port3Fc.flipped = !_port3Fc.flipped; port3FcRender(); }
function port3FcNext() { _port3Fc.idx = (_port3Fc.idx + 1) % (_port3Fc.cards.length || 1); _port3Fc.flipped = false; port3FcRender(); }
function port3FcPrev() { _port3Fc.idx = (_port3Fc.idx - 1 + (_port3Fc.cards.length || 1)) % (_port3Fc.cards.length || 1); _port3Fc.flipped = false; port3FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _port3Teste = { cap: 1, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function port3TesteBuildNav() {
  if (!_port3TemConteudo(_port3Teste.cap)) _port3Teste.cap = 1;
  _port3BuildCapRow('port3-teste-cap-row', _port3Teste.cap, 'port3TesteSelectCap');
  port3TesteRenderConfig();
}

function port3TesteSelectCap(cap, btn) {
  if (!_port3TemConteudo(cap)) return;
  _port3SetActiveCapBtn('port3-teste-cap-row', btn, cap);
  _port3Teste.cap = cap;
  port3TesteRenderConfig();
}

function port3TesteRenderConfig() {
  var app = document.getElementById('port3-teste-app');
  if (!app) return;
  if (_port3Teste.timer) { clearInterval(_port3Teste.timer); _port3Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="port3-teste-nivel">' +
            '<button class="gen-level-btn' + (_port3Teste.nivel==='facil'?' active':'') + '" onclick="port3TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_port3Teste.nivel==='medio'?' active':'') + '" onclick="port3TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_port3Teste.nivel==='dificil'?' active':'') + '" onclick="port3TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="port3-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="port3-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="port3TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function port3TesteSetNivel(nivel, btn) {
  _port3Teste.nivel = nivel;
  var grp = document.getElementById('port3-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function port3TesteStart() {
  var gen = _port3Gerador(_port3Teste.cap);
  var qtdEl = document.getElementById('port3-teste-qtd'), tempoEl = document.getElementById('port3-teste-tempo');
  _port3Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _port3Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var exs = [];
  if (gen) {
    var nTemas = _port3TemasCount[_port3Teste.cap] || 1;
    var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
    for (var i = 0; i < _port3Teste.qtd; i++) {
      var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _port3Teste.nivel);
      if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
    }
  } else {
    // PT: teste sai do banco alargado do domínio
    exs = _port3FichaSlice(_port3FichaBanco(_port3Teste.cap), _port3Teste.qtd, _port3Teste.nivel);
  }
  if (!exs.length) return;
  _port3Teste.exs = exs; _port3Teste.answered = {}; _port3Teste.score = { correct: 0, total: 0 };
  _port3Teste.restante = _port3Teste.tempo;

  var app = document.getElementById('port3-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'port3TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="port3-teste-timer">' + _port3FmtTime(_port3Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="port3-teste-answered">0 / ' + _port3Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="port3TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="port3-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _port3Teste.timer = setInterval(function() {
    _port3Teste.restante--;
    var t = document.getElementById('port3-teste-timer');
    if (t) t.textContent = _port3FmtTime(_port3Teste.restante);
    if (_port3Teste.restante <= 0) port3TesteFinish();
  }, 1000);
}

function _port3FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function port3TesteCheck(qid, tipo, val, btn) {
  if (_port3Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _port3Teste.answered[qid] = true;
  if (r.correct) _port3Teste.score.correct++;
  _port3Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('port3-teste-answered');
  if (a) a.textContent = _port3Teste.score.total + ' / ' + _port3Teste.qtd;
  port3SaveProgress(_port3Teste.cap, r.correct);
  if (_port3Teste.score.total >= _port3Teste.qtd) setTimeout(port3TesteFinish, 600);
}

function port3TesteFinish() {
  if (_port3Teste.timer) { clearInterval(_port3Teste.timer); _port3Teste.timer = null; }
  var app = document.getElementById('port3-teste-app');
  if (!app) return;
  var total = _port3Teste.qtd;
  var correct = _port3Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _port3PM(_port3Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="port3TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function port3JogosInit() {
  (function(){ var pj=document.getElementById('port3p-jogos'); if(pj && !document.getElementById('port3-jogos-atr')){ var d=document.createElement('div'); d.id='port3-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('port3-jogos-atr',{curso:'port3',cursoNome:'Português 7.º',tipo:'jogo',nivel:'',caps:_port3CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  if (typeof ptJogosRender === 'function') {
    if (typeof ptJogosConfig === 'function') {
      ptJogosConfig({
        appId: 'port3-jogos-app',
        fonte: function () {
          var out = [];
          [1, 2, 3, 4, 5].forEach(function (c) { out = out.concat(_port3FichaBanco(c)); });
          return out;
        },
        forca: PT7_FORCA
      });
    ...
    var app = document.getElementById('port3-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_port3_capN (gravado por port3SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _port3ProgGetCaps() {
  var out = [];
  var nCaps = _port3CapMeta.length;
  for (var cap = 1; cap <= nCaps; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_port3_cap' + cap) || '{}');
...
function port3RenderProgresso() {
  var el = document.getElementById('port3-progresso-content');
  if (!el) return;
  var caps = _port3ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="port3ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m9c1-base);border:1.5px solid var(--m9c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="port3ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  caps.forEach(function(c) {
    var m = _port3CapMeta[c.cap - 1];
    var color = _port3CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _port3CapMeta[pior.cap - 1];
      var colorp = _port3CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="port3TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="port3ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function port3TreinarCap(cap) {
  _port3Prat.cap = cap;
  _port3Prat.st = 0;
  port3SwitchTab('exercicios', null);
}

function port3ProgReset() {
  ...
  for (var cap = 1; cap <= _port3CapMeta.length; cap++) { try { localStorage.removeItem('edupt_port3_cap' + cap); } catch (e) {} }
...
function port3ProgDownloadPDF() {
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-port3.pdf');
...
function _port3gfStsSel(cap) {
  var sel = _port3gf.sts[cap];
  if (!sel) return null;
  var out = [];
...
function _port3gfTemasSel(cap) {
  var sts = _port3gfStsSel(cap);
  if (!sts) return null;
  var mapa = _port3SubtemaTemas[cap] || {};
  var temas = [];
...
function port3FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('port3-fichas-atr','port3AtribuirFicha');
  var el = document.getElementById('port3-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com conteúdo
  var temAlgum = false;
  for (var k in _port3gf.caps) { if (_port3gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _port3CapMeta.forEach(function(m) {
    var hasGen = _port3TemConteudo(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _port3gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_port3gf.caps[m.n];
    var color = _port3CapColors[m.n] || '#516860';
    var style = sel ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    h += '<button class="gf-cap-btn' + (sel ? ' active' : '') + '" data-cap="' + m.n + '" onclick="port3gfToggleCap(' + m.n + ',this)" style="' + style + '">' + m.icon + ' ' + m.label + '</button>';
    // tópicos do capítulo selecionado
    if (sel) {
      var sts = _port3Subtemas[m.n] || [];
      if (sts.length) {
        var escolhidos = _port3gf.sts[m.n] || {};
        var algumSt = _port3gfStsSel(m.n) !== null;
        ...
        h += '<button onclick="port3gfToggleSt(' + m.n + ',0,this)" style="' + _port3gfStStyle(!algumSt, color) + '">Todos</button>';
        sts.forEach(function (st, i) {
          var on = !!escolhidos[i + 1];
          h += '<button onclick="port3gfToggleSt(' + m.n + ',' + (i + 1) + ',this)" style="' + _port3gfStStyle(on, color) + '">' + st + '</button>';
        });
        h += '</div>';
      }
    }
...
function _port3gfStStyle(on, color) {
...
function port3gfToggleCap(cap, btn) {
  _port3gf.caps[cap] = !_port3gf.caps[cap];
  if (!_port3gf.caps[cap]) delete _port3gf.sts[cap]; // desmarcar cap limpa os tópicos
  port3FichasBuildNav(); // re-render para mostrar/esconder a linha de tópicos
}

// idx 0 = «Todos» (limpa a seleção); idx 1..n alterna o tópico.
function port3gfToggleSt(cap, idx, btn) {
  if (idx === 0) {
    delete _port3gf.sts[cap];
  } else {
    if (!_port3gf.sts[cap]) _port3gf.sts[cap] = {};
    _port3gf.sts[cap][idx] = !_port3gf.sts[cap][idx];
    if (_port3gfStsSel(cap) === null) delete _port3gf.sts[cap]; // tudo desmarcado = todos
  }
  port3FichasBuildNav();
}

function port3gfToggleType(btn) {
  var t = btn.getAttribute('data-type');
  _port3gf.tipos[t] = !_port3gf.tipos[t];
  btn.classList.toggle('active', _port3gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _port3gf.tipos[t] ? '✓' : '';
}

function port3gfSetDif(btn) {
  _port3gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('port3-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function port3gfSetQty(btn) {
  _port3gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('port3-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _port3gfExBloco(exs, startNum) {
  var h = '';
...
function _port3gfGenExs(cap, n) {
  var gen = _port3Gerador(cap);
  if (!gen) {
    ...
    var pool = _port3FichaBanco(cap);
    var temasSel = _port3gfTemasSel(cap);
    if (temasSel) {
      var filtrado = pool.filter(function (q) { return temasSel.indexOf(q.t) !== -1; });
      if (filtrado.length) pool = filtrado;
    }
    return _port3FichaSlice(pool, n, _port3gf.dif);
  }
  var nTemas = _port3TemasCount[cap] || 1;
  ...
  for (var i = 0, tent = 0; geradas.length < n && tent < n * 6; tent++) {
    var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _port3gf.dif);
    i++;
    if (!ex) continue;
  ...
  var banco = (typeof _port3Banco !== 'undefined' && _port3Banco[cap]) ? _port3Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _port3gf.dif);
  }
  return geradas;
}

function port3gfGerar(formato) {
  // capítulos selecionados (conta o banco alargado — em PT não há geradores)
  var capsSel = [];
  _port3CapMeta.forEach(function(m) { if (_port3gf.caps[m.n] && (_port3Gerador(m.n) || _port3FichaBanco(m.n).length)) capsSel.push(m.n); });
  var status = document.getElementById('port3-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _port3gf.tipos.resumo || _port3gf.tipos.exercicios || _port3gf.tipos.teste || _port3gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_port3gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _port3PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _port3CapMeta[cap - 1];
    var color = _port3CapColors[cap] || '#36527a';
    ...
    if (_port3gf.tipos.resumo) {
      var cards = _port3Cards[cap] || [];
      var stsR = _port3gfStsSel(cap);
      if (stsR && cards.length) {
        var palavras = [];
        stsR.forEach(function (i) {
          String((_port3Subtemas[cap] || [])[i - 1] || '').toLowerCase().split(/\s+/).forEach(function (w) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_port3gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) {
    var nome = _port3CapMeta[c - 1].label;
    var sts = _port3gfStsSel(c);
    if (sts) {
      var nomes = sts.map(function (i) { return (_port3Subtemas[c] || [])[i - 1] || ''; }).filter(function (x) { return x; });
  ...
  var _temasNomes = capsSel.map(function(c){ return _port3CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('9', 'portugues', _temasNomes, formato) : ('ficha-port3.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _port3Init);
else _port3Init();
...
_port3Banco[2] = _port3Banco[2].concat([
...
_port3Banco[1] = _port3Banco[1].concat([
...
_port3Banco[3] = _port3Banco[3].concat([
  // F1-F2 · Publicidade
...
/* atribuir: deep-link port3 */
function _port3DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_port3gf){ _port3gf.caps={}; cs.forEach(function(n){ _port3gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _port3gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ port3SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_port3Prat) _port3Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ port3SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_port3TarefaAtiva=p.get('tarefa');_port3TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _port3Prat.cap=cap; _port3Prat.st=st; _port3Prat.nivel=nivel; setTimeout(function(){ port3SwitchTab('exercicios',null); if(typeof port3GerarExercicios==='function') port3GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_port3DeepLinkAuto,300);});else setTimeout(_port3DeepLinkAuto,300);

function port3AtribuirFicha(){
  var caps=[]; _port3CapMeta.forEach(function(m){ if(_port3gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('port3-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_port3CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'port3', cursoNome:'Português 7.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_port3gf.dif };
}

function port3EntregarTarefa(){
  if(!_port3TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_port3TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_port3TarefaResp[qid]; if(ok)certas++; var ex=_port3Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_port3TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _port3TarefaAtiva=null; _port3TarefaResp={}; port3GerarExercicios();
