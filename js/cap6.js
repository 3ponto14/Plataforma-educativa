...
var BANCO6={
  questoes:[
    ...
    {id:'q6-2',tema:1,visual:svgCoordGrid({points:[{x:-3,y:5,label:'A(−3,5)'}],xMin:-6,xMax:6,yMin:-6,yMax:6}),enunciado:'O ponto A(−3, 5) pertence ao:',opts:['A) 1.º quadrante','B) 2.º quadrante','C) 3.º quadrante','D) Eixo Ox'],correct:'B',fb:'Regra dos quadrantes: 2.º Q → abcissa < 0 e ordenada > 0.\nA(−3, 5): abcissa = −3 < 0 ✓ e ordenada = 5 > 0 ✓.\nLogo A pertence ao 2.º quadrante.'},
    {id:'q6-3',tema:1,visual:svgCoordGrid({points:[{x:5,y:5,label:'F(5,5)',color:'#2a7ae2'},{x:5,y:-5,label:"F'(5,−5)",color:'#e05c5c'}],xMin:-6,xMax:6,yMin:-6,yMax:6}),enunciado:'O ponto F(5, 5) é refletido em relação ao eixo Ox. Quais são as coordenadas de F\'?',opts:["A) (−5, 5)","B) (5, 5)","C) (5, −5)","D) (−5, −5)"],correct:'C',fb:"Regra: reflexão em relação ao eixo Ox → (x, y) → (x, −y).\nA abcissa mantém-se; a ordenada muda de sinal.\nF(5, 5) → F'(5, −5). ✓"},
    {id:'q6-4',tema:1,visual:svgCoordGrid({points:[{x:3,y:2,label:'K(3,2)',color:'#2a7ae2'},{x:-3,y:2,label:"K'(−3,2)",color:'#e05c5c'}],xMin:-6,xMax:6,yMin:-6,yMax:6}),enunciado:'O ponto K(3, 2) é refletido em relação ao eixo Oy. Quais são as coordenadas de K\'?',opts:["A) (3, 2)","B) (3, −2)","C) (−3, −2)","D) (−3, 2)"],correct:'D',fb:"Regra: reflexão em relação ao eixo Oy → (x, y) → (−x, y).\nA ordenada mantém-se; a abcissa muda de sinal.\nK(3, 2) → K'(−3, 2). ✓"},
    ...
    {id:'q6-13',tema:5,visual:svgCoordGrid({line:{m:8,b:0,color:'#2a9e5c'},points:[{x:0.6,y:4.8,label:'(0,6 ; 4,8)'}],xMin:0,xMax:2,yMin:0,yMax:16}),enunciado:'Considera uma função de proporcionalidade direta f com f(0,6) = 4,8. A constante de proporcionalidade é:',opts:['A) 0,125','B) 0,8','C) 8','D) 80'],correct:'C',fb:'Na proporcionalidade direta f(x) = kx, logo k = f(x) / x.\nk = f(0,6) / 0,6 = 4,8 / 0,6 = 8.\nLogo k = 8. ✓'},
    ...
    {id:'q6-15',tema:5,visual:svgCoordGrid({line:{m:2/3,b:0,color:'#2a9e5c'},points:[{x:6,y:4,label:'(6,4)'}],xMin:0,xMax:8,yMin:0,yMax:8}),enunciado:'O ponto (6, 4) pertence ao gráfico de f(x) = kx. Qual é a expressão algébrica de f?',opts:['A) f(x) = (2/3)x','B) f(x) = (3/2)x','C) f(x) = 2x','D) f(x) = 24x'],correct:'A',fb:'Para encontrar k, usa o ponto (6, 4): f(6) = 4.\nk × 6 = 4\nk = 4/6 = 2/3.\nLogo f(x) = (2/3)x. ✓'},
    ...
    {id:'q6-21',tema:1,visual:svgCoordGrid({points:[{x:0,y:2,label:'A'},{x:3,y:1,label:'B'},{x:2,y:0,label:'C'},{x:-2,y:-3,label:'D'}],xMin:-5,xMax:5,yMin:-5,yMax:5}),enunciado:'As coordenadas dos pontos A, B, C e D representados no referencial são, respetivamente:',opts:['A) (2, 0), (3, 1), (0, 2), (−2, −3)','B) (0, 2), (1, 3), (2, 0), (−3, −2)','C) (2, 0), (1, 3), (0, 2), (−3, −2)','D) (0, 2), (3, 1), (2, 0), (−2, −3)'],correct:'D',fb:'Ler coordenadas: (abcissa, ordenada) = (horizontal, vertical).\nA: 0 unidades na horizontal, 2 na vertical → A(0, 2).\nB: 3 direita, 1 cima → B(3, 1).\nC: 2 direita, 0 → C(2, 0).\nD: 2 esquerda (−2), 3 baixo (−3) → D(−2, −3). ✓'},
    ...
    {id:'q6-25',tema:5,visual:'<div style="overflow-x:auto;margin:4px 0"><table style="border-collapse:collapse;font-size:12px;margin:0 auto"><thead><tr><th style="padding:4px 10px;background:#f0f0f0;border:1px solid #ccc">x</th><th style="padding:4px 10px;background:#e8f4ea;border:1px solid #ccc">A: y</th><th style="padding:4px 10px;background:#fde8e8;border:1px solid #ccc">B: y</th><th style="padding:4px 10px;background:#e8f4ea;border:1px solid #ccc">C: y</th><th style="padding:4px 10px;background:#e8f4ea;border:1px solid #ccc">D: y</th></tr></thead><tbody><tr><td style="padding:3px 10px;border:1px solid #ccc;text-align:center">2</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">4</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#fde8e8">28</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">6</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">0,5</td></tr><tr><td style="padding:3px 10px;border:1px solid #ccc;text-align:center">4</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">8</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#fde8e8">55</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">12</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">1</td></tr><tr><td style="padding:3px 10px;border:1px solid #ccc;text-align:center">6</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">12</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#fde8e8">84</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">18</td><td style="padding:3px 10px;border:1px solid #ccc;text-align:center;background:#e8f4ea">1,5</td></tr></tbody></table></div>',enunciado:'Qual das seguintes tabelas NÃO representa uma função de proporcionalidade direta?',opts:['A) x: 2, 4, 6 → y: 4, 8, 12','B) x: 7, 14, 21 → y: 28, 55, 84','C) x: 3, 6, 9 → y: 6, 12, 18','D) x: 1, 2, 3 → y: 0,5; 1; 1,5'],correct:'B',fb:'Na proporcionalidade direta, o quociente y/x deve ser constante (= k).\nA: 4/2=2, 8/4=2, 12/6=2 → k=2 constante ✓\nB: 28/7=4, mas 55/14≈3,93 ≠ 4 → quociente não constante → NÃO é proporcional direta ✗\nC: 6/3=2, 12/6=2, 18/9=2 → k=2 ✓\nD: 0,5/1=0,5, 1/2=0,5, 1,5/3=0,5 → k=0,5 ✓\nLogo a resposta é B. ✓'},
    // QA 36 PRISMA — Gráfico deslocação
    {id:'q6-26',tema:6,visual:svgPiecewiseGraph({segments:[{x1:0,y1:0,x2:3,y2:6},{x1:3,y1:6,x2:5,y2:6},{x1:5,y1:6,x2:8,y2:0}],xLabel:'Tempo (min)',yLabel:'Distância (km)',xMax:10,yMax:8}),enunciado:'O Pedro saiu de casa e foi ao supermercado, onde permaneceu algum tempo, regressando depois a casa. Qual dos seguintes gráficos melhor representa a sua deslocação em função do tempo?',opts:['A) Reta sempre crescente','B) Reta crescente, seguida de segmento horizontal, depois reta decrescente até 0','C) Reta sempre decrescente','D) Reta decrescente seguida de reta crescente'],correct:'B',fb:'Analisa cada fase:\nFase 1 — vai ao supermercado: distância aumenta → segmento CRESCENTE.\nFase 2 — está no supermercado: distância é constante → segmento HORIZONTAL.\nFase 3 — volta a casa: distância diminui até 0 → segmento DECRESCENTE até 0.\nO gráfico B representa exatamente estas três fases. ✓'},
    ...
    {id:'q6-33',tema:6,visual:svgPiecewiseGraph({segments:[{x1:0,y1:0,x2:20,y2:800}],xLabel:'Tempo (min)',yLabel:'Volume (L)',xMax:22,yMax:900}),enunciado:'Um depósito de água é enchido a uma taxa constante de 40 litros por minuto. Ao fim de 20 minutos o depósito está cheio. Qual é a capacidade do depósito?',opts:['A) 400 litros','B) 500 litros','C) 600 litros','D) 800 litros'],correct:'D',fb:'Taxa constante → proporcionalidade direta: V = taxa × tempo.\nV = 40 litros/min × 20 min = 800 litros.\nVerificação: 40 × 20 = 800 ✓'}
  ],
  minitestes:[
