...
var _port8Cards = {
  ...
  5: [
    { st: 1, tag: 'Figura', q: 'Comparação', a: 'Comparas duas coisas usando «como» ou «tal como». Ex.: «frio <b>como</b> gelo». Cria logo uma imagem na cabeça. Truque: procura o «como».' },
    { st: 1, tag: 'Figura', q: 'Metáfora', a: 'Como uma comparação, mas <b>sem</b> o «como» — dizes que uma coisa <b>é</b> a outra. Ex.: «os teus olhos são duas estrelas» (brilham, não são mesmo estrelas). Dá mais força e poesia.' },
    { st: 1, tag: 'Figura', q: 'Personificação', a: 'Dás vida humana a coisas que não são pessoas. Ex.: «a cidade <b>adormeceu</b>». Torna a cena mais viva, como se tudo tivesse sentimentos.' },
    { st: 1, tag: 'Figura', q: 'Hipérbole', a: 'É um exagero de propósito, para realçar. Ex.: «<b>morria</b> de rir», «esperei uma <b>eternidade</b>». Ninguém leva à letra — é só para dar ênfase.' },
    { st: 1, tag: 'Figura', q: 'Antítese', a: 'Juntas duas ideias <b>opostas</b> na mesma frase, para criar contraste. Ex.: «é fogo que <b>arde</b> sem se <b>ver</b>». O choque entre os contrários chama a atenção.' },
    { st: 1, tag: 'Figura', q: 'Enumeração', a: 'Fazes uma lista de coisas seguidas para reforçar uma ideia. Ex.: «Vim, vi, venci». Acumular dá sensação de quantidade ou rapidez.' },
    { st: 1, tag: 'Figura', q: 'Anáfora', a: 'Repetes a mesma palavra no <b>início</b> de versos ou frases seguidas. A repetição cria ritmo e fixa a ideia na cabeça de quem lê.' },
    { st: 2, tag: 'Figura', q: 'Aliteração e onomatopeia', a: '<b>Aliteração</b>: repetes o mesmo som em palavras seguidas, para dar musicalidade («o <b>r</b>ato <b>r</b>oeu a <b>r</b>olha»). <b>Onomatopeia</b>: palavra que imita um som (tique-taque, miau). As duas fazem-nos «ouvir» o texto.' },
    ...
    { st: 1, tag: 'Figura', q: 'Metonímia', a: 'Trocas uma palavra por outra que tem relação com ela. Ex.: «ler <b>Camões</b>» (= ler a obra dele); «beber um <b>copo</b>» (= beber o que está no copo). Dizes uma coisa para te referires a outra ligada.' },
    ...
    { st: 2, tag: 'Definição', q: 'O que são recursos expressivos e para que servem?', a: 'São maneiras criativas de usar as palavras para dar <b>força e emoção</b> ao texto. Quando os analisas, não chega dizer o nome — explica também o que ele <b>faz ali</b>.' },
    { st: 2, tag: 'Estratégia', q: 'Como explicar o efeito de uma figura?', a: 'Não basta dizer «é uma comparação». Diz o que ela <b>sugere</b> ali: «olhos como o mar» passa a ideia de algo <b>imenso e profundo</b>. O efeito é a parte que vale pontos.' },
...
function port8SwitchTab(tab, btn) {
  ...
  else if (tab === 'aprender') { if (typeof port8AprenderInit === 'function') port8AprenderInit(); }
  else if (tab === 'fichas') port8FichasBuildNav();
  else if (tab === 'progresso') port8RenderProgresso();
}

/* Aba APRENDER: catálogo de explicadores lúdicos de Português 8.º + abrir/voltar. */
var _port8AprenderWired = false;
function port8AprenderInit() {
  if (typeof APRENDER === 'undefined') {
    var c = document.getElementById('port8-aprender-cat');
    if (c) c.innerHTML = '<p style="color:var(--ink4);padding:2rem;text-align:center">Explicadores indisponíveis (motor não carregado).</p>';
    return;
  }
  var host = document.getElementById('port8-aprender-host');
  var cat = document.getElementById('port8-aprender-cat');
  if (!host || !cat) return;
  APRENDER.catalogo('port8-aprender-cat', function (id, t) {
    return /port8/.test(id);
  });
  host.style.display = 'none';
  if (_port8AprenderWired) return;
  _port8AprenderWired = true;
...
function port8gfGerar(formato) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_port8gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
...
var _port8Banco = {
  ...
  1: [
    ...
    { t: '2', tipo: 'mc', dif: 'facil', enun: 'Num poema, cada linha chama-se:', opcoes: ['verso', 'estrofe', 'rima', 'parágrafo'], resposta: 'verso', expl: 'Verso = cada linha do poema; o conjunto de versos forma uma estrofe.', tema: 'T3 · Poesia' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'A correspondência de sons no final dos versos chama-se:', opcoes: ['rima', 'métrica', 'estrofe', 'didascália'], resposta: 'rima', expl: 'A rima é a igualdade de sons a partir da última vogal tónica de versos diferentes.', tema: 'T3 · Poesia' }
  ],
  // ── Domínio 2 · Gramática ──
  2: [
  ...
  3: [
    ...
    { t: '2', tipo: 'mc', dif: 'facil', enun: 'Um texto que defende uma ideia e procura convencer o leitor é:', opcoes: ['um texto argumentativo (de opinião)', 'uma notícia', 'um conto', 'uma receita'], resposta: 'um texto argumentativo (de opinião)', expl: 'O texto argumentativo apresenta uma tese e defende-a com argumentos e exemplos.', tema: 'T2 · Tipologias' },
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'O sentido global de um texto constrói-se:', opcoes: ['relacionando as ideias de todos os parágrafos', 'lendo apenas a conclusão', 'olhando só para as imagens', 'contando as palavras'], resposta: 'relacionando as ideias de todos os parágrafos', expl: 'A compreensão global resulta de ligar a informação ao longo do texto, não de uma frase isolada.', tema: 'T1 · Compreensão' },
    { t: '3', tipo: 'mc', dif: 'facil', enun: 'A afirmação «Este é, sem dúvida, o melhor livro do ano.» é:', opcoes: ['uma opinião', 'um facto verificável', 'uma data', 'uma citação'], resposta: 'uma opinião', expl: 'Exprime uma avaliação pessoal («melhor»): é opinião, não um facto comprovável.', tema: 'T3 · Sentidos' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'A entrevista, como texto, caracteriza-se por:', opcoes: ['uma sequência de perguntas e respostas', 'narrar uma história inventada', 'dar instruções passo a passo', 'descrever uma paisagem'], resposta: 'uma sequência de perguntas e respostas', expl: 'A entrevista organiza-se em perguntas do entrevistador e respostas do entrevistado.', tema: 'T2 · Tipologias' },
    { t: '3', tipo: 'mc', dif: 'dificil', enun: 'Em «Ele é uma raposa nos negócios.», a expressão significa que ele é:', opcoes: ['astuto e esperto', 'um animal', 'desonesto e ladrão', 'tímido'], resposta: 'astuto e esperto', expl: 'Sentido conotativo: a raposa simboliza astúcia; aplicado à pessoa, indica esperteza.', tema: 'T3 · Sentidos' },
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'Para distinguir a ideia principal das ideias secundárias de um parágrafo, deves perguntar:', opcoes: ['«qual é a informação de que tudo o resto depende?»', '«qual é a frase mais comprida?»', '«qual é a primeira palavra?»', '«quantas vírgulas tem?»'], resposta: '«qual é a informação de que tudo o resto depende?»', expl: 'A ideia principal sustenta o parágrafo; as secundárias dão pormenores, exemplos ou explicações dela.', tema: 'T1 · Compreensão' }
  ],
  // ── Domínio 4 · Escrita ──
  4: [
    ...
    { t: '3', tipo: 'mc', dif: 'facil', enun: 'A primeira etapa para fazer um bom resumo é:', opcoes: ['identificar as ideias principais do texto', 'copiar o primeiro parágrafo', 'contar as linhas', 'escrever a tua opinião'], resposta: 'identificar as ideias principais do texto', expl: 'Resumir começa por sublinhar/anotar o essencial; só depois se reescreve de forma condensada.', tema: 'T3 · Resumo' },
    { t: '3', tipo: 'vf', dif: 'medio', enun: 'Num resumo deve manter-se a 1.ª pessoa e as opiniões do autor original.', resposta: 'F', expl: 'O resumo faz-se na 3.ª pessoa, de forma objetiva e sem juízos: condensa o conteúdo, não comenta.', tema: 'T3 · Resumo' },
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'Para situar a ação no tempo numa narrativa, usam-se expressões como:', opcoes: ['«ao amanhecer», «pouco depois», «naquela noite»', '«porque», «visto que»', '«ou seja», «isto é»', '«mas», «contudo»'], resposta: '«ao amanhecer», «pouco depois», «naquela noite»', expl: 'São marcadores temporais: organizam a sequência dos acontecimentos.', tema: 'T1 · Texto narrativo' },
    { t: '2', tipo: 'mc', dif: 'dificil', enun: 'Num texto de opinião, o argumento de autoridade consiste em:', opcoes: ['citar um especialista ou estudo para apoiar a tese', 'gritar mais alto', 'repetir a opinião', 'insultar quem discorda'], resposta: 'citar um especialista ou estudo para apoiar a tese', expl: 'Apoiar-se em quem percebe do assunto (cientistas, estudos) reforça a credibilidade do argumento.', tema: 'T2 · Texto de opinião' },
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'O parágrafo serve, na organização de um texto, para:', opcoes: ['separar e organizar blocos de ideias', 'aumentar o número de páginas', 'decorar a folha', 'marcar o título'], resposta: 'separar e organizar blocos de ideias', expl: 'Cada parágrafo desenvolve uma ideia ou etapa: ajuda o leitor a seguir o raciocínio.', tema: 'T1 · Texto narrativo' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'A introdução de um texto de opinião deve, sobretudo:', opcoes: ['apresentar o tema e a tese que vais defender', 'já dar a conclusão final', 'enumerar a bibliografia', 'pedir desculpa ao leitor'], resposta: 'apresentar o tema e a tese que vais defender', expl: 'A introdução situa o assunto e enuncia a posição; o desenvolvimento traz os argumentos.', tema: 'T2 · Texto de opinião' },
    { t: '3', tipo: 'mc', dif: 'dificil', enun: 'A redução de «O João, que é muito trabalhador, foi premiado.» num resumo pode ser:', opcoes: ['«O João, trabalhador, foi premiado.»', '«O João foi premiado e é trabalhador e foi premiado.»', '«Foi premiado o João que trabalha que é trabalhador.»', '«O João, que como toda a gente sabe é muito mas mesmo muito trabalhador, foi premiado.»'], resposta: '«O João, trabalhador, foi premiado.»', expl: 'Resumir é condensar: a oração relativa transforma-se num modificador curto, sem perder o essencial.', tema: 'T3 · Resumo' }
  ],
  ...
  5: [
    ...
    { t: '1', tipo: 'mc', dif: 'medio', enun: 'Em «vi mil vezes o pôr do sol», a figura de estilo é:', opcoes: ['hipérbole', 'metáfora', 'personificação', 'antítese'], resposta: 'hipérbole', expl: 'Exagero intencional («mil vezes») para enfatizar: hipérbole.', tema: 'T1 · Figuras' },
    { t: '1', tipo: 'mc', dif: 'dificil', enun: 'Em «era um silêncio ensurdecedor», a figura que junta ideias contraditórias chama-se:', opcoes: ['paradoxo (ou antítese)', 'comparação', 'onomatopeia', 'enumeração'], resposta: 'paradoxo (ou antítese)', expl: '«Silêncio» e «ensurdecedor» contradizem-se: aproximação de opostos para criar efeito expressivo.', tema: 'T1 · Figuras' },
    { t: '1', tipo: 'mc', dif: 'facil', enun: 'Em «forte como um touro», a figura de estilo é:', opcoes: ['comparação', 'metáfora', 'personificação', 'hipérbole'], resposta: 'comparação', expl: 'Usa o conector «como» a ligar os dois elementos: comparação.', tema: 'T1 · Figuras' },
    { t: '2', tipo: 'mc', dif: 'medio', enun: 'Em «Chegou, viu, venceu.», a sucessão de verbos sem conjunções é uma:', opcoes: ['enumeração (assíndeto)', 'anáfora', 'aliteração', 'onomatopeia'], resposta: 'enumeração (assíndeto)', expl: 'Elementos em série sem conectores (assíndeto) dão rapidez e força à expressão.', tema: 'T2 · Recursos' },
    { t: '2', tipo: 'vf', dif: 'facil', enun: 'A anáfora é a repetição de uma palavra ou expressão no início de versos ou frases seguidos.', resposta: 'V', expl: 'Ex.: «Tudo passa, tudo cansa, tudo se rende»: a repetição de «tudo» no início é anáfora.', tema: 'T2 · Recursos' },
    { t: '3', tipo: 'mc', dif: 'medio', enun: 'Um verso com dez sílabas métricas chama-se:', opcoes: ['decassílabo', 'redondilha maior', 'octossílabo', 'alexandrino'], resposta: 'decassílabo', expl: 'Decassílabo (10 sílabas) é o verso de Os Lusíadas; a redondilha maior tem 7 e o alexandrino 12.', tema: 'T3 · Versificação' },
    { t: '3', tipo: 'mc', dif: 'facil', enun: 'A estrofe de oito versos típica de Os Lusíadas chama-se:', opcoes: ['oitava', 'quadra', 'terceto', 'dístico'], resposta: 'oitava', expl: 'Os Lusíadas estão em oitava rima (estrofes de 8 versos decassílabos, esquema ABABABCC).', tema: 'T3 · Versificação' }
  ]
};
