/* ════════════════════════════════════════════════════════════════
   ...
   Sem módulos realojados: a prática é banco-driven (_port4Banco).
...
// Subtemas por domínio (4.º ano · iniciação à leitura e à escrita)
var _port4Subtemas = {
...
var _port4Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function port4SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('port4p-' + tab);
  ...
  if (tab === 'resumo') port4BuildResumoNav();
  else if (tab === 'exercicios') port4BuildPraticarNav();
  else if (tab === 'quiz') port4QuizBuildNav();
  else if (tab === 'flashcards') port4FcBuildNav();
  else if (tab === 'teste') port4TesteBuildNav();
  else if (tab === 'jogos') port4JogosInit();
  else if (tab === 'jogar') { if (typeof LudicoPort4 !== 'undefined') LudicoPort4.init('port4-ludico-menu', 'port4-ludico-jogo'); }
  else if (tab === 'fichas') port4FichasBuildNav();
  else if (tab === 'progresso') port4RenderProgresso();
}

// Abre um sub-modo de prática a partir dos cartões do menu Praticar.
function port4OpenPraticar(modo) { port4SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function port4BuildResumoNav() {
  var capRow = document.getElementById('port4-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _port4Sel['resumo'] || 1;
  var h = '';
  _port4CapMeta.forEach(function(m) {
    var color = _port4CapColors[m.n] || '#516860';
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="port4ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  port4ResumoShowSts(activeCap);
  port4RenderResumoInline();
}

function port4ResumoSelectCap(cap, btn) {
  _port4Sel['resumo'] = cap;
  _port4Sel['resumo-st'] = 0;
  var capRow = document.getElementById('port4-resumo-cap-row');
  if (capRow) {
  ...
  if (btn) {
    var color = _port4CapColors[cap] || '#516860';
    btn.classList.add('active');
    btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff';
  }
  port4ResumoShowSts(cap);
  port4RenderResumoInline();
}

function port4ResumoShowSts(cap) {
  var stRow = document.getElementById('port4-resumo-st-row');
  if (!stRow) return;
  var sts = _port4Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  // conta cards por subtema para esconder chips de subtemas sem teoria
  var _cc = (typeof _port4Cards !== 'undefined' && _port4Cards[cap]) ? _port4Cards[cap] : [];
  var _nSt = {}; _cc.forEach(function(c){ if (c.st) _nSt[c.st] = (_nSt[c.st]||0)+1; });
  var _stSel = _port4Sel['resumo-st'] || 0;
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_stSel === 0 ? ' active' : '') + '" data-st="0" onclick="port4ResumoSelectSt(this,' + cap + ',0)">Todos</button>';
  sts.forEach(function(st, i) {
    if (!_nSt[i + 1]) return; // subtema sem flashcards → não mostra o chip
    var isActive = _stSel === (i + 1);
    h += '<button class="resumo-st-btn' + (isActive ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="port4ResumoSelectSt(this,' + cap + ',' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function port4ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('port4-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _port4Sel['resumo'] = cap;
  _port4Sel['resumo-st'] = stIdx;
  port4RenderResumoInline();
}

function port4RenderResumoInline() {
  var cap = _port4Sel['resumo'] || 1;
  var dest = document.getElementById('port4-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _port4CapColors[cap] || '#516860';
    return c;
  })();
  ...
  var allCards = _port4Cards[cap] || [];

  // Filtrar por subtema, se selecionado
  var stIdx = _port4Sel['resumo-st'] || 0;
  ...
  _port4PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
// Português não tem geradores procedurais — a prática usa o _port4Banco.
// Só os caps com gerador são "praticáveis".
// Português não tem geradores procedurais (as perguntas são de análise/banco).
// _port4Gerador devolve sempre null → o hub usa apenas o _port4Banco.
function _port4Gerador(cap) {
  return null;
}
// Um domínio está "disponível" para prática se tiver questões no banco.
function _port4TemConteudo(cap) {
  return !!(_port4Banco[cap] && _port4Banco[cap].length);
}
// Nº de subtemas por domínio (espelha _port4Subtemas) — usado na seleção de prática.
var _port4TemasCount = { 1: 4, 2: 5, 3: 3, 4: 3, 5: 3 };

// Estado da prática
var _port4Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _port4TarefaAtiva=null, _port4TarefaResp={};

function _port4PratStorageKey(cap) { return 'edupt_port4_cap' + cap; }

function port4BuildPraticarNav() {
  var capRow = document.getElementById('port4-praticar-cap-row');
  if (!capRow) return;
  var activeCap = _port4Prat.cap || 1;
  var h = '';
  _port4CapMeta.forEach(function(m) {
    var hasGen = _port4TemConteudo(m.n);
    var color = _port4CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    var disabledStyle = hasGen ? '' : 'opacity:.45;cursor:not-allowed';
    var onclick = hasGen ? 'port4PraticarSelectCap(' + m.n + ',this)' : '';
  ...
  capRow.innerHTML = h;
  port4PraticarShowSts(activeCap);
  port4GerarExercicios(); // renderiza logo o domínio ativo (módulo ou banco)
}

function port4PraticarShowSts(cap) {
  var stRow = document.getElementById('port4-praticar-st-row');
  if (!stRow) return;
  var sts = _port4Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_port4Prat.st === 0 ? ' active' : '') + '" data-st="0" onclick="port4PraticarSelectSt(this,0)">Todos</button>';
  sts.forEach(function(st, i) {
    h += '<button class="resumo-st-btn' + (_port4Prat.st === (i + 1) ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="port4PraticarSelectSt(this,' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function port4PraticarSelectCap(cap, btn) {
  if (!_port4TemConteudo(cap)) return;
  _port4Prat.cap = cap;
  _port4Prat.st = 0;
  var capRow = document.getElementById('port4-praticar-cap-row');
  ...
  if (btn) { var color = _port4CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff'; }
  port4PraticarShowSts(cap);
  port4GerarExercicios();
}

function port4PraticarSelectSt(btn, stIdx) {
  var stRow = document.getElementById('port4-praticar-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _port4Prat.st = stIdx;
...
function port4PraticarSetNivel(nivel, btn) {
  _port4Prat.nivel = nivel;
  var grp = document.getElementById('port4-praticar-nivel');
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _port4PratModulo(cap, dest) {
  // O 7.º ano não tem módulos realojados: a prática é toda banco-driven.
  return false;
}

function port4GerarExercicios() {
  var dest = document.getElementById('port4-praticar-content');
  if (!dest) return;
  var cap = _port4Prat.cap, gen = _port4Gerador(cap);
  ...
  var lvlBar = document.getElementById('port4-praticar-levelbar');
  var stRow = document.getElementById('port4-praticar-st-row');
  if (_port4PratModulo(cap, dest)) {
    if (lvlBar) lvlBar.style.display = 'none';
    if (stRow) stRow.style.display = 'none';
    if (_port4TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_port4TarefaAtiva, 'Trabalho concluído');
    return;
  }
  if (lvlBar) lvlBar.style.display = '';

  // Sem gerador procedural nem banco → nada a mostrar
  if (!gen && !(_port4Banco[cap] && _port4Banco[cap].length)) { dest.innerHTML = ''; return; }

  // Que temas usar?
  var temas;
  if (_port4Prat.st > 0 && _port4SubtemaTemas[cap] && _port4SubtemaTemas[cap][_port4Prat.st]) {
    temas = _port4SubtemaTemas[cap][_port4Prat.st];
  } else {
    temas = [];
    for (var t = 1; t <= (_port4TemasCount[cap] || 1); t++) temas.push(String(t));
  }
  ...
  if (gen) {
    for (var i = 0; i < QTD; i++) {
      ...
      var ex = gen(tema, tipos[i % tipos.length], _port4Prat.nivel);
      if (ex) geradas.push(ex);
    }
  }
  var banco = (typeof _port4Banco !== 'undefined' && _port4Banco[cap]) ? _port4Banco[cap].filter(function (q) { return temas.indexOf(q.t) !== -1; }) : [];
  var exs;
  if (!gen) {
    ...
    exs = (typeof _mixBancoGeradas === 'function') ? _mixBancoGeradas(banco, geradas, QTD, 3, _port4Prat.nivel)
      : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  }
  _port4Prat.exs = exs;
  _port4Prat.answered = {};
  _port4Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="port4-prat-scorebar">'
    + '<div><div class="score-num" id="port4-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="port4-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="port4-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="port4GerarExercicios()">↺ Novas questões</button>'
    + (_port4TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="port4EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'port4CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="port4-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _port4CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _sn = (_port4Subtemas[cap] && _port4Prat.st > 0) ? (_port4Subtemas[cap][_port4Prat.st - 1] || '') : '';
    Atribuir.montar('port4-atribuir', { curso: 'port4', cursoNome: 'Português 7.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_port4Prat.st || ''), subNome: _sn, tipo: 'quiz', nivel: _port4Prat.nivel });
  }
}

function port4CheckEx(qid, tipo, val, btn) {
  if (_port4Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _port4Prat.answered[qid] = true;
  if (r.correct) _port4Prat.score.correct++;
  _port4Prat.score.total++; if(_port4TarefaAtiva){_port4TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('port4-prat-score'); if (se) se.textContent = _port4Prat.score.correct;
  var te = document.getElementById('port4-prat-total'); if (te) te.textContent = '/ ' + _port4Prat.score.total;
  var pe = document.getElementById('port4-prat-prog'); if (pe && _port4Prat.exs.length) pe.style.width = (_port4Prat.score.total / _port4Prat.exs.length * 100) + '%';
  port4SaveProgress(_port4Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function port4SaveProgress(cap, correct) {
  try {
    var key = _port4PratStorageKey(cap);
...
function _port4PM(cap, tipo, opts) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _port4BuildCapRow(rowId, activeCap, onclickName) {
...
function _port4SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
...
function _port4FichaSlice(banco, n, dif) {
...
function _port4BuildMcQuestion(cap) {
  var qs = _port4FichaBanco(cap).filter(function (q) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _port4Quiz = { cap: 1, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function port4QuizBuildNav() {
  if (!_port4TemConteudo(_port4Quiz.cap)) _port4Quiz.cap = 1;
  _port4BuildCapRow('port4-quiz-cap-row', _port4Quiz.cap, 'port4QuizSelectCap');
  port4QuizStart();
}

function port4QuizSelectCap(cap, btn) {
  if (!_port4TemConteudo(cap)) return;
  _port4SetActiveCapBtn('port4-quiz-cap-row', btn, cap);
  _port4Quiz.cap = cap;
  port4QuizStart();
}

function port4QuizStart() {
  _port4Quiz.lives = 3; _port4Quiz.streak = 0; _port4Quiz.maxStreak = 0;
  _port4Quiz.score = 0; _port4Quiz.total = 0; _port4Quiz.answered = false;
  port4QuizNext();
}

function port4QuizNext() {
  var app = document.getElementById('port4-quiz-app');
  if (!app) return;
  if (_port4Quiz.lives <= 0) { port4QuizGameOver(app); return; }
  var ex = _port4BuildMcQuestion(_port4Quiz.cap);
  ...
  _port4Quiz.current = ex; _port4Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _port4Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="port4qopt-' + idx + '" onclick="port4QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_port4Quiz.streak > 1 ? '🔥 ' + _port4Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _port4Quiz.score + ' / ' + _port4Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="port4-quiz-fb" style="min-height:2.5rem"></div>';
}

function port4QuizAnswer(idx) {
  if (_port4Quiz.answered) return;
  _port4Quiz.answered = true;
  var ex = _port4Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('port4qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('port4-quiz-fb');
  if (correct) {
    _port4Quiz.score++; _port4Quiz.streak++;
    if (_port4Quiz.streak > _port4Quiz.maxStreak) _port4Quiz.maxStreak = _port4Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _port4Quiz.lives--; _port4Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  port4SaveProgress(_port4Quiz.cap, correct);
  setTimeout(function(){ port4QuizNext(); }, 1400);
}

function port4QuizGameOver(app) {
  var pct = _port4Quiz.total > 0 ? Math.round(_port4Quiz.score / _port4Quiz.total * 100) : 0;
  _port4PM(_port4Quiz.cap, 'quiz', { pontuacao: _port4Quiz.score, total: _port4Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _port4Quiz.score + ' certas em ' + _port4Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_port4Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="port4QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _port4Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function port4FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_port4Cards[_port4Fc.cap] && _port4Cards[_port4Fc.cap].length)) {
    for (var c = 1; c <= _port4CapMeta.length; c++) { if (_port4Cards[c] && _port4Cards[c].length) { _port4Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('port4-fc-cap-row');
  if (row) {
    var h = '';
    _port4CapMeta.forEach(function(m) {
      var has = !!(_port4Cards[m.n] && _port4Cards[m.n].length);
      var color = _port4CapColors[m.n] || '#516860';
      var isActive = _port4Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('port4FcSelectCap(' + m.n + ',this)') : '';
  ...
  port4FcSelectCap(_port4Fc.cap, null);
}

function port4FcSelectCap(cap, btn) {
  if (!(_port4Cards[cap] && _port4Cards[cap].length)) return;
  if (btn) _port4SetActiveCapBtn('port4-fc-cap-row', btn, cap);
  _port4Fc.cap = cap;
  _port4Fc.cards = _port4Cards[cap].slice();
  _port4Fc.idx = 0; _port4Fc.flipped = false;
  _port4PM(cap, 'flashcard');
  port4FcRender();
}

function port4FcRender() {
  var app = document.getElementById('port4-fc-app');
  if (!app) return;
  var cards = _port4Fc.cards;
  ...
  var card = cards[_port4Fc.idx];
  var color = _port4CapColors[_port4Fc.cap] || '#516860';
  var pct = Math.round((_port4Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_port4Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="port4FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_port4Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_port4Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="port4FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="port4FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="port4FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function port4FcFlip() { _port4Fc.flipped = !_port4Fc.flipped; port4FcRender(); }
function port4FcNext() { _port4Fc.idx = (_port4Fc.idx + 1) % (_port4Fc.cards.length || 1); _port4Fc.flipped = false; port4FcRender(); }
function port4FcPrev() { _port4Fc.idx = (_port4Fc.idx - 1 + (_port4Fc.cards.length || 1)) % (_port4Fc.cards.length || 1); _port4Fc.flipped = false; port4FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _port4Teste = { cap: 1, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function port4TesteBuildNav() {
  if (!_port4TemConteudo(_port4Teste.cap)) _port4Teste.cap = 1;
  _port4BuildCapRow('port4-teste-cap-row', _port4Teste.cap, 'port4TesteSelectCap');
  port4TesteRenderConfig();
}

function port4TesteSelectCap(cap, btn) {
  if (!_port4TemConteudo(cap)) return;
  _port4SetActiveCapBtn('port4-teste-cap-row', btn, cap);
  _port4Teste.cap = cap;
  port4TesteRenderConfig();
}

function port4TesteRenderConfig() {
  var app = document.getElementById('port4-teste-app');
  if (!app) return;
  if (_port4Teste.timer) { clearInterval(_port4Teste.timer); _port4Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="port4-teste-nivel">' +
            '<button class="gen-level-btn' + (_port4Teste.nivel==='facil'?' active':'') + '" onclick="port4TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_port4Teste.nivel==='medio'?' active':'') + '" onclick="port4TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_port4Teste.nivel==='dificil'?' active':'') + '" onclick="port4TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="port4-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="port4-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="port4TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function port4TesteSetNivel(nivel, btn) {
  _port4Teste.nivel = nivel;
  var grp = document.getElementById('port4-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function port4TesteStart() {
  var gen = _port4Gerador(_port4Teste.cap);
  var qtdEl = document.getElementById('port4-teste-qtd'), tempoEl = document.getElementById('port4-teste-tempo');
  _port4Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _port4Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var exs = [];
  if (gen) {
    var nTemas = _port4TemasCount[_port4Teste.cap] || 1;
    var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
    for (var i = 0; i < _port4Teste.qtd; i++) {
      var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _port4Teste.nivel);
      if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
    }
  } else {
    // PT: teste sai do banco alargado do domínio
    exs = _port4FichaSlice(_port4FichaBanco(_port4Teste.cap), _port4Teste.qtd, _port4Teste.nivel);
  }
  if (!exs.length) return;
  _port4Teste.exs = exs; _port4Teste.answered = {}; _port4Teste.score = { correct: 0, total: 0 };
  _port4Teste.restante = _port4Teste.tempo;

  var app = document.getElementById('port4-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'port4TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="port4-teste-timer">' + _port4FmtTime(_port4Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="port4-teste-answered">0 / ' + _port4Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="port4TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="port4-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _port4Teste.timer = setInterval(function() {
    _port4Teste.restante--;
    var t = document.getElementById('port4-teste-timer');
    if (t) t.textContent = _port4FmtTime(_port4Teste.restante);
    if (_port4Teste.restante <= 0) port4TesteFinish();
  }, 1000);
}

function _port4FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function port4TesteCheck(qid, tipo, val, btn) {
  if (_port4Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _port4Teste.answered[qid] = true;
  if (r.correct) _port4Teste.score.correct++;
  _port4Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('port4-teste-answered');
  if (a) a.textContent = _port4Teste.score.total + ' / ' + _port4Teste.qtd;
  port4SaveProgress(_port4Teste.cap, r.correct);
  if (_port4Teste.score.total >= _port4Teste.qtd) setTimeout(port4TesteFinish, 600);
}

function port4TesteFinish() {
  if (_port4Teste.timer) { clearInterval(_port4Teste.timer); _port4Teste.timer = null; }
  var app = document.getElementById('port4-teste-app');
  if (!app) return;
  var total = _port4Teste.qtd;
  var correct = _port4Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _port4PM(_port4Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="port4TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function port4JogosInit() {
  (function(){ var pj=document.getElementById('port4p-jogos'); if(pj && !document.getElementById('port4-jogos-atr')){ var d=document.createElement('div'); d.id='port4-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('port4-jogos-atr',{curso:'port4',cursoNome:'Português 7.º',tipo:'jogo',nivel:'',caps:_port4CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  if (typeof ptJogosRender === 'function') {
    if (typeof ptJogosConfig === 'function') {
      ptJogosConfig({
        appId: 'port4-jogos-app',
        fonte: function () {
          var out = [];
          [1, 2, 3, 4, 5].forEach(function (c) { out = out.concat(_port4FichaBanco(c)); });
          return out;
        },
        forca: PT7_FORCA
      });
    ...
    var app = document.getElementById('port4-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_port4_capN (gravado por port4SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _port4ProgGetCaps() {
  var out = [];
  var nCaps = _port4CapMeta.length;
  for (var cap = 1; cap <= nCaps; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_port4_cap' + cap) || '{}');
...
function port4RenderProgresso() {
  var el = document.getElementById('port4-progresso-content');
  if (!el) return;
  var caps = _port4ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="port4ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m9c1-base);border:1.5px solid var(--m9c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="port4ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  caps.forEach(function(c) {
    var m = _port4CapMeta[c.cap - 1];
    var color = _port4CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _port4CapMeta[pior.cap - 1];
      var colorp = _port4CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="port4TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="port4ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function port4TreinarCap(cap) {
  _port4Prat.cap = cap;
  _port4Prat.st = 0;
  port4SwitchTab('exercicios', null);
}

function port4ProgReset() {
  ...
  for (var cap = 1; cap <= _port4CapMeta.length; cap++) { try { localStorage.removeItem('edupt_port4_cap' + cap); } catch (e) {} }
...
function port4ProgDownloadPDF() {
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-port4.pdf');
...
function _port4gfStsSel(cap) {
  var sel = _port4gf.sts[cap];
  if (!sel) return null;
  var out = [];
...
function _port4gfTemasSel(cap) {
  var sts = _port4gfStsSel(cap);
  if (!sts) return null;
  var mapa = _port4SubtemaTemas[cap] || {};
  var temas = [];
...
function port4FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('port4-fichas-atr','port4AtribuirFicha');
  var el = document.getElementById('port4-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com conteúdo
  var temAlgum = false;
  for (var k in _port4gf.caps) { if (_port4gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _port4CapMeta.forEach(function(m) {
    var hasGen = _port4TemConteudo(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _port4gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_port4gf.caps[m.n];
    var color = _port4CapColors[m.n] || '#516860';
    var style = sel ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    h += '<button class="gf-cap-btn' + (sel ? ' active' : '') + '" data-cap="' + m.n + '" onclick="port4gfToggleCap(' + m.n + ',this)" style="' + style + '">' + m.icon + ' ' + m.label + '</button>';
    // tópicos do capítulo selecionado
    if (sel) {
      var sts = _port4Subtemas[m.n] || [];
      if (sts.length) {
        var escolhidos = _port4gf.sts[m.n] || {};
        var algumSt = _port4gfStsSel(m.n) !== null;
        ...
        h += '<button onclick="port4gfToggleSt(' + m.n + ',0,this)" style="' + _port4gfStStyle(!algumSt, color) + '">Todos</button>';
        sts.forEach(function (st, i) {
          var on = !!escolhidos[i + 1];
          h += '<button onclick="port4gfToggleSt(' + m.n + ',' + (i + 1) + ',this)" style="' + _port4gfStStyle(on, color) + '">' + st + '</button>';
        });
        h += '</div>';
      }
    }
...
function _port4gfStStyle(on, color) {
...
function port4gfToggleCap(cap, btn) {
  _port4gf.caps[cap] = !_port4gf.caps[cap];
  if (!_port4gf.caps[cap]) delete _port4gf.sts[cap]; // desmarcar cap limpa os tópicos
  port4FichasBuildNav(); // re-render para mostrar/esconder a linha de tópicos
}

// idx 0 = «Todos» (limpa a seleção); idx 1..n alterna o tópico.
function port4gfToggleSt(cap, idx, btn) {
  if (idx === 0) {
    delete _port4gf.sts[cap];
  } else {
    if (!_port4gf.sts[cap]) _port4gf.sts[cap] = {};
    _port4gf.sts[cap][idx] = !_port4gf.sts[cap][idx];
    if (_port4gfStsSel(cap) === null) delete _port4gf.sts[cap]; // tudo desmarcado = todos
  }
  port4FichasBuildNav();
}

function port4gfToggleType(btn) {
  var t = btn.getAttribute('data-type');
  _port4gf.tipos[t] = !_port4gf.tipos[t];
  btn.classList.toggle('active', _port4gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _port4gf.tipos[t] ? '✓' : '';
}

function port4gfSetDif(btn) {
  _port4gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('port4-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function port4gfSetQty(btn) {
  _port4gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('port4-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _port4gfExBloco(exs, startNum) {
  var h = '';
...
function _port4gfGenExs(cap, n) {
  var gen = _port4Gerador(cap);
  if (!gen) {
    ...
    var pool = _port4FichaBanco(cap);
    var temasSel = _port4gfTemasSel(cap);
    if (temasSel) {
      var filtrado = pool.filter(function (q) { return temasSel.indexOf(q.t) !== -1; });
      if (filtrado.length) pool = filtrado;
    }
    return _port4FichaSlice(pool, n, _port4gf.dif);
  }
  var nTemas = _port4TemasCount[cap] || 1;
  ...
  for (var i = 0, tent = 0; geradas.length < n && tent < n * 6; tent++) {
    var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _port4gf.dif);
    i++;
    if (!ex) continue;
  ...
  var banco = (typeof _port4Banco !== 'undefined' && _port4Banco[cap]) ? _port4Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _port4gf.dif);
  }
  return geradas;
}

function port4gfGerar(formato) {
  // capítulos selecionados (conta o banco alargado — em PT não há geradores)
  var capsSel = [];
  _port4CapMeta.forEach(function(m) { if (_port4gf.caps[m.n] && (_port4Gerador(m.n) || _port4FichaBanco(m.n).length)) capsSel.push(m.n); });
  var status = document.getElementById('port4-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _port4gf.tipos.resumo || _port4gf.tipos.exercicios || _port4gf.tipos.teste || _port4gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_port4gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _port4PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _port4CapMeta[cap - 1];
    var color = _port4CapColors[cap] || '#36527a';
    ...
    if (_port4gf.tipos.resumo) {
      var cards = _port4Cards[cap] || [];
      var stsR = _port4gfStsSel(cap);
      if (stsR && cards.length) {
        var palavras = [];
        stsR.forEach(function (i) {
          String((_port4Subtemas[cap] || [])[i - 1] || '').toLowerCase().split(/\s+/).forEach(function (w) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_port4gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) {
    var nome = _port4CapMeta[c - 1].label;
    var sts = _port4gfStsSel(c);
    if (sts) {
      var nomes = sts.map(function (i) { return (_port4Subtemas[c] || [])[i - 1] || ''; }).filter(function (x) { return x; });
  ...
  var _temasNomes = capsSel.map(function(c){ return _port4CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('9', 'portugues', _temasNomes, formato) : ('ficha-port4.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _port4Init);
else _port4Init();
...
_port4Banco[2] = _port4Banco[2].concat([
...
_port4Banco[1] = _port4Banco[1].concat([
...
_port4Banco[3] = _port4Banco[3].concat([
  // F1-F2 · Publicidade
...
/* atribuir: deep-link port4 */
function _port4DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_port4gf){ _port4gf.caps={}; cs.forEach(function(n){ _port4gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _port4gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ port4SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_port4Prat) _port4Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ port4SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_port4TarefaAtiva=p.get('tarefa');_port4TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _port4Prat.cap=cap; _port4Prat.st=st; _port4Prat.nivel=nivel; setTimeout(function(){ port4SwitchTab('exercicios',null); if(typeof port4GerarExercicios==='function') port4GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_port4DeepLinkAuto,300);});else setTimeout(_port4DeepLinkAuto,300);

function port4AtribuirFicha(){
  var caps=[]; _port4CapMeta.forEach(function(m){ if(_port4gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('port4-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_port4CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'port4', cursoNome:'Português 7.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_port4gf.dif };
}

function port4EntregarTarefa(){
  if(!_port4TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_port4TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_port4TarefaResp[qid]; if(ok)certas++; var ex=_port4Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_port4TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _port4TarefaAtiva=null; _port4TarefaResp={}; port4GerarExercicios();
