/* ════════════════════════════════════════════════════════════════
   ...
   Sem módulos realojados: a prática é banco-driven (_em4Banco).
...
// Subtemas por domínio (4.º ano · Estudo do Meio)
var _em4Subtemas = {
...
var _em4Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function em4SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('em4p-' + tab);
  ...
  if (titles[tab]) document.title = 'Estudo do Meio 4.º · ' + titles[tab] + ' · 3ponto14';

  if (tab === 'resumo') em4BuildResumoNav();
  else if (tab === 'exercicios') em4BuildPraticarNav();
  else if (tab === 'quiz') em4QuizBuildNav();
  else if (tab === 'flashcards') em4FcBuildNav();
  else if (tab === 'teste') em4TesteBuildNav();
  else if (tab === 'jogos') em4JogosInit();
  else if (tab === 'jogar') { if (typeof LudicoEm4 !== 'undefined') LudicoEm4.init('em4-ludico-menu', 'em4-ludico-jogo'); }
  else if (tab === 'fichas') em4FichasBuildNav();
  else if (tab === 'progresso') em4RenderProgresso();
}

// Abre um sub-modo de prática a partir dos cartões do menu Praticar.
function em4OpenPraticar(modo) { em4SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function em4BuildResumoNav() {
  var capRow = document.getElementById('em4-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _em4Sel['resumo'] || 1;
  var h = '';
  _em4CapMeta.forEach(function(m) {
    var color = _em4CapColors[m.n] || '#516860';
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="em4ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  em4ResumoShowSts(activeCap);
  em4RenderResumoInline();
}

function em4ResumoSelectCap(cap, btn) {
  _em4Sel['resumo'] = cap;
  _em4Sel['resumo-st'] = 0;
  var capRow = document.getElementById('em4-resumo-cap-row');
  if (capRow) {
  ...
  if (btn) {
    var color = _em4CapColors[cap] || '#516860';
    btn.classList.add('active');
    btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff';
  }
  em4ResumoShowSts(cap);
  em4RenderResumoInline();
}

function em4ResumoShowSts(cap) {
  var stRow = document.getElementById('em4-resumo-st-row');
  if (!stRow) return;
  var sts = _em4Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  // conta cards por subtema para esconder chips de subtemas sem teoria
  var _cc = (typeof _em4Cards !== 'undefined' && _em4Cards[cap]) ? _em4Cards[cap] : [];
  var _nSt = {}; _cc.forEach(function(c){ if (c.st) _nSt[c.st] = (_nSt[c.st]||0)+1; });
  var _stSel = _em4Sel['resumo-st'] || 0;
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_stSel === 0 ? ' active' : '') + '" data-st="0" onclick="em4ResumoSelectSt(this,' + cap + ',0)">Todos</button>';
  sts.forEach(function(st, i) {
    if (!_nSt[i + 1]) return; // subtema sem flashcards → não mostra o chip
    var isActive = _stSel === (i + 1);
    h += '<button class="resumo-st-btn' + (isActive ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="em4ResumoSelectSt(this,' + cap + ',' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function em4ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('em4-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _em4Sel['resumo'] = cap;
  _em4Sel['resumo-st'] = stIdx;
  em4RenderResumoInline();
}

function em4RenderResumoInline() {
  var cap = _em4Sel['resumo'] || 1;
  var dest = document.getElementById('em4-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _em4CapColors[cap] || '#516860';
    return c;
  })();
  ...
  var allCards = _em4Cards[cap] || [];

  // Filtrar por subtema, se selecionado
  var stIdx = _em4Sel['resumo-st'] || 0;
  ...
  _em4PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
// Português não tem geradores procedurais — a prática usa o _em4Banco.
// Só os caps com gerador são "praticáveis".
// Português não tem geradores procedurais (as perguntas são de análise/banco).
// _em4Gerador devolve sempre null → o hub usa apenas o _em4Banco.
function _em4Gerador(cap) {
  return null;
}
// Um domínio está "disponível" para prática se tiver questões no banco.
function _em4TemConteudo(cap) {
  return !!(_em4Banco[cap] && _em4Banco[cap].length);
}
// Nº de subtemas por domínio (espelha _em4Subtemas) — usado na seleção de prática.
var _em4TemasCount = { 1: 4, 2: 5, 3: 3, 4: 3, 5: 3 };

// Estado da prática
var _em4Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _em4TarefaAtiva=null, _em4TarefaResp={};

function _em4PratStorageKey(cap) { return 'edupt_em4_cap' + cap; }

function em4BuildPraticarNav() {
  var capRow = document.getElementById('em4-praticar-cap-row');
  if (!capRow) return;
  var activeCap = _em4Prat.cap || 1;
  var h = '';
  _em4CapMeta.forEach(function(m) {
    var hasGen = _em4TemConteudo(m.n);
    var color = _em4CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    var disabledStyle = hasGen ? '' : 'opacity:.45;cursor:not-allowed';
    var onclick = hasGen ? 'em4PraticarSelectCap(' + m.n + ',this)' : '';
  ...
  capRow.innerHTML = h;
  em4PraticarShowSts(activeCap);
  em4GerarExercicios(); // renderiza logo o domínio ativo (módulo ou banco)
}

function em4PraticarShowSts(cap) {
  var stRow = document.getElementById('em4-praticar-st-row');
  if (!stRow) return;
  var sts = _em4Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_em4Prat.st === 0 ? ' active' : '') + '" data-st="0" onclick="em4PraticarSelectSt(this,0)">Todos</button>';
  sts.forEach(function(st, i) {
    h += '<button class="resumo-st-btn' + (_em4Prat.st === (i + 1) ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="em4PraticarSelectSt(this,' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function em4PraticarSelectCap(cap, btn) {
  if (!_em4TemConteudo(cap)) return;
  _em4Prat.cap = cap;
  _em4Prat.st = 0;
  var capRow = document.getElementById('em4-praticar-cap-row');
  ...
  if (btn) { var color = _em4CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff'; }
  em4PraticarShowSts(cap);
  em4GerarExercicios();
}

function em4PraticarSelectSt(btn, stIdx) {
  var stRow = document.getElementById('em4-praticar-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _em4Prat.st = stIdx;
...
function em4PraticarSetNivel(nivel, btn) {
  _em4Prat.nivel = nivel;
  var grp = document.getElementById('em4-praticar-nivel');
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _em4PratModulo(cap, dest) {
  // O 7.º ano não tem módulos realojados: a prática é toda banco-driven.
  return false;
}

function em4GerarExercicios() {
  var dest = document.getElementById('em4-praticar-content');
  if (!dest) return;
  var cap = _em4Prat.cap, gen = _em4Gerador(cap);
  ...
  var lvlBar = document.getElementById('em4-praticar-levelbar');
  var stRow = document.getElementById('em4-praticar-st-row');
  if (_em4PratModulo(cap, dest)) {
    if (lvlBar) lvlBar.style.display = 'none';
    if (stRow) stRow.style.display = 'none';
    if (_em4TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_em4TarefaAtiva, 'Trabalho concluído');
    return;
  }
  if (lvlBar) lvlBar.style.display = '';

  // Sem gerador procedural nem banco → nada a mostrar
  if (!gen && !(_em4Banco[cap] && _em4Banco[cap].length)) { dest.innerHTML = ''; return; }

  // Que temas usar?
  var temas;
  if (_em4Prat.st > 0 && _em4SubtemaTemas[cap] && _em4SubtemaTemas[cap][_em4Prat.st]) {
    temas = _em4SubtemaTemas[cap][_em4Prat.st];
  } else {
    temas = [];
    for (var t = 1; t <= (_em4TemasCount[cap] || 1); t++) temas.push(String(t));
  }
  ...
  if (gen) {
    for (var i = 0; i < QTD; i++) {
      ...
      var ex = gen(tema, tipos[i % tipos.length], _em4Prat.nivel);
      if (ex) geradas.push(ex);
    }
  }
  var banco = (typeof _em4Banco !== 'undefined' && _em4Banco[cap]) ? _em4Banco[cap].filter(function (q) { return temas.indexOf(q.t) !== -1; }) : [];
  var exs;
  if (!gen) {
    ...
    exs = (typeof _mixBancoGeradas === 'function') ? _mixBancoGeradas(banco, geradas, QTD, 3, _em4Prat.nivel)
      : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  }
  _em4Prat.exs = exs;
  _em4Prat.answered = {};
  _em4Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="em4-prat-scorebar">'
    + '<div><div class="score-num" id="em4-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="em4-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="em4-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="em4GerarExercicios()">↺ Novas questões</button>'
    + (_em4TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="em4EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'em4CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="em4-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _em4CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _sn = (_em4Subtemas[cap] && _em4Prat.st > 0) ? (_em4Subtemas[cap][_em4Prat.st - 1] || '') : '';
    Atribuir.montar('em4-atribuir', { curso: 'em4', cursoNome: 'Estudo do Meio 4.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_em4Prat.st || ''), subNome: _sn, tipo: 'quiz', nivel: _em4Prat.nivel });
  }
}

function em4CheckEx(qid, tipo, val, btn) {
  if (_em4Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _em4Prat.answered[qid] = true;
  if (r.correct) _em4Prat.score.correct++;
  _em4Prat.score.total++; if(_em4TarefaAtiva){_em4TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('em4-prat-score'); if (se) se.textContent = _em4Prat.score.correct;
  var te = document.getElementById('em4-prat-total'); if (te) te.textContent = '/ ' + _em4Prat.score.total;
  var pe = document.getElementById('em4-prat-prog'); if (pe && _em4Prat.exs.length) pe.style.width = (_em4Prat.score.total / _em4Prat.exs.length * 100) + '%';
  em4SaveProgress(_em4Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function em4SaveProgress(cap, correct) {
  try {
    var key = _em4PratStorageKey(cap);
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _em4BuildCapRow(rowId, activeCap, onclickName) {
...
function _em4SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
...
function _em4FichaSlice(banco, n, dif) {
...
function _em4BuildMcQuestion(cap) {
  var qs = _em4FichaBanco(cap).filter(function (q) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _em4Quiz = { cap: 1, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function em4QuizBuildNav() {
  if (!_em4TemConteudo(_em4Quiz.cap)) _em4Quiz.cap = 1;
  _em4BuildCapRow('em4-quiz-cap-row', _em4Quiz.cap, 'em4QuizSelectCap');
  em4QuizStart();
}

function em4QuizSelectCap(cap, btn) {
  if (!_em4TemConteudo(cap)) return;
  _em4SetActiveCapBtn('em4-quiz-cap-row', btn, cap);
  _em4Quiz.cap = cap;
  em4QuizStart();
}

function em4QuizStart() {
  _em4Quiz.lives = 3; _em4Quiz.streak = 0; _em4Quiz.maxStreak = 0;
  _em4Quiz.score = 0; _em4Quiz.total = 0; _em4Quiz.answered = false;
  em4QuizNext();
}

function em4QuizNext() {
  var app = document.getElementById('em4-quiz-app');
  if (!app) return;
  if (_em4Quiz.lives <= 0) { em4QuizGameOver(app); return; }
  var ex = _em4BuildMcQuestion(_em4Quiz.cap);
  ...
  _em4Quiz.current = ex; _em4Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _em4Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="em4qopt-' + idx + '" onclick="em4QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_em4Quiz.streak > 1 ? '🔥 ' + _em4Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _em4Quiz.score + ' / ' + _em4Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="em4-quiz-fb" style="min-height:2.5rem"></div>';
}

function em4QuizAnswer(idx) {
  if (_em4Quiz.answered) return;
  _em4Quiz.answered = true;
  var ex = _em4Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('em4qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('em4-quiz-fb');
  if (correct) {
    _em4Quiz.score++; _em4Quiz.streak++;
    if (_em4Quiz.streak > _em4Quiz.maxStreak) _em4Quiz.maxStreak = _em4Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _em4Quiz.lives--; _em4Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  em4SaveProgress(_em4Quiz.cap, correct);
  setTimeout(function(){ em4QuizNext(); }, 1400);
}

function em4QuizGameOver(app) {
  var pct = _em4Quiz.total > 0 ? Math.round(_em4Quiz.score / _em4Quiz.total * 100) : 0;
  _em4PM(_em4Quiz.cap, 'quiz', { pontuacao: _em4Quiz.score, total: _em4Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _em4Quiz.score + ' certas em ' + _em4Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_em4Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="em4QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _em4Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function em4FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_em4Cards[_em4Fc.cap] && _em4Cards[_em4Fc.cap].length)) {
    for (var c = 1; c <= _em4CapMeta.length; c++) { if (_em4Cards[c] && _em4Cards[c].length) { _em4Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('em4-fc-cap-row');
  if (row) {
    var h = '';
    _em4CapMeta.forEach(function(m) {
      var has = !!(_em4Cards[m.n] && _em4Cards[m.n].length);
      var color = _em4CapColors[m.n] || '#516860';
      var isActive = _em4Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('em4FcSelectCap(' + m.n + ',this)') : '';
  ...
  em4FcSelectCap(_em4Fc.cap, null);
}

function em4FcSelectCap(cap, btn) {
  if (!(_em4Cards[cap] && _em4Cards[cap].length)) return;
  if (btn) _em4SetActiveCapBtn('em4-fc-cap-row', btn, cap);
  _em4Fc.cap = cap;
  _em4Fc.cards = _em4Cards[cap].slice();
  _em4Fc.idx = 0; _em4Fc.flipped = false;
  _em4PM(cap, 'flashcard');
  em4FcRender();
}

function em4FcRender() {
  var app = document.getElementById('em4-fc-app');
  if (!app) return;
  var cards = _em4Fc.cards;
  ...
  var card = cards[_em4Fc.idx];
  var color = _em4CapColors[_em4Fc.cap] || '#516860';
  var pct = Math.round((_em4Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_em4Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="em4FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_em4Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_em4Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="em4FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="em4FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="em4FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function em4FcFlip() { _em4Fc.flipped = !_em4Fc.flipped; em4FcRender(); }
function em4FcNext() { _em4Fc.idx = (_em4Fc.idx + 1) % (_em4Fc.cards.length || 1); _em4Fc.flipped = false; em4FcRender(); }
function em4FcPrev() { _em4Fc.idx = (_em4Fc.idx - 1 + (_em4Fc.cards.length || 1)) % (_em4Fc.cards.length || 1); _em4Fc.flipped = false; em4FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _em4Teste = { cap: 1, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function em4TesteBuildNav() {
  if (!_em4TemConteudo(_em4Teste.cap)) _em4Teste.cap = 1;
  _em4BuildCapRow('em4-teste-cap-row', _em4Teste.cap, 'em4TesteSelectCap');
  em4TesteRenderConfig();
}

function em4TesteSelectCap(cap, btn) {
  if (!_em4TemConteudo(cap)) return;
  _em4SetActiveCapBtn('em4-teste-cap-row', btn, cap);
  _em4Teste.cap = cap;
  em4TesteRenderConfig();
}

function em4TesteRenderConfig() {
  var app = document.getElementById('em4-teste-app');
  if (!app) return;
  if (_em4Teste.timer) { clearInterval(_em4Teste.timer); _em4Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="em4-teste-nivel">' +
            '<button class="gen-level-btn' + (_em4Teste.nivel==='facil'?' active':'') + '" onclick="em4TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_em4Teste.nivel==='medio'?' active':'') + '" onclick="em4TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_em4Teste.nivel==='dificil'?' active':'') + '" onclick="em4TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="em4-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="em4-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="em4TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function em4TesteSetNivel(nivel, btn) {
  _em4Teste.nivel = nivel;
  var grp = document.getElementById('em4-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function em4TesteStart() {
  var gen = _em4Gerador(_em4Teste.cap);
  var qtdEl = document.getElementById('em4-teste-qtd'), tempoEl = document.getElementById('em4-teste-tempo');
  _em4Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _em4Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var exs = [];
  if (gen) {
    var nTemas = _em4TemasCount[_em4Teste.cap] || 1;
    var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
    for (var i = 0; i < _em4Teste.qtd; i++) {
      var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _em4Teste.nivel);
      if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
    }
  } else {
    // PT: teste sai do banco alargado do domínio
    exs = _em4FichaSlice(_em4FichaBanco(_em4Teste.cap), _em4Teste.qtd, _em4Teste.nivel);
  }
  if (!exs.length) return;
  _em4Teste.exs = exs; _em4Teste.answered = {}; _em4Teste.score = { correct: 0, total: 0 };
  _em4Teste.restante = _em4Teste.tempo;

  var app = document.getElementById('em4-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'em4TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="em4-teste-timer">' + _em4FmtTime(_em4Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="em4-teste-answered">0 / ' + _em4Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="em4TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="em4-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _em4Teste.timer = setInterval(function() {
    _em4Teste.restante--;
    var t = document.getElementById('em4-teste-timer');
    if (t) t.textContent = _em4FmtTime(_em4Teste.restante);
    if (_em4Teste.restante <= 0) em4TesteFinish();
  }, 1000);
}

function _em4FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function em4TesteCheck(qid, tipo, val, btn) {
  if (_em4Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _em4Teste.answered[qid] = true;
  if (r.correct) _em4Teste.score.correct++;
  _em4Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('em4-teste-answered');
  if (a) a.textContent = _em4Teste.score.total + ' / ' + _em4Teste.qtd;
  em4SaveProgress(_em4Teste.cap, r.correct);
  if (_em4Teste.score.total >= _em4Teste.qtd) setTimeout(em4TesteFinish, 600);
}

function em4TesteFinish() {
  if (_em4Teste.timer) { clearInterval(_em4Teste.timer); _em4Teste.timer = null; }
  var app = document.getElementById('em4-teste-app');
  if (!app) return;
  var total = _em4Teste.qtd;
  var correct = _em4Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _em4PM(_em4Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="em4TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function em4JogosInit() {
  (function(){ var pj=document.getElementById('em4p-jogos'); if(pj && !document.getElementById('em4-jogos-atr')){ var d=document.createElement('div'); d.id='em4-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('em4-jogos-atr',{curso:'em4',cursoNome:'Português 7.º',tipo:'jogo',nivel:'',caps:_em4CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  if (typeof ptJogosRender === 'function') {
    if (typeof ptJogosConfig === 'function') {
      ptJogosConfig({
        ...
        fonte: function () {
          var out = [];
          [1, 2, 3, 4, 5].forEach(function (c) { out = out.concat(_em4FichaBanco(c)); });
          return out;
        },
        forca: PT7_FORCA
      });
    ...
    var app = document.getElementById('em4-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_em4_capN (gravado por em4SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _em4ProgGetCaps() {
  var out = [];
  var nCaps = _em4CapMeta.length;
  for (var cap = 1; cap <= nCaps; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_em4_cap' + cap) || '{}');
...
function em4RenderProgresso() {
  var el = document.getElementById('em4-progresso-content');
  if (!el) return;
  var caps = _em4ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="em4ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m9c1-base);border:1.5px solid var(--m9c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="em4ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  caps.forEach(function(c) {
    var m = _em4CapMeta[c.cap - 1];
    var color = _em4CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _em4CapMeta[pior.cap - 1];
      var colorp = _em4CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="em4TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="em4ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function em4TreinarCap(cap) {
  _em4Prat.cap = cap;
  _em4Prat.st = 0;
  em4SwitchTab('exercicios', null);
}

function em4ProgReset() {
  ...
  for (var cap = 1; cap <= _em4CapMeta.length; cap++) { try { localStorage.removeItem('edupt_em4_cap' + cap); } catch (e) {} }
...
function em4ProgDownloadPDF() {
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-em4.pdf');
...
function _em4gfStsSel(cap) {
  var sel = _em4gf.sts[cap];
  if (!sel) return null;
  var out = [];
...
function _em4gfTemasSel(cap) {
  var sts = _em4gfStsSel(cap);
  if (!sts) return null;
  var mapa = _em4SubtemaTemas[cap] || {};
  var temas = [];
...
function em4FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('em4-fichas-atr','em4AtribuirFicha');
  var el = document.getElementById('em4-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com conteúdo
  var temAlgum = false;
  for (var k in _em4gf.caps) { if (_em4gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _em4CapMeta.forEach(function(m) {
    var hasGen = _em4TemConteudo(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _em4gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_em4gf.caps[m.n];
    var color = _em4CapColors[m.n] || '#516860';
    var style = sel ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    h += '<button class="gf-cap-btn' + (sel ? ' active' : '') + '" data-cap="' + m.n + '" onclick="em4gfToggleCap(' + m.n + ',this)" style="' + style + '">' + m.icon + ' ' + m.label + '</button>';
    // tópicos do capítulo selecionado
    if (sel) {
      var sts = _em4Subtemas[m.n] || [];
      if (sts.length) {
        var escolhidos = _em4gf.sts[m.n] || {};
        var algumSt = _em4gfStsSel(m.n) !== null;
        ...
        h += '<button onclick="em4gfToggleSt(' + m.n + ',0,this)" style="' + _em4gfStStyle(!algumSt, color) + '">Todos</button>';
        sts.forEach(function (st, i) {
          var on = !!escolhidos[i + 1];
          h += '<button onclick="em4gfToggleSt(' + m.n + ',' + (i + 1) + ',this)" style="' + _em4gfStStyle(on, color) + '">' + st + '</button>';
        });
        h += '</div>';
      }
    }
...
function _em4gfStStyle(on, color) {
...
function em4gfToggleCap(cap, btn) {
  _em4gf.caps[cap] = !_em4gf.caps[cap];
  if (!_em4gf.caps[cap]) delete _em4gf.sts[cap]; // desmarcar cap limpa os tópicos
  em4FichasBuildNav(); // re-render para mostrar/esconder a linha de tópicos
}

// idx 0 = «Todos» (limpa a seleção); idx 1..n alterna o tópico.
function em4gfToggleSt(cap, idx, btn) {
  if (idx === 0) {
    delete _em4gf.sts[cap];
  } else {
    if (!_em4gf.sts[cap]) _em4gf.sts[cap] = {};
    _em4gf.sts[cap][idx] = !_em4gf.sts[cap][idx];
    if (_em4gfStsSel(cap) === null) delete _em4gf.sts[cap]; // tudo desmarcado = todos
  }
  em4FichasBuildNav();
}

function em4gfToggleType(btn) {
  var t = btn.getAttribute('data-type');
  _em4gf.tipos[t] = !_em4gf.tipos[t];
  btn.classList.toggle('active', _em4gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _em4gf.tipos[t] ? '✓' : '';
}

function em4gfSetDif(btn) {
  _em4gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('em4-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function em4gfSetQty(btn) {
  _em4gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('em4-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _em4gfExBloco(exs, startNum) {
  var h = '';
...
function _em4gfGenExs(cap, n) {
  var gen = _em4Gerador(cap);
  if (!gen) {
    ...
    var pool = _em4FichaBanco(cap);
    var temasSel = _em4gfTemasSel(cap);
    if (temasSel) {
      var filtrado = pool.filter(function (q) { return temasSel.indexOf(q.t) !== -1; });
      if (filtrado.length) pool = filtrado;
    }
    return _em4FichaSlice(pool, n, _em4gf.dif);
  }
  var nTemas = _em4TemasCount[cap] || 1;
  ...
  for (var i = 0, tent = 0; geradas.length < n && tent < n * 6; tent++) {
    var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _em4gf.dif);
    i++;
    if (!ex) continue;
  ...
  var banco = (typeof _em4Banco !== 'undefined' && _em4Banco[cap]) ? _em4Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _em4gf.dif);
  }
  return geradas;
}

function em4gfGerar(formato) {
  // capítulos selecionados (conta o banco alargado — em PT não há geradores)
  var capsSel = [];
  _em4CapMeta.forEach(function(m) { if (_em4gf.caps[m.n] && (_em4Gerador(m.n) || _em4FichaBanco(m.n).length)) capsSel.push(m.n); });
  var status = document.getElementById('em4-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _em4gf.tipos.resumo || _em4gf.tipos.exercicios || _em4gf.tipos.teste || _em4gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_em4gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _em4PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _em4CapMeta[cap - 1];
    var color = _em4CapColors[cap] || '#36527a';
    ...
    if (_em4gf.tipos.resumo) {
      var cards = _em4Cards[cap] || [];
      var stsR = _em4gfStsSel(cap);
      if (stsR && cards.length) {
        var palavras = [];
        stsR.forEach(function (i) {
          String((_em4Subtemas[cap] || [])[i - 1] || '').toLowerCase().split(/\s+/).forEach(function (w) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_em4gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) {
    var nome = _em4CapMeta[c - 1].label;
    var sts = _em4gfStsSel(c);
    if (sts) {
      var nomes = sts.map(function (i) { return (_em4Subtemas[c] || [])[i - 1] || ''; }).filter(function (x) { return x; });
  ...
  var _temasNomes = capsSel.map(function(c){ return _em4CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('9', 'portugues', _temasNomes, formato) : ('ficha-em4.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _em4Init);
else _em4Init();
...
_em4Banco[2] = _em4Banco[2].concat([
...
_em4Banco[1] = _em4Banco[1].concat([
...
_em4Banco[3] = _em4Banco[3].concat([
  // F1-F2 · Publicidade
...
/* atribuir: deep-link em4 */
function _em4DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_em4gf){ _em4gf.caps={}; cs.forEach(function(n){ _em4gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _em4gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ em4SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_em4Prat) _em4Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ em4SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_em4TarefaAtiva=p.get('tarefa');_em4TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _em4Prat.cap=cap; _em4Prat.st=st; _em4Prat.nivel=nivel; setTimeout(function(){ em4SwitchTab('exercicios',null); if(typeof em4GerarExercicios==='function') em4GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_em4DeepLinkAuto,300);});else setTimeout(_em4DeepLinkAuto,300);

function em4AtribuirFicha(){
  var caps=[]; _em4CapMeta.forEach(function(m){ if(_em4gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('em4-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_em4CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'em4', cursoNome:'Português 7.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_em4gf.dif };
}

function em4EntregarTarefa(){
  if(!_em4TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_em4TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_em4TarefaResp[qid]; if(ok)certas++; var ex=_em4Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_em4TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _em4TarefaAtiva=null; _em4TarefaResp={}; em4GerarExercicios();
