/* ════════════════════════════════════════════════════════════════
   ...
   Sem módulos realojados: a prática é banco-driven (_port2Banco).
...
// Subtemas por domínio (2.º ano · iniciação à leitura e à escrita)
var _port2Subtemas = {
...
var _port2Sel = { resumo: 1, 'resumo-st': 0 };

// ═══ NAVEGAÇÃO DE TABS ═══
function port2SwitchTab(tab, btn) {
  ...
  var panel = document.getElementById('port2p-' + tab);
  ...
  if (tab === 'resumo') port2BuildResumoNav();
  else if (tab === 'exercicios') port2BuildPraticarNav();
  else if (tab === 'quiz') port2QuizBuildNav();
  else if (tab === 'flashcards') port2FcBuildNav();
  else if (tab === 'teste') port2TesteBuildNav();
  else if (tab === 'jogos') port2JogosInit();
  else if (tab === 'jogar') { if (typeof LudicoPort2 !== 'undefined') LudicoPort2.init('port2-ludico-menu', 'port2-ludico-jogo'); }
  else if (tab === 'fichas') port2FichasBuildNav();
  else if (tab === 'progresso') port2RenderProgresso();
}

// Abre um sub-modo de prática a partir dos cartões do menu Praticar.
function port2OpenPraticar(modo) { port2SwitchTab(modo, null); }

// ═══ TAB TEORIA ═══
function port2BuildResumoNav() {
  var capRow = document.getElementById('port2-resumo-cap-row');
  if (!capRow) return;
  var activeCap = _port2Sel['resumo'] || 1;
  var h = '';
  _port2CapMeta.forEach(function(m) {
    var color = _port2CapColors[m.n] || '#516860';
    ...
    h += '<button class="resumo-cap-btn' + (isActive ? ' active' : '') + '" data-cap="' + m.n + '" onclick="port2ResumoSelectCap(' + m.n + ',this)" style="' + activeStyle + '"' + hoverCode + '>'
       + '<span class="resumo-cap-icon">' + m.icon + '</span>' + m.label + '</button>';
  });
  capRow.innerHTML = h;
  port2ResumoShowSts(activeCap);
  port2RenderResumoInline();
}

function port2ResumoSelectCap(cap, btn) {
  _port2Sel['resumo'] = cap;
  _port2Sel['resumo-st'] = 0;
  var capRow = document.getElementById('port2-resumo-cap-row');
  if (capRow) {
  ...
  if (btn) {
    var color = _port2CapColors[cap] || '#516860';
    btn.classList.add('active');
    btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff';
  }
  port2ResumoShowSts(cap);
  port2RenderResumoInline();
}

function port2ResumoShowSts(cap) {
  var stRow = document.getElementById('port2-resumo-st-row');
  if (!stRow) return;
  var sts = _port2Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  // conta cards por subtema para esconder chips de subtemas sem teoria
  var _cc = (typeof _port2Cards !== 'undefined' && _port2Cards[cap]) ? _port2Cards[cap] : [];
  var _nSt = {}; _cc.forEach(function(c){ if (c.st) _nSt[c.st] = (_nSt[c.st]||0)+1; });
  var _stSel = _port2Sel['resumo-st'] || 0;
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_stSel === 0 ? ' active' : '') + '" data-st="0" onclick="port2ResumoSelectSt(this,' + cap + ',0)">Todos</button>';
  sts.forEach(function(st, i) {
    if (!_nSt[i + 1]) return; // subtema sem flashcards → não mostra o chip
    var isActive = _stSel === (i + 1);
    h += '<button class="resumo-st-btn' + (isActive ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="port2ResumoSelectSt(this,' + cap + ',' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function port2ResumoSelectSt(btn, cap, stIdx) {
  var stRow = document.getElementById('port2-resumo-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _port2Sel['resumo'] = cap;
  _port2Sel['resumo-st'] = stIdx;
  port2RenderResumoInline();
}

function port2RenderResumoInline() {
  var cap = _port2Sel['resumo'] || 1;
  var dest = document.getElementById('port2-resumo-content');
  if (!dest) return;

  var color = (function(){
    var c = _port2CapColors[cap] || '#516860';
    return c;
  })();
  ...
  var allCards = _port2Cards[cap] || [];

  // Filtrar por subtema, se selecionado
  var stIdx = _port2Sel['resumo-st'] || 0;
  ...
  _port2PM(cap, 'teoria'); // estudou a teoria deste capítulo
...
// Português não tem geradores procedurais — a prática usa o _port2Banco.
// Só os caps com gerador são "praticáveis".
// Português não tem geradores procedurais (as perguntas são de análise/banco).
// _port2Gerador devolve sempre null → o hub usa apenas o _port2Banco.
function _port2Gerador(cap) {
  return null;
}
// Um domínio está "disponível" para prática se tiver questões no banco.
function _port2TemConteudo(cap) {
  return !!(_port2Banco[cap] && _port2Banco[cap].length);
}
// Nº de subtemas por domínio (espelha _port2Subtemas) — usado na seleção de prática.
var _port2TemasCount = { 1: 4, 2: 5, 3: 3, 4: 3, 5: 3 };

// Estado da prática
var _port2Prat = { cap: 1, st: 0, nivel: 'medio', score: { correct: 0, total: 0 }, answered: {}, exs: [] };
var _port2TarefaAtiva=null, _port2TarefaResp={};

function _port2PratStorageKey(cap) { return 'edupt_port2_cap' + cap; }

function port2BuildPraticarNav() {
  var capRow = document.getElementById('port2-praticar-cap-row');
  if (!capRow) return;
  var activeCap = _port2Prat.cap || 1;
  var h = '';
  _port2CapMeta.forEach(function(m) {
    var hasGen = _port2TemConteudo(m.n);
    var color = _port2CapColors[m.n] || '#516860';
    var isActive = activeCap === m.n;
    var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    var disabledStyle = hasGen ? '' : 'opacity:.45;cursor:not-allowed';
    var onclick = hasGen ? 'port2PraticarSelectCap(' + m.n + ',this)' : '';
  ...
  capRow.innerHTML = h;
  port2PraticarShowSts(activeCap);
  port2GerarExercicios(); // renderiza logo o domínio ativo (módulo ou banco)
}

function port2PraticarShowSts(cap) {
  var stRow = document.getElementById('port2-praticar-st-row');
  if (!stRow) return;
  var sts = _port2Subtemas[cap] || [];
  if (!sts.length) { stRow.style.display = 'none'; return; }
  var h = '<div class="resumo-st-label">Subtema</div>';
  h += '<button class="resumo-st-btn' + (_port2Prat.st === 0 ? ' active' : '') + '" data-st="0" onclick="port2PraticarSelectSt(this,0)">Todos</button>';
  sts.forEach(function(st, i) {
    h += '<button class="resumo-st-btn' + (_port2Prat.st === (i + 1) ? ' active' : '') + '" data-st="' + (i + 1) + '" onclick="port2PraticarSelectSt(this,' + (i + 1) + ')">' + st + '</button>';
  });
  stRow.innerHTML = h;
  stRow.style.display = 'flex';
}

function port2PraticarSelectCap(cap, btn) {
  if (!_port2TemConteudo(cap)) return;
  _port2Prat.cap = cap;
  _port2Prat.st = 0;
  var capRow = document.getElementById('port2-praticar-cap-row');
  ...
  if (btn) { var color = _port2CapColors[cap] || '#516860'; btn.classList.add('active'); btn.style.background = color; btn.style.borderColor = color; btn.style.color = '#fff'; }
  port2PraticarShowSts(cap);
  port2GerarExercicios();
}

function port2PraticarSelectSt(btn, stIdx) {
  var stRow = document.getElementById('port2-praticar-st-row');
  if (stRow) stRow.querySelectorAll('.resumo-st-btn').forEach(function(b) { b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
  _port2Prat.st = stIdx;
...
function port2PraticarSetNivel(nivel, btn) {
  _port2Prat.nivel = nivel;
  var grp = document.getElementById('port2-praticar-nivel');
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _port2PratModulo(cap, dest) {
  // O 7.º ano não tem módulos realojados: a prática é toda banco-driven.
  return false;
}

function port2GerarExercicios() {
  var dest = document.getElementById('port2-praticar-content');
  if (!dest) return;
  var cap = _port2Prat.cap, gen = _port2Gerador(cap);
  ...
  var lvlBar = document.getElementById('port2-praticar-levelbar');
  var stRow = document.getElementById('port2-praticar-st-row');
  if (_port2PratModulo(cap, dest)) {
    if (lvlBar) lvlBar.style.display = 'none';
    if (stRow) stRow.style.display = 'none';
    if (_port2TarefaAtiva && typeof tarefaEntregaBar === 'function') tarefaEntregaBar(_port2TarefaAtiva, 'Trabalho concluído');
    return;
  }
  if (lvlBar) lvlBar.style.display = '';

  // Sem gerador procedural nem banco → nada a mostrar
  if (!gen && !(_port2Banco[cap] && _port2Banco[cap].length)) { dest.innerHTML = ''; return; }

  // Que temas usar?
  var temas;
  if (_port2Prat.st > 0 && _port2SubtemaTemas[cap] && _port2SubtemaTemas[cap][_port2Prat.st]) {
    temas = _port2SubtemaTemas[cap][_port2Prat.st];
  } else {
    temas = [];
    for (var t = 1; t <= (_port2TemasCount[cap] || 1); t++) temas.push(String(t));
  }
  ...
  if (gen) {
    for (var i = 0; i < QTD; i++) {
      ...
      var ex = gen(tema, tipos[i % tipos.length], _port2Prat.nivel);
      if (ex) geradas.push(ex);
    }
  }
  var banco = (typeof _port2Banco !== 'undefined' && _port2Banco[cap]) ? _port2Banco[cap].filter(function (q) { return temas.indexOf(q.t) !== -1; }) : [];
  var exs;
  if (!gen) {
    ...
    exs = (typeof _mixBancoGeradas === 'function') ? _mixBancoGeradas(banco, geradas, QTD, 3, _port2Prat.nivel)
      : geradas.map(function (e, idx) { return Object.assign({}, e, { num: idx + 1 }); });
  }
  _port2Prat.exs = exs;
  _port2Prat.answered = {};
  _port2Prat.score = { correct: 0, total: 0 };

  // Score bar + questões (reutiliza motor genérico do chapter-engine)
  var scoreBar = '<div class="score-bar" id="port2-prat-scorebar">'
    + '<div><div class="score-num" id="port2-prat-score">0</div><div class="score-label">pontos</div></div>'
    + '<div><div class="score-num" id="port2-prat-total">/ 0</div><div class="score-label">respondidas</div></div>'
    + '<div class="progress-track"><div class="progress-fill" id="port2-prat-prog" style="width:0%"></div></div>'
    + '<button class="btn btn-ghost ml-auto" onclick="port2GerarExercicios()">↺ Novas questões</button>'
    + (_port2TarefaAtiva ? '<button class="btn" style="background:#2e7d52;color:#fff;border:none;border-radius:999px;padding:6px 14px;font-weight:800;cursor:pointer;margin-left:.4rem" onclick="port2EntregarTarefa()">\u2713 Terminar e entregar</button>' : '')
    + '</div>';
  var quizHTML = (typeof _capBuildQuizHTML === 'function')
    ? _capBuildQuizHTML(exs, 'm8ex', 'port2CheckEx')
    : '<p style="color:var(--ink4)">Motor de exercícios indisponível.</p>';
  dest.innerHTML = scoreBar + '<div id="port2-atribuir" style="margin:.2rem 0 .8rem"></div>' + quizHTML;
  if (typeof Atribuir !== 'undefined' && Atribuir.montar) {
    var _cm = _port2CapMeta.filter(function (m) { return m.n === cap; })[0] || {};
    var _sn = (_port2Subtemas[cap] && _port2Prat.st > 0) ? (_port2Subtemas[cap][_port2Prat.st - 1] || '') : '';
    Atribuir.montar('port2-atribuir', { curso: 'port2', cursoNome: 'Português 7.º', tema: String(cap), temaNome: (_cm.label || ('Cap. ' + cap)), sub: String(_port2Prat.st || ''), subNome: _sn, tipo: 'quiz', nivel: _port2Prat.nivel });
  }
}

function port2CheckEx(qid, tipo, val, btn) {
  if (_port2Prat.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return; // resposta vazia
  _port2Prat.answered[qid] = true;
  if (r.correct) _port2Prat.score.correct++;
  _port2Prat.score.total++; if(_port2TarefaAtiva){_port2TarefaResp[qid]=!!r.correct;}
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var se = document.getElementById('port2-prat-score'); if (se) se.textContent = _port2Prat.score.correct;
  var te = document.getElementById('port2-prat-total'); if (te) te.textContent = '/ ' + _port2Prat.score.total;
  var pe = document.getElementById('port2-prat-prog'); if (pe && _port2Prat.exs.length) pe.style.width = (_port2Prat.score.total / _port2Prat.exs.length * 100) + '%';
  port2SaveProgress(_port2Prat.cap, r.correct);
}

// ═══ PROGRESSO (localStorage por capítulo) ═══
function port2SaveProgress(cap, correct) {
  try {
    var key = _port2PratStorageKey(cap);
...
function _port2PM(cap, tipo, opts) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function _port2BuildCapRow(rowId, activeCap, onclickName) {
...
function _port2SetActiveCapBtn(rowId, btn, cap) {
  var row = document.getElementById(rowId);
...
function _port2FichaSlice(banco, n, dif) {
...
function _port2BuildMcQuestion(cap) {
  var qs = _port2FichaBanco(cap).filter(function (q) {
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
var _port2Quiz = { cap: 1, lives: 3, streak: 0, maxStreak: 0, score: 0, total: 0, answered: false, current: null };

function port2QuizBuildNav() {
  if (!_port2TemConteudo(_port2Quiz.cap)) _port2Quiz.cap = 1;
  _port2BuildCapRow('port2-quiz-cap-row', _port2Quiz.cap, 'port2QuizSelectCap');
  port2QuizStart();
}

function port2QuizSelectCap(cap, btn) {
  if (!_port2TemConteudo(cap)) return;
  _port2SetActiveCapBtn('port2-quiz-cap-row', btn, cap);
  _port2Quiz.cap = cap;
  port2QuizStart();
}

function port2QuizStart() {
  _port2Quiz.lives = 3; _port2Quiz.streak = 0; _port2Quiz.maxStreak = 0;
  _port2Quiz.score = 0; _port2Quiz.total = 0; _port2Quiz.answered = false;
  port2QuizNext();
}

function port2QuizNext() {
  var app = document.getElementById('port2-quiz-app');
  if (!app) return;
  if (_port2Quiz.lives <= 0) { port2QuizGameOver(app); return; }
  var ex = _port2BuildMcQuestion(_port2Quiz.cap);
  ...
  _port2Quiz.current = ex; _port2Quiz.answered = false;
  var lives = '';
  for (var i = 0; i < 3; i++) lives += (i < _port2Quiz.lives ? '❤️' : '🖤') + ' ';
  var opts = '';
  ex.opcoes.forEach(function(opt, idx) {
    opts += '<button class="qg-opt-btn" id="port2qopt-' + idx + '" onclick="port2QuizAnswer(' + idx + ')">' + opt + '</button>';
  });
  app.innerHTML =
    '<div class="qg-hub-bar">' +
      '<div class="qg-hub-lives">' + lives + '</div>' +
      '<div class="qg-hub-streak">' + (_port2Quiz.streak > 1 ? '🔥 ' + _port2Quiz.streak + ' seguidas' : '') + '</div>' +
      '<div class="qg-hub-score">✓ ' + _port2Quiz.score + ' / ' + _port2Quiz.total + '</div>' +
    '</div>' +
    '<div class="qg-hub-question">' + ex.enun + '</div>' +
    '<div class="qg-hub-opts">' + opts + '</div>' +
    '<div class="qg-hub-feedback" id="port2-quiz-fb" style="min-height:2.5rem"></div>';
}

function port2QuizAnswer(idx) {
  if (_port2Quiz.answered) return;
  _port2Quiz.answered = true;
  var ex = _port2Quiz.current; if (!ex) return;
  ...
  var clicked = document.getElementById('port2qopt-' + idx);
  if (clicked && !correct) { clicked.style.background = '#f44336'; clicked.style.color = '#fff'; }
  var fb = document.getElementById('port2-quiz-fb');
  if (correct) {
    _port2Quiz.score++; _port2Quiz.streak++;
    if (_port2Quiz.streak > _port2Quiz.maxStreak) _port2Quiz.maxStreak = _port2Quiz.streak;
    if (fb) fb.innerHTML = '<span style="color:var(--correct,#3ecf8e);font-weight:700">✓ Certo!</span> ' + (ex.expl || '');
  } else {
    _port2Quiz.lives--; _port2Quiz.streak = 0;
    if (fb) fb.innerHTML = '<span style="color:var(--wrong,#ff6b6b);font-weight:700">✗ Errado.</span> ' + (ex.expl || '');
  }
  port2SaveProgress(_port2Quiz.cap, correct);
  setTimeout(function(){ port2QuizNext(); }, 1400);
}

function port2QuizGameOver(app) {
  var pct = _port2Quiz.total > 0 ? Math.round(_port2Quiz.score / _port2Quiz.total * 100) : 0;
  _port2PM(_port2Quiz.cap, 'quiz', { pontuacao: _port2Quiz.score, total: _port2Quiz.total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<div style="color:var(--ink3);margin:.5rem 0 1.5rem">' + _port2Quiz.score + ' certas em ' + _port2Quiz.total + ' questões</div>' +
      '<div style="font-size:1.5rem;margin-bottom:1.5rem">Melhor sequência: ' + (_port2Quiz.maxStreak || 0) + ' 🔥</div>' +
      '<button class="btn btn-primary" onclick="port2QuizStart()">↺ Jogar novamente</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   SUB-MODO: FLASHCARDS
   ════════════════════════════════════════════════════════════════ */
var _port2Fc = { cap: 1, idx: 0, flipped: false, cards: [] };

function port2FcBuildNav() {
  // primeiro capítulo com cartões
  if (!(_port2Cards[_port2Fc.cap] && _port2Cards[_port2Fc.cap].length)) {
    for (var c = 1; c <= _port2CapMeta.length; c++) { if (_port2Cards[c] && _port2Cards[c].length) { _port2Fc.cap = c; break; } }
  }
  // linha de capítulos: só os que têm cartões
  var row = document.getElementById('port2-fc-cap-row');
  if (row) {
    var h = '';
    _port2CapMeta.forEach(function(m) {
      var has = !!(_port2Cards[m.n] && _port2Cards[m.n].length);
      var color = _port2CapColors[m.n] || '#516860';
      var isActive = _port2Fc.cap === m.n;
      var activeStyle = isActive ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
      var disabled = has ? '' : 'opacity:.45;cursor:not-allowed';
      var onclick = has ? ('port2FcSelectCap(' + m.n + ',this)') : '';
  ...
  port2FcSelectCap(_port2Fc.cap, null);
}

function port2FcSelectCap(cap, btn) {
  if (!(_port2Cards[cap] && _port2Cards[cap].length)) return;
  if (btn) _port2SetActiveCapBtn('port2-fc-cap-row', btn, cap);
  _port2Fc.cap = cap;
  _port2Fc.cards = _port2Cards[cap].slice();
  _port2Fc.idx = 0; _port2Fc.flipped = false;
  _port2PM(cap, 'flashcard');
  port2FcRender();
}

function port2FcRender() {
  var app = document.getElementById('port2-fc-app');
  if (!app) return;
  var cards = _port2Fc.cards;
  ...
  var card = cards[_port2Fc.idx];
  var color = _port2CapColors[_port2Fc.cap] || '#516860';
  var pct = Math.round((_port2Fc.idx + 1) / cards.length * 100);
  app.innerHTML =
    '<div class="card card-narrow">' +
      '<div class="flex items-center justify-between mb-3">' +
        '<span class="fc2-tag-pill" style="background:' + color + ';color:#fff">' + (card.tag || '') + '</span>' +
        '<span class="fc-counter">' + (_port2Fc.idx + 1) + ' / ' + cards.length + '</span>' +
      '</div>' +
      '<div class="fc2-prog-track"><div class="fc2-prog-fill" style="width:' + pct + '%;background:' + color + '"></div></div>' +
      '<div onclick="port2FcFlip()" class="fc2-card-inner" style="cursor:pointer;min-height:140px">' +
        '<div>' +
          '<p style="font-size:1.05rem;font-weight:600;color:var(--ink);line-height:1.4">' + (card.q || '') + '</p>' +
          '<p style="font-size:.9rem;color:' + color + ';margin-top:.75rem;line-height:1.5;display:' + (_port2Fc.flipped ? 'block' : 'none') + '">' + (card.a || '') + '</p>' +
          '<p style="font-size:.75rem;color:var(--ink4);margin-top:.5rem">' + (_port2Fc.flipped ? '' : 'Carrega para ver a resposta') + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="flex flex-wrap gap-3 justify-center mt-5">' +
        '<button class="btn btn-ghost" onclick="port2FcPrev()">← Anterior</button>' +
        '<button class="btn btn-primary" onclick="port2FcFlip()">↕ Virar</button>' +
        '<button class="btn btn-ghost" onclick="port2FcNext()">Seguinte →</button>' +
      '</div>' +
    '</div>';
}

function port2FcFlip() { _port2Fc.flipped = !_port2Fc.flipped; port2FcRender(); }
function port2FcNext() { _port2Fc.idx = (_port2Fc.idx + 1) % (_port2Fc.cards.length || 1); _port2Fc.flipped = false; port2FcRender(); }
function port2FcPrev() { _port2Fc.idx = (_port2Fc.idx - 1 + (_port2Fc.cards.length || 1)) % (_port2Fc.cards.length || 1); _port2Fc.flipped = false; port2FcRender(); }

/* ════════════════════════════════════════════════════════════════
   SUB-MODO: TESTE CRONOMETRADO
   ════════════════════════════════════════════════════════════════ */
var _port2Teste = { cap: 1, nivel: 'medio', qtd: 10, tempo: 600, exs: [], answered: {}, score: { correct: 0, total: 0 }, timer: null, restante: 0 };

function port2TesteBuildNav() {
  if (!_port2TemConteudo(_port2Teste.cap)) _port2Teste.cap = 1;
  _port2BuildCapRow('port2-teste-cap-row', _port2Teste.cap, 'port2TesteSelectCap');
  port2TesteRenderConfig();
}

function port2TesteSelectCap(cap, btn) {
  if (!_port2TemConteudo(cap)) return;
  _port2SetActiveCapBtn('port2-teste-cap-row', btn, cap);
  _port2Teste.cap = cap;
  port2TesteRenderConfig();
}

function port2TesteRenderConfig() {
  var app = document.getElementById('port2-teste-app');
  if (!app) return;
  if (_port2Teste.timer) { clearInterval(_port2Teste.timer); _port2Teste.timer = null; }
  app.innerHTML =
    '<div class="card">' +
      '<div class="card-title">Configurar teste</div>' +
      '<div class="flex flex-wrap gap-6 items-end">' +
        '<div><div class="label-small">Nível</div>' +
          '<div class="gen-level-group" id="port2-teste-nivel">' +
            '<button class="gen-level-btn' + (_port2Teste.nivel==='facil'?' active':'') + '" onclick="port2TesteSetNivel(\'facil\',this)"><span class="status-dot status-dot--easy"></span> Fácil</button>' +
            '<button class="gen-level-btn' + (_port2Teste.nivel==='medio'?' active':'') + '" onclick="port2TesteSetNivel(\'medio\',this)"><span class="status-dot status-dot--medium"></span> Médio</button>' +
            '<button class="gen-level-btn' + (_port2Teste.nivel==='dificil'?' active':'') + '" onclick="port2TesteSetNivel(\'dificil\',this)"><span class="status-dot status-dot--hard"></span> Difícil</button>' +
          '</div></div>' +
        '<div><div class="label-small">N.º questões</div>' +
          '<select class="ai-select" id="port2-teste-qtd"><option value="10">10</option><option value="15">15</option><option value="20">20</option></select></div>' +
        '<div><div class="label-small">Tempo</div>' +
          '<select class="ai-select" id="port2-teste-tempo"><option value="600">10 min</option><option value="900">15 min</option><option value="1800">30 min</option></select></div>' +
        '<button class="btn btn-primary" onclick="port2TesteStart()">▶ Iniciar teste</button>' +
      '</div>' +
    '</div>';
}

function port2TesteSetNivel(nivel, btn) {
  _port2Teste.nivel = nivel;
  var grp = document.getElementById('port2-teste-nivel');
  if (grp) grp.querySelectorAll('.gen-level-btn').forEach(function(b){ b.classList.remove('active'); });
  if (btn) btn.classList.add('active');
}

function port2TesteStart() {
  var gen = _port2Gerador(_port2Teste.cap);
  var qtdEl = document.getElementById('port2-teste-qtd'), tempoEl = document.getElementById('port2-teste-tempo');
  _port2Teste.qtd = qtdEl ? parseInt(qtdEl.value) : 10;
  _port2Teste.tempo = tempoEl ? parseInt(tempoEl.value) : 600;
  var exs = [];
  if (gen) {
    var nTemas = _port2TemasCount[_port2Teste.cap] || 1;
    var tipos = ['mc', 'fill', 'mc', 'vf', 'fill'];
    for (var i = 0; i < _port2Teste.qtd; i++) {
      var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _port2Teste.nivel);
      if (ex) exs.push(Object.assign({}, ex, { num: i + 1 }));
    }
  } else {
    // PT: teste sai do banco alargado do domínio
    exs = _port2FichaSlice(_port2FichaBanco(_port2Teste.cap), _port2Teste.qtd, _port2Teste.nivel);
  }
  if (!exs.length) return;
  _port2Teste.exs = exs; _port2Teste.answered = {}; _port2Teste.score = { correct: 0, total: 0 };
  _port2Teste.restante = _port2Teste.tempo;

  var app = document.getElementById('port2-teste-app');
  var quizHTML = (typeof _capBuildQuizHTML === 'function') ? _capBuildQuizHTML(exs, 'm8teste', 'port2TesteCheck') : '';
  app.innerHTML =
    '<div class="exam-progress-bar" style="position:sticky;top:0;z-index:5;background:var(--surface);padding:.75rem;border-radius:12px;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">' +
      '<div style="font-family:\'Cormorant Garamond\',serif;font-size:1.8rem;font-weight:900" id="port2-teste-timer">' + _port2FmtTime(_port2Teste.restante) + '</div>' +
      '<div style="font-size:.78rem;color:var(--ink3)">Respondidas: <span id="port2-teste-answered">0 / ' + _port2Teste.qtd + '</span></div>' +
      '<button class="btn btn-ghost ml-auto" onclick="port2TesteFinish()">⏹ Terminar</button>' +
    '</div>' +
    '<div id="port2-teste-questions" style="margin-top:1rem">' + quizHTML + '</div>';

  _port2Teste.timer = setInterval(function() {
    _port2Teste.restante--;
    var t = document.getElementById('port2-teste-timer');
    if (t) t.textContent = _port2FmtTime(_port2Teste.restante);
    if (_port2Teste.restante <= 0) port2TesteFinish();
  }, 1000);
}

function _port2FmtTime(s) {
  var m = Math.floor(s / 60), sec = s % 60;
  return (m < 10 ? '0' : '') + m + ':' + (sec < 10 ? '0' : '') + sec;
}

function port2TesteCheck(qid, tipo, val, btn) {
  if (_port2Teste.answered[qid]) return;
  if (typeof _capCheckAnswer !== 'function') return;
  var r = _capCheckAnswer(qid, tipo, val);
  if (!r) return;
  _port2Teste.answered[qid] = true;
  if (r.correct) _port2Teste.score.correct++;
  _port2Teste.score.total++;
  if (typeof _capShowFeedback === 'function') _capShowFeedback(qid, r.correct, r.expl, val, btn);
  var a = document.getElementById('port2-teste-answered');
  if (a) a.textContent = _port2Teste.score.total + ' / ' + _port2Teste.qtd;
  port2SaveProgress(_port2Teste.cap, r.correct);
  if (_port2Teste.score.total >= _port2Teste.qtd) setTimeout(port2TesteFinish, 600);
}

function port2TesteFinish() {
  if (_port2Teste.timer) { clearInterval(_port2Teste.timer); _port2Teste.timer = null; }
  var app = document.getElementById('port2-teste-app');
  if (!app) return;
  var total = _port2Teste.qtd;
  var correct = _port2Teste.score.correct;
  var pct = total > 0 ? Math.round(correct / total * 100) : 0;
  _port2PM(_port2Teste.cap, 'quiz', { pontuacao: correct, total: total });
  ...
  app.innerHTML =
    '<div style="text-align:center;padding:2.5rem 1rem">' +
      ...
      '<button class="btn btn-primary" onclick="port2TesteRenderConfig()">↺ Novo teste</button>' +
    '</div>';
}
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
function port2JogosInit() {
  (function(){ var pj=document.getElementById('port2p-jogos'); if(pj && !document.getElementById('port2-jogos-atr')){ var d=document.createElement('div'); d.id='port2-jogos-atr'; d.style.margin='0 0 .8rem'; if(pj.firstChild) pj.insertBefore(d,pj.firstChild); else pj.appendChild(d); } if(typeof Atribuir!=='undefined'&&Atribuir.montar) Atribuir.montar('port2-jogos-atr',{curso:'port2',cursoNome:'Português 7.º',tipo:'jogo',nivel:'',caps:_port2CapMeta.map(function(mm){return {n:mm.n,label:mm.label};})}); })();
  if (typeof ptJogosRender === 'function') {
    if (typeof ptJogosConfig === 'function') {
      ptJogosConfig({
        appId: 'port2-jogos-app',
        fonte: function () {
          var out = [];
          [1, 2, 3, 4, 5].forEach(function (c) { out = out.concat(_port2FichaBanco(c)); });
          return out;
        },
        forca: PT7_FORCA
      });
    ...
    var app = document.getElementById('port2-jogos-app');
...
/* ════════════════════════════════════════════════════════════════
   TAB PROGRESSO
   Lê edupt_port2_capN (gravado por port2SaveProgress). Mostra resumo
   global, stats, barras por capítulo, Relatório PDF e Limpar.
   ════════════════════════════════════════════════════════════════ */
function _port2ProgGetCaps() {
  var out = [];
  var nCaps = _port2CapMeta.length;
  for (var cap = 1; cap <= nCaps; cap++) {
    var correct = 0, total = 0;
    try {
      var d = JSON.parse(localStorage.getItem('edupt_port2_cap' + cap) || '{}');
...
function port2RenderProgresso() {
  var el = document.getElementById('port2-progresso-content');
  if (!el) return;
  var caps = _port2ProgGetCaps();
  ...
  h += '<div style="display:flex;align-items:center;gap:.75rem;background:#fdf0ef;border:1px solid #e8b4b0;border-radius:10px;padding:.65rem 1rem;margin-bottom:1.25rem;font-size:.82rem;color:#8b3a35;flex-wrap:wrap">'
     ...
     + '<button onclick="port2ProgDownloadPDF()" style="font-family:inherit;font-size:.82rem;font-weight:700;color:#8b3a35;background:none;border:none;cursor:pointer;text-decoration:underline;padding:0">Guarda o relatório PDF</button> para o manteres.</span>'
     + '</div>';
  ...
  h += '<div style="background:var(--m9c1-base);border:1.5px solid var(--m9c1-mid);border-radius:16px;padding:1.1rem 1.25rem;margin-bottom:1.25rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap">'
     ...
     +   '</div>'
     +   '<button class="btn btn-ghost" onclick="port2ProgDownloadPDF()" style="font-size:.78rem;padding:7px 14px;display:inline-flex;align-items:center;gap:5px"><i class="ph ph-file-text"></i>Relatório PDF</button>'
     + '</div>';
  ...
  caps.forEach(function(c) {
    var m = _port2CapMeta[c.cap - 1];
    var color = _port2CapColors[c.cap] || '#516860';
  ...
  if (totalT > 0) {
    ...
    if (pior) {
      var mp = _port2CapMeta[pior.cap - 1];
      var colorp = _port2CapColors[pior.cap] || '#516860';
      h += '<div class="card mt-4" style="border-color:' + colorp + '">'
        ...
        + '<button class="btn btn-primary" onclick="port2TreinarCap(' + pior.cap + ')" style="display:inline-flex;align-items:center;gap:6px"><i class="ph ph-target"></i> Treinar ' + mp.label + '</button>'
        + '</div>';
    }
  }
  ...
  // Limpar
  h += '<div class="mt-4 flex flex-wrap gap-3"><button class="btn btn-ghost" onclick="port2ProgReset()"><i class="ph ph-trash"></i> Limpar progresso</button></div>';

  el.innerHTML = h;
}

// Abre a tab Exercícios da Praticar já no capítulo indicado (treino direcionado).
function port2TreinarCap(cap) {
  _port2Prat.cap = cap;
  _port2Prat.st = 0;
  port2SwitchTab('exercicios', null);
}

function port2ProgReset() {
  ...
  for (var cap = 1; cap <= _port2CapMeta.length; cap++) { try { localStorage.removeItem('edupt_port2_cap' + cap); } catch (e) {} }
...
function port2ProgDownloadPDF() {
  ...
  if (typeof htmlToPdfDownload === 'function') htmlToPdfDownload(html, 'progresso-port2.pdf');
...
function _port2gfStsSel(cap) {
  var sel = _port2gf.sts[cap];
  if (!sel) return null;
  var out = [];
...
function _port2gfTemasSel(cap) {
  var sts = _port2gfStsSel(cap);
  if (!sts) return null;
  var mapa = _port2SubtemaTemas[cap] || {};
  var temas = [];
...
function port2FichasBuildNav() {
  if(typeof Atribuir!=='undefined'&&Atribuir.fixo) Atribuir.fixo('port2-fichas-atr','port2AtribuirFicha');
  var el = document.getElementById('port2-fichas-caps');
  if (!el) return;
  // por defeito, seleciona o primeiro capítulo com conteúdo
  var temAlgum = false;
  for (var k in _port2gf.caps) { if (_port2gf.caps[k]) { temAlgum = true; break; } }
  var h = '';
  _port2CapMeta.forEach(function(m) {
    var hasGen = _port2TemConteudo(m.n);
    if (!hasGen) return;
    if (!temAlgum) { _port2gf.caps[m.n] = true; temAlgum = true; } // 1.º disponível
    var sel = !!_port2gf.caps[m.n];
    var color = _port2CapColors[m.n] || '#516860';
    var style = sel ? 'background:' + color + ';border-color:' + color + ';color:#fff' : '';
    h += '<button class="gf-cap-btn' + (sel ? ' active' : '') + '" data-cap="' + m.n + '" onclick="port2gfToggleCap(' + m.n + ',this)" style="' + style + '">' + m.icon + ' ' + m.label + '</button>';
    // tópicos do capítulo selecionado
    if (sel) {
      var sts = _port2Subtemas[m.n] || [];
      if (sts.length) {
        var escolhidos = _port2gf.sts[m.n] || {};
        var algumSt = _port2gfStsSel(m.n) !== null;
        ...
        h += '<button onclick="port2gfToggleSt(' + m.n + ',0,this)" style="' + _port2gfStStyle(!algumSt, color) + '">Todos</button>';
        sts.forEach(function (st, i) {
          var on = !!escolhidos[i + 1];
          h += '<button onclick="port2gfToggleSt(' + m.n + ',' + (i + 1) + ',this)" style="' + _port2gfStStyle(on, color) + '">' + st + '</button>';
        });
        h += '</div>';
      }
    }
...
function _port2gfStStyle(on, color) {
...
function port2gfToggleCap(cap, btn) {
  _port2gf.caps[cap] = !_port2gf.caps[cap];
  if (!_port2gf.caps[cap]) delete _port2gf.sts[cap]; // desmarcar cap limpa os tópicos
  port2FichasBuildNav(); // re-render para mostrar/esconder a linha de tópicos
}

// idx 0 = «Todos» (limpa a seleção); idx 1..n alterna o tópico.
function port2gfToggleSt(cap, idx, btn) {
  if (idx === 0) {
    delete _port2gf.sts[cap];
  } else {
    if (!_port2gf.sts[cap]) _port2gf.sts[cap] = {};
    _port2gf.sts[cap][idx] = !_port2gf.sts[cap][idx];
    if (_port2gfStsSel(cap) === null) delete _port2gf.sts[cap]; // tudo desmarcado = todos
  }
  port2FichasBuildNav();
}

function port2gfToggleType(btn) {
  var t = btn.getAttribute('data-type');
  _port2gf.tipos[t] = !_port2gf.tipos[t];
  btn.classList.toggle('active', _port2gf.tipos[t]);
  var tick = btn.querySelector('.gf-tick'); if (tick) tick.textContent = _port2gf.tipos[t] ? '✓' : '';
}

function port2gfSetDif(btn) {
  _port2gf.dif = btn.getAttribute('data-dif');
  var grp = document.getElementById('port2-fichas-dif');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

function port2gfSetQty(btn) {
  _port2gf.qty = parseInt(btn.getAttribute('data-qty')) || 10;
  var grp = document.getElementById('port2-fichas-qty');
  if (grp) grp.querySelectorAll('.gf-dif-btn').forEach(function(b){ b.classList.remove('active'); });
  btn.classList.add('active');
}

// Renderiza um bloco de exercícios (com ou sem espaço de resposta).
function _port2gfExBloco(exs, startNum) {
  var h = '';
...
function _port2gfGenExs(cap, n) {
  var gen = _port2Gerador(cap);
  if (!gen) {
    ...
    var pool = _port2FichaBanco(cap);
    var temasSel = _port2gfTemasSel(cap);
    if (temasSel) {
      var filtrado = pool.filter(function (q) { return temasSel.indexOf(q.t) !== -1; });
      if (filtrado.length) pool = filtrado;
    }
    return _port2FichaSlice(pool, n, _port2gf.dif);
  }
  var nTemas = _port2TemasCount[cap] || 1;
  ...
  for (var i = 0, tent = 0; geradas.length < n && tent < n * 6; tent++) {
    var ex = gen(String((i % nTemas) + 1), tipos[i % tipos.length], _port2gf.dif);
    i++;
    if (!ex) continue;
  ...
  var banco = (typeof _port2Banco !== 'undefined' && _port2Banco[cap]) ? _port2Banco[cap] : [];
  if (banco.length && typeof _mixBancoGeradas === 'function') {
    return _mixBancoGeradas(banco, geradas, n, Math.min(Math.ceil(n / 3), banco.length), _port2gf.dif);
  }
  return geradas;
}

function port2gfGerar(formato) {
  // capítulos selecionados (conta o banco alargado — em PT não há geradores)
  var capsSel = [];
  _port2CapMeta.forEach(function(m) { if (_port2gf.caps[m.n] && (_port2Gerador(m.n) || _port2FichaBanco(m.n).length)) capsSel.push(m.n); });
  var status = document.getElementById('port2-fichas-status');
  if (!capsSel.length) { if (status) status.textContent = 'Seleciona pelo menos um capítulo.'; return; }
  var algumTipo = _port2gf.tipos.resumo || _port2gf.tipos.exercicios || _port2gf.tipos.teste || _port2gf.tipos.minitestes;
  ...
  var difLabel = { facil: 'Fácil', medio: 'Médio', dificil: 'Difícil' }[_port2gf.dif];
  var solucoes = []; // {num, ex} acumuladas para a secção final
  var solCounter = 0;

  var corpo = '';
  capsSel.forEach(function(cap, ci) {
    _port2PM(cap, 'ficha'); // gerou ficha deste capítulo
    var m = _port2CapMeta[cap - 1];
    var color = _port2CapColors[cap] || '#36527a';
    ...
    if (_port2gf.tipos.resumo) {
      var cards = _port2Cards[cap] || [];
      var stsR = _port2gfStsSel(cap);
      if (stsR && cards.length) {
        var palavras = [];
        stsR.forEach(function (i) {
          String((_port2Subtemas[cap] || [])[i - 1] || '').toLowerCase().split(/\s+/).forEach(function (w) {
  ...
  // Secção de soluções
  var solHTML = '';
  if (_port2gf.tipos.solucoes && eduPodeSolucoes() && solucoes.length) {
  ...
  var nomesCaps = capsSel.map(function(c) {
    var nome = _port2CapMeta[c - 1].label;
    var sts = _port2gfStsSel(c);
    if (sts) {
      var nomes = sts.map(function (i) { return (_port2Subtemas[c] || [])[i - 1] || ''; }).filter(function (x) { return x; });
  ...
  var _temasNomes = capsSel.map(function(c){ return _port2CapMeta[c-1].label; });
  var fname = (typeof _nomeFicha === 'function') ? _nomeFicha('9', 'portugues', _temasNomes, formato) : ('ficha-port2.' + (formato === 'html' ? 'html' : 'pdf'));
  if (formato === 'html') {
    try {
...
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', _port2Init);
else _port2Init();
...
_port2Banco[2] = _port2Banco[2].concat([
...
_port2Banco[1] = _port2Banco[1].concat([
...
_port2Banco[3] = _port2Banco[3].concat([
  // F1-F2 · Publicidade
...
/* atribuir: deep-link port2 */
function _port2DeepLinkAuto(){ try{ var p=new URLSearchParams(window.location.search); if(p.get('abrir')==='fichas'){ var cs=(p.get('caps')||'').split(',').filter(Boolean); if(_port2gf){ _port2gf.caps={}; cs.forEach(function(n){ _port2gf.caps[parseInt(n,10)]=true; }); if(p.get('dif')) _port2gf.dif=p.get('dif'); } if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Ficha concluída');},400); setTimeout(function(){ port2SwitchTab('fichas',null); },350); return; }
    if(p.get('abrir')==='jogos'){ var jc=parseInt(p.get('cap'),10); if(jc&&_port2Prat) _port2Prat.cap=jc; if(p.get('tarefa'))setTimeout(function(){tarefaEntregaBar(p.get('tarefa'),'Jogo concluído');},400); setTimeout(function(){ port2SwitchTab('jogos',null); var jg=p.get('jogo'); if(jg&&typeof ptJogoAbrir==='function')setTimeout(function(){try{ptJogoAbrir(jg);}catch(e){}},250); },350); return; }
    if(p.get('abrir')!=='praticar')return; if(p.get('tarefa')){_port2TarefaAtiva=p.get('tarefa');_port2TarefaResp={};} var cap=parseInt(p.get('cap'),10)||1, st=parseInt(p.get('st'),10)||0, nivel=p.get('nivel')||'medio'; _port2Prat.cap=cap; _port2Prat.st=st; _port2Prat.nivel=nivel; setTimeout(function(){ port2SwitchTab('exercicios',null); if(typeof port2GerarExercicios==='function') port2GerarExercicios(); },350); }catch(e){} }
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',function(){setTimeout(_port2DeepLinkAuto,300);});else setTimeout(_port2DeepLinkAuto,300);

function port2AtribuirFicha(){
  var caps=[]; _port2CapMeta.forEach(function(m){ if(_port2gf.caps[m.n]) caps.push(m.n); });
  if(!caps.length){ var st=document.getElementById('port2-fichas-status'); if(st) st.textContent='Escolhe pelo menos um capítulo para atribuir.'; return null; }
  var nomes=caps.map(function(n){ var mm=_port2CapMeta[n-1]||{}; return mm.label||('Cap. '+n); });
  return { curso:'port2', cursoNome:'Português 7.º', tema:caps.join('.'), temaNome:nomes.join(', '), sub:'', subNome:'', tipo:'ficha', nivel:_port2gf.dif };
}

function port2EntregarTarefa(){
  if(!_port2TarefaAtiva||typeof Turmas==='undefined'||!Turmas.guardarResultado)return;
  var qids=Object.keys(_port2TarefaResp||{});
  if(!qids.length){alert('Responde a pelo menos uma pergunta antes de entregar.');return;}
  var certas=0,detalhe=[];
  qids.forEach(function(qid,i){ var ok=!!_port2TarefaResp[qid]; if(ok)certas++; var ex=_port2Prat.exs[i]||{}; detalhe.push({q:(ex.enun||ex.pergunta||('Pergunta '+(i+1))),certo:ok}); });
  Turmas.guardarResultado(_port2TarefaAtiva,certas,qids.length,detalhe).then(function(){
    ...
    _port2TarefaAtiva=null; _port2TarefaResp={}; port2GerarExercicios();
